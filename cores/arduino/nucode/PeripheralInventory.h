...
namespace nucode::arduino
{
	...
	[[nodiscard]] constexpr PeripheralDmaCapability operator|(
		PeripheralDmaCapability lhs, PeripheralDmaCapability rhs) noexcept
	{
		return static_cast<PeripheralDmaCapability>(
			static_cast<std::uint8_t>(lhs) | static_cast<std::uint8_t>(rhs));
	}

	/** @brief DMA capability mask가 요청 비트를 포함하는지 확인합니다. */
	[[nodiscard]] constexpr bool hasPeripheralDmaCapability(
		PeripheralDmaCapability capabilities,
	...
	[[nodiscard]] const PeripheralDescriptor *peripheralInventoryAt(
		std::size_t index) noexcept;

	/** @brief kind와 instance가 모두 일치하는 실제 identity를 찾습니다. */
	[[nodiscard]] const PeripheralDescriptor *findPeripheral(
		PeripheralKind kind, std::uint8_t instance) noexcept;
	 ...
	 */
	[[nodiscard]] const PeripheralDescriptor *findPeripheralByObject(
		const char *public_object) noexcept;
	...
	[[nodiscard]] const char *peripheralVerificationStateToken(
		PeripheralVerificationState state) noexcept;
	 ...
	 */
	[[nodiscard]] std::size_t formatPeripheralIdentity(
		const PeripheralDescriptor &descriptor, char *buffer,
