...
namespace
{
    ...
    [[nodiscard]] int retriggerAssertedLevel(InterruptSlot &slot) noexcept
    {
        ...
        int result = gpio_pin_interrupt_configure(description->gpio.port, description->gpio.pin,
                                                  GPIO_INT_DISABLE);
        if (result < 0)
        {
            return result;
    ...
    void levelRearmHandler(struct k_work *work)
    {
        ...
        if (isLevelAsserted(trigger_kind, raw_value))
        {
            static_cast<void>(k_work_reschedule(&slot->level_rearm_work, level_rearm_interval));
            return;
        }
                              ...
                              gpio_port_pins_t pins)
    {
        ...
        if (slot->registered && slot->active)
        {
            ...
            const bool callback_masked = slot->suspended || (atomic_get(&callback_mask_depth) != 0);
            suppress_asserted_level = level_delivery && callback_masked;
            if (!callback_masked && (!level_delivery || atomic_cas(&slot->level_latched, 0, 1)))
            {
        ...
        else if ((callback_kind == CallbackKind::parameter) && (parameter_callback != nullptr))
        {
    ...
    int removeSlot(const PinDescription &description, InterruptSlot &slot) noexcept
    {
        ...
        if (was_registered)
        {
            const int disable_result = gpio_pin_interrupt_configure(
                description.gpio.port, description.gpio.pin, GPIO_INT_DISABLE);
            if (disable_result < 0)
            {
            ...
            const int remove_result =
                gpio_remove_callback(description.gpio.port, &slot.gpio_callback);
            if (remove_result < 0)
            {
                             ...
                             voidFuncPtrParam parameter_callback, void *parameter, PinStatus mode)
    {
        ...
        {
            result =
                gpio_pin_interrupt_configure(description->gpio.port, description->gpio.pin, flags);
            if (result < 0)
            {
...
namespace nucode::arduino::internal
{
                                       ...
                                       PinInterruptHandoverState &state) noexcept
    {
        ...
        const int result = gpio_pin_interrupt_configure(description->gpio.port,
                                                        description->gpio.pin, GPIO_INT_DISABLE);
        while (atomic_get(&slot.in_flight) != 0)
        {
            k_yield();
        }
    ...
    namespace
    {
        static_assert(sizeof(PinInterruptHandoverState) <= sizeof(SpiInterruptMaskToken::words),
        ...
        [[nodiscard]] bool validSpiInterrupt(int interrupt_number) noexcept
        {
            ...
            const PinDescription *const description = pinDescription(canonical_pin);
            return (description != nullptr) && !isGpioPinHandoverFaulted(canonical_pin) &&
                   hasPinCapability(description->capabilities, PinCapability::interrupt);
        }

        /** @brief SPI transaction 동안 지정한 GPIO interrupt 하나만 정지합니다. */
        [[nodiscard]] int suspendSpiInterrupt(int interrupt_number,
                                              SpiInterruptMaskToken &token) noexcept
        {
            ...
            lockGpioTransition();
            const int result =
                suspendInterruptForPinHandover(static_cast<std::size_t>(interrupt_number), state);
            if (result < 0)
            {
        ...
        SYS_INIT(initializeSpiInterruptMaskAdapter, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
    } // namespace
#endif
} // namespace nucode::arduino::internal

extern "C" void attachInterrupt(pin_size_t interrupt_number, voidFuncPtr callback, PinStatus mode)
{
...
extern "C" void detachInterrupt(pin_size_t interrupt_number)
{
    ...
    const int result = nucode::arduino::internal::detachInterruptForPinTransition(logical_pin);
    if (result < 0)
    {
        setGpioBackendError(GpioError::driver_error, result);
    }
