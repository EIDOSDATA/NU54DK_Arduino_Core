...
namespace
{
	...
	public:
		...
		bool setPins(pin_size_t, pin_size_t) noexcept override { return false; }
		...
		std::uint8_t endTransmission(bool) override { return 4U; }

		/** @brief Arduino Wire의 기타 오류 상태를 반환합니다. */
		std::uint8_t endTransmission() override { return 4U; }

		/** @brief 비활성 backend에서는 수신 byte가 없습니다. */
		std::size_t requestFrom(std::uint8_t, std::size_t, bool) override { return 0U; }

		/** @brief 비활성 backend에서는 수신 byte가 없습니다. */
		std::size_t requestFrom(std::uint8_t, std::size_t) override { return 0U; }
...
namespace
{
	...
	public:
		...
		std::uint8_t transfer(std::uint8_t) override { return 0U; }

		/** @brief 비활성 backend에서는 수신값 0을 반환합니다. */
		std::uint16_t transfer16(std::uint16_t) override { return 0U; }
...
extern "C" int analogRead(pin_size_t) { return -1; }
#endif
...
extern "C" bool analogWriteFrequency(pin_size_t, std::uint32_t) { return false; }
