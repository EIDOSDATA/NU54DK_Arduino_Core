...
namespace
{
	 ...
	 */
    [[nodiscard]] bool pollDeadline(std::uint64_t started_cycles, std::uint64_t timeout_cycles,
                                    bool cooperative, std::uint32_t &polls) noexcept
    {
        ++polls;
    ...
    {
        if ((state != static_cast<std::uint8_t>(LOW)) && (state != static_cast<std::uint8_t>(HIGH)))
        {
        ...
        setGpioBackendSuccess();
        return static_cast<unsigned long>((elapsed_microseconds == 0U) ? 1U : elapsed_microseconds);
    }
...
extern "C" unsigned long pulseInLong(pin_size_t pin, std::uint8_t state, unsigned long timeout)
{
...
extern "C" std::uint8_t shiftIn(pin_size_t data_pin, pin_size_t clock_pin, BitOrder bit_order)
{
    if (k_is_in_isr())
    {
        setGpioBackendError(GpioError::invalid_context);
