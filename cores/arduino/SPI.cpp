...
namespace
{
	...
	class ZephyrSPI final : public arduino::HardwareSPI
	{
	public:
		...
		void beginTransaction(arduino::SPISettings settings) override
		{
			...
			else
			{
				...
				if (buildSpiConfiguration(settings, configuration))
				{
		...
		/** @brief interrupt masking 연동은 v0.1에서 제공하지 않습니다. */
		void usingInterrupt(int interrupt_number) override
		{
			ARG_UNUSED(interrupt_number);
