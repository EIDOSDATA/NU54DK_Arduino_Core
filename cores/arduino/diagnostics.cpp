...
namespace nucode::arduino
{
    namespace
    {
        ...
        [[nodiscard]] constexpr Diagnostic
        unsupportedDiagnostic(DiagnosticSubsystem subsystem) noexcept
        {
        ...
        [[nodiscard]] Diagnostic gpioDiagnostic() noexcept
        {
            ...
            const int driver_error =
                code == DiagnosticCode::driver_error ? internal::lastGpioDriverError() : 0;
        ...
        [[nodiscard]] Diagnostic serialDiagnostic(DiagnosticSubsystem subsystem,
                                                  internal::SerialError error, int backend_error,
                                                  std::uint32_t dropped_bytes) noexcept
        {
            const auto code = serialCode(error);
            const int driver_error = code == DiagnosticCode::driver_error ? backend_error : 0;
            const std::uint32_t detail =
                error == internal::SerialError::rx_overflow
                    ? dropped_bytes
                    : (error == internal::SerialError::invalid_pin_route && backend_error > 0
        ...
        {
            return serialDiagnostic(DiagnosticSubsystem::serial, internal::lastSerialError(),
        ...
        {
            return serialDiagnostic(DiagnosticSubsystem::serial1, internal::lastSerial1Error(),
                                    internal::lastSerial1DriverError(),
                                    internal::serial1DroppedRxBytes());
        }
#endif
#endif
        ...
        [[nodiscard]] Diagnostic analogDiagnostic() noexcept
        {
            ...
            const int driver_error =
                code == DiagnosticCode::driver_error ? internal::lastAnalogDriverError() : 0;
    ...
    std::size_t formatDiagnostic(const Diagnostic &diagnostic, char *buffer,
                                 std::size_t capacity) noexcept
    {
        ...
        const int required = ::snprintf(
            buffer, capacity, "NU54:%s:%s:driver=%d:detail=%lu",
            diagnosticSubsystemToken(diagnostic.subsystem), diagnosticCodeToken(diagnostic.code),
            diagnostic.driver_error, static_cast<unsigned long>(diagnostic.detail));
