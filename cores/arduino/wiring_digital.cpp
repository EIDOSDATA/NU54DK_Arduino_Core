...
namespace
{
    ...
    [[nodiscard]] int recoverPendingPinInterrupt(PinRuntimeState &state) noexcept
    {
        ...
        const int result =
            nucode::arduino::internal::rollbackInterruptForPinHandover(state.interrupt_recovery);
        if (result == 0)
        {
    ...
    [[nodiscard]] bool suspendAnalogWriteForGpio(
        pin_size_t pin, nucode::arduino::internal::PwmRuntimeSuspendedOutput &snapshot) noexcept
    {
    ...
    [[nodiscard]] bool isOwnedByGpio(const PinDescription &description) noexcept
    {
        ...
        if (nucode::arduino::internal::ioResourceSnapshot(gpioIoResource(description.gpio),
    ...
    [[nodiscard]] gpio_flags_t runtimeModeFlags(const PinDescription &description,
                                                RuntimePinMode mode, bool output_latch) noexcept
    {
    ...
    [[nodiscard]] nucode::arduino::internal::PinHandoverResult
    handoverResult(IoResourceResult result) noexcept
    {
...
namespace nucode::arduino::internal
{
    ...
    bool isPinConfiguredForInput(std::size_t logical_pin) noexcept
    {
        ...
        const auto mode =
            static_cast<RuntimePinMode>(atomic_get(&pin_runtime_states[canonical_pin].mode));
        return (mode == RuntimePinMode::input) || (mode == RuntimePinMode::input_pullup) ||
    ...
    bool isPinConfiguredForOutput(std::size_t logical_pin) noexcept
    {
        ...
        const auto mode =
            static_cast<RuntimePinMode>(atomic_get(&pin_runtime_states[canonical_pin].mode));
        return (mode == RuntimePinMode::output) || (mode == RuntimePinMode::output_open_drain);
    }
    ...
    PinHandoverResult beginGpioPinHandover(std::size_t logical_pin, IoResourceOwner target_owner,
                                           GpioPinHandover &handover) noexcept
    {
        ...
        {
            reserve_result = reserveIoResources(
                target_owner, &resource, 1U, IoAcquirePolicy::exclusive, handover.ownership_lease);
        }
...
#if defined(CONFIG_NUCODE_ARDUINO_INTERRUPTS)
        const int interrupt_result =
            suspendInterruptForPinHandover(handover.canonical_pin, handover.interrupt);
        if (interrupt_result < 0)
        {
            const int interrupt_rollback = rollbackInterruptForPinHandover(handover.interrupt);
            const IoResourceResult ownership_rollback =
                rollbackIoResources(handover.ownership_lease);
            const bool cleanup_failed =
                interrupt_rollback < 0 || ownership_rollback != IoResourceResult::success;
            handover.phase =
                cleanup_failed ? PinHandoverPhase::faulted : PinHandoverPhase::rolled_back;
        ...
        if (handover.previous_gpio_owned && (previous_mode != RuntimePinMode::unconfigured))
        {
    ...
    PinHandoverResult rollbackGpioPinHandover(GpioPinHandover &handover) noexcept
    {
        ...
        if (handover.previous_gpio_owned && (previous_mode != RuntimePinMode::unconfigured))
        {
            driver_result = gpio_pin_configure(
                ...
                runtimeModeFlags(*description, previous_mode, handover.previous_output_latch));
        }
        const IoResourceResult ownership_result = rollbackIoResources(handover.ownership_lease);
#if defined(CONFIG_NUCODE_ARDUINO_INTERRUPTS)
...
#endif
        handover.phase = (ownership_result == IoResourceResult::success && driver_result >= 0)
                             ? PinHandoverPhase::rolled_back
                             : PinHandoverPhase::faulted;
        if (handover.phase == PinHandoverPhase::faulted && handover.canonical_pin < pin_slot_count)
        {
        ...
        return driver_result < 0 ? PinHandoverResult::driver_error : PinHandoverResult::success;
    }
    ...
    PinHandoverResult commitGpioPinHandover(GpioPinHandover &handover) noexcept
    {
        ...
        if (result != IoResourceResult::success)
        {
            ...
            return rollback_result == PinHandoverResult::success ? handoverResult(result)
                                                                 : rollback_result;
        }
        PinRuntimeState &runtime = pin_runtime_states[handover.canonical_pin];
    ...
    PinHandoverResult restoreGpioAfterPeripheral(GpioPinHandover &handover) noexcept
    {
        ...
        {
            const IoResourceResult release_result = releaseIoResources(handover.ownership_lease);
#if defined(CONFIG_NUCODE_ARDUINO_INTERRUPTS)
        ...
        if (commit_result != IoResourceResult::success)
        {
            ...
            const int interrupt_suspend_result =
                suspendInterruptForPinHandover(handover.canonical_pin, handover.interrupt);
#else
...
#endif
        handover.phase =
            interrupt_result < 0 ? PinHandoverPhase::faulted : PinHandoverPhase::rolled_back;
        if (handover.phase == PinHandoverPhase::faulted && handover.canonical_pin < pin_slot_count)
        {
            atomic_set(&pin_runtime_states[handover.canonical_pin].handover_faulted, 1);
        }
        unlockGpioTransition();
        return interrupt_result < 0 ? PinHandoverResult::driver_error : PinHandoverResult::success;
    }
...
void pinMode(pin_size_t pin, PinMode mode)
{
    ...
    const bool configured = [&]() noexcept -> bool
    {
        ...
        const RuntimePinMode previous_mode = static_cast<RuntimePinMode>(atomic_get(&state->mode));
        ...
        const int suspend_result = nucode::arduino::internal::suspendInterruptForPinHandover(
        ...
        if (commit_result != IoResourceResult::success)
        {
            const int restore_result =
                gpio_pin_configure(description->gpio.port, description->gpio.pin,
        ...
        const int interrupt_commit =
            nucode::arduino::internal::commitInterruptForPinHandover(state->interrupt_recovery);
        if (interrupt_commit < 0)
        {
            ...
            const int restore_result =
                gpio_pin_configure(description->gpio.port, description->gpio.pin,
                                   runtimeModeFlags(*description, previous_mode, previous_latch));
            transition_recoverable = interrupt_rollback >= 0 && restore_result >= 0;
            if (restore_result < 0)
            {
                atomic_set(&state->handover_faulted, 1);
            }
