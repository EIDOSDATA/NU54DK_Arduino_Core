...
namespace
{
	...
	atomic_t g_random_state = ATOMIC_INIT(
		static_cast<atomic_val_t>(nucode::arduino::internal::kRandomDefaultState));
	...
	[[nodiscard]] std::uint32_t nextRandom32() noexcept
	{
		for (;;)
		{
			...
			const auto next = nucode::arduino::internal::nextLcg32(
...
long random(long howsmall, long howbig)
{
	...
	const auto span = static_cast<std::uint32_t>(
		static_cast<std::int64_t>(howbig) - static_cast<std::int64_t>(howsmall));
