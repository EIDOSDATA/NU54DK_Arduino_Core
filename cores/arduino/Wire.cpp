...
#error                                                                                             \
    "NUCODE_M7_WIRE_CHOSEN_REQUIRED: Wire에는 app overlay의 nucode,arduino-wire chosen I2C controller가 필요합니다."
#endif
...
namespace
{
    ...
    nucode::arduino::internal::RuntimePeripheralRoute
        wire_route(wire_binding.device, wire_binding.pinctrl_config, wire_binding.owner,
      ...
      public:
        ...
        bool setPins(pin_size_t sda_pin, pin_size_t scl_pin) noexcept override
        {
            ...
            const auto result = nucode::arduino::internal::buildPeripheralRoute(
                nucode::arduino::internal::PinRoute::i2c22, pins, signals, 2U, configuration);
            const bool staged =
        ...
        void end() override
        {
            ...
            const bool route_present = was_started || wire_route.active() || wire_route.faulted();
		 ...
		 */
        std::size_t requestFrom(std::uint8_t address, std::size_t length, bool stop_bit) override
        {
            if (k_is_in_isr())
            {
                recordWireError(WireError::invalid_context);
            ...
            if (atomic_get(&wire_pending_restart) != 0)
            {
                ...
                result =
                    i2c_write_read(wire_device, address, tx_buffer, tx_length, rx_buffer, length);
                clearTransmitState();
            }
            else
            {
        ...
        int available() override
        {
            ...
            static_cast<void>(k_mutex_lock(&wire_mutex, K_FOREVER));
            const int count = wire_started ? static_cast<int>(rx_length - rx_position) : 0;
