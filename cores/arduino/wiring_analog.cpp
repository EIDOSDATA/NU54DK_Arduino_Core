...
#define NUCODE_ADC_SPEC_AND_COMMA(node_id, prop, index) ADC_DT_SPEC_GET_BY_IDX(node_id, index),
...
namespace
{
    ...
    const struct adc_dt_spec adc_inputs[] = {
        DT_FOREACH_PROP_ELEM(NUCODE_ARDUINO_ADC_NODE, io_channels, NUCODE_ADC_SPEC_AND_COMMA)};

    std::uint32_t configured_adc_channels = 0U;
    std::uint8_t analog_read_resolution = nucode::arduino::internal::analog_read_resolution_bits;
#endif
    ...
    std::uint8_t analog_write_resolution = nucode::arduino::internal::analog_write_resolution_bits;
    ...
    void recordPwmResult(PwmRuntimeResult result) noexcept
    {
        ...
        {
            const int driver_error = nucode::arduino::internal::lastPwmRuntimeDriverError();
            recordAnalogError(AnalogError::driver_error, driver_error != 0 ? driver_error : -EIO);
            break;
        }
        default:
            recordAnalogError(AnalogError::unsupported_devicetree);
    ...
    {
        const auto *const description = nucode::arduino::internal::pinDescription(pin);
        if (description == nullptr || description->analog_channel < 0 ||
            !nucode::arduino::internal::hasPinCapability(description->capabilities,
        ...
        {
            if (input.channel_id == static_cast<std::uint8_t>(description->analog_channel))
            {
                return &input;
            }
        }
    ...
    [[nodiscard]] bool adcDevicetreeContractMatches(const struct adc_dt_spec &input) noexcept
    {
    ...
    void rollbackAnalogRead(GpioPinHandover *handover, IoResourceLease &block_lease) noexcept
    {
        if (handover != nullptr)
        {
            static_cast<void>(nucode::arduino::internal::rollbackGpioPinHandover(*handover));
        }
    ...
    [[nodiscard]] std::uint32_t analogWriteFrequencyForPin(std::size_t canonical_pin) noexcept
    {
    ...
    void stopToneLocked() noexcept
    {
        ...
        const PwmRuntimeResult result =
            nucode::arduino::internal::pwmRuntimeStop(PwmRuntimeClient::tone, tone_state.pin);
    ...
    void toneStopWorkHandler(struct k_work *work)
    {
        ...
        if (tone_state.active && tone_state.scheduled_generation == tone_state.generation)
        {
            stopToneLocked();
        }
...
extern "C" int analogRead(pin_size_t pin)
{
    ...
    const auto block_resource =
        nucode::arduino::internal::peripheralIoResource(IoResourceKind::adc_block, 0U, input->dev);
    const IoResourceResult reserve_result = nucode::arduino::internal::reserveIoResources(
        {IoOwnerKind::adc, 0U}, &block_resource, 1U, IoAcquirePolicy::exclusive, block_lease);
    if (reserve_result != IoResourceResult::success)
    {
        recordAnalogError(AnalogError::driver_error, ownershipError(reserve_result));
        static_cast<void>(k_mutex_unlock(&analog_mutex));
        return -1;
    }

    GpioPinHandover handover{};
    const PinHandoverResult begin_result = nucode::arduino::internal::beginGpioPinHandover(
        canonical, {IoOwnerKind::adc, 0U}, handover);
    if (begin_result != PinHandoverResult::success)
    {
        static_cast<void>(nucode::arduino::internal::rollbackIoResources(block_lease));
        recordAnalogError(AnalogError::driver_error, handoverError(begin_result));
        static_cast<void>(k_mutex_unlock(&analog_mutex));
        return -1;
    }

    const std::uint32_t channel_mask = static_cast<std::uint32_t>(1UL) << input->channel_id;
...
extern "C" bool analogWriteFrequency(pin_size_t pin, std::uint32_t frequency_hz)
{
    ...
    if (!nucode::arduino::internal::frequencyToPeriodNanoseconds(frequency_hz, period_ns))
    {
        recordAnalogError(AnalogError::invalid_value);
        return false;
    }
    ...
    if (canonical >= NUM_PIN_ROLES ||
        !nucode::arduino::internal::pwmRuntimePinSupported(PwmRuntimeClient::analog_write,
                                                           static_cast<pin_size_t>(canonical)))
    {
        recordAnalogError(AnalogError::invalid_pin);
        return false;
    }
    ...
    static_cast<void>(k_mutex_lock(&analog_mutex, K_FOREVER));
    if (nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::analog_write,
                                                    static_cast<pin_size_t>(canonical)))
    {
        const PwmRuntimeResult result = nucode::arduino::internal::pwmRuntimeRetune(
            PwmRuntimeClient::analog_write, static_cast<pin_size_t>(canonical), period_ns);
...
extern "C" void analogWrite(pin_size_t pin, int value)
{
    ...
    const std::uint32_t pulse_ns = nucode::arduino::internal::scaleAnalogDutyToPulse(
        period_ns, static_cast<std::uint32_t>(value), analog_write_resolution);
    const PwmRuntimeResult result = nucode::arduino::internal::pwmRuntimeWrite(
        PwmRuntimeClient::analog_write, static_cast<pin_size_t>(canonical), period_ns, pulse_ns);
    recordPwmResult(result);
    static_cast<void>(k_mutex_unlock(&analog_mutex));
}
...
void tone(std::uint8_t pin, unsigned int frequency, unsigned long duration)
{
    ...
    const PwmRuntimeResult result = nucode::arduino::internal::pwmRuntimeWrite(
        PwmRuntimeClient::tone, static_cast<pin_size_t>(canonical), period_ns, period_ns / 2U);
    if (result == PwmRuntimeResult::success)
    {
        ...
        {
            static_cast<void>(
                k_work_reschedule(&tone_stop_work, K_MSEC(static_cast<std::int64_t>(duration))));
        }
    }
    else
    {
...
void noTone(std::uint8_t pin)
{
    ...
    static_cast<void>(k_mutex_lock(&tone_state_mutex, K_FOREVER));
    ++tone_state.generation;
    if (tone_state.active && tone_state.pin == static_cast<pin_size_t>(canonical))
    {
        stopToneLocked();
    }
    else
