...
namespace nucode::arduino::internal
{
    ...
    struct PwmRuntimeRouteBackend
    {
        ...
        PwmRuntimeResult (*block)(std::uint8_t block_instance, PwmRuntimeBlock &block) noexcept;
    ...
    [[nodiscard]] constexpr std::uint8_t pwmRuntimeBlockInstance(PwmRuntimeClient client) noexcept
    {
        switch (client)
        {
	 ...
	 */
    [[nodiscard]] bool
    installPwmRuntimeRouteBackend(const PwmRuntimeRouteBackend &backend) noexcept;

    /** @brief 지정 핀을 client용 PWM route로 사용할 수 있는지 확인합니다. */
    [[nodiscard]] bool pwmRuntimePinSupported(PwmRuntimeClient client, pin_size_t pin) noexcept;

    /**
	 * @brief 핀에 지정 주기와 pulse를 출력하거나 기존 출력을 갱신합니다.
	 */
    [[nodiscard]] PwmRuntimeResult pwmRuntimeWrite(PwmRuntimeClient client, pin_size_t pin,
                                                   std::uint32_t period_ns,
                                                   std::uint32_t pulse_ns) noexcept;

    /** @brief 활성 출력의 duty 비율을 보존하면서 주기만 변경합니다. */
    [[nodiscard]] PwmRuntimeResult pwmRuntimeRetune(PwmRuntimeClient client, pin_size_t pin,
                                                    std::uint32_t period_ns) noexcept;

    /** @brief 지정 client가 소유한 핀 출력을 중지하고 GPIO 상태를 복원합니다. */
    [[nodiscard]] PwmRuntimeResult pwmRuntimeStop(PwmRuntimeClient client, pin_size_t pin) noexcept;

    /** @brief 출력값을 snapshot에 보존한 채 route에서 안전하게 제거합니다. */
    [[nodiscard]] PwmRuntimeResult pwmRuntimeSuspend(PwmRuntimeClient client, pin_size_t pin,
                                                     PwmRuntimeSuspendedOutput &snapshot) noexcept;

    /** @brief 실패한 GPIO 전환 뒤 보존한 PWM 출력을 다시 적용합니다. */
    [[nodiscard]] PwmRuntimeResult pwmRuntimeResume(PwmRuntimeSuspendedOutput &snapshot) noexcept;

    /** @brief 핀의 client 출력이 활성 상태인지 확인합니다. */
    [[nodiscard]] bool pwmRuntimeActive(PwmRuntimeClient client, pin_size_t pin) noexcept;
