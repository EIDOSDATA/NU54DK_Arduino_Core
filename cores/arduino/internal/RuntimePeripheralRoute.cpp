...
namespace nucode::arduino::internal
{
    ...
    bool RuntimePeripheralRoute::activate() noexcept
    {
        ...
        for (; committed_pin_count_ < staged_configuration_.pin_count; ++committed_pin_count_)
        {
            const PinHandoverResult result =
    ...
    bool RuntimePeripheralRoute::deactivate() noexcept
    {
        ...
        const int pinctrl_result =
            pinctrl_update_states(pinctrl_config_, previous_states_, previous_state_count_);
        if (pinctrl_result < 0)
        {
            faulted_ = true;
            active_ = false;
