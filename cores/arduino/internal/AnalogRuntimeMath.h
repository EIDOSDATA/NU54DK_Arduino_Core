...
namespace nucode::arduino::internal
{
    ...
    [[nodiscard]] constexpr bool isSupportedAnalogReadResolution(std::uint8_t bits) noexcept
    {
        return bits == 8U || bits == 10U || bits == 12U || bits == 14U;
    }

    /** @brief PWM duty 입력에 허용하는 해상도인지 확인합니다. */
    [[nodiscard]] constexpr bool isSupportedAnalogWriteResolution(std::uint8_t bits) noexcept
    {
	 ...
	 */
    [[nodiscard]] constexpr std::uint32_t analogResolutionMaximum(std::uint8_t bits) noexcept
    {
        return bits >= 1U && bits <= 31U ? (static_cast<std::uint32_t>(1UL) << bits) - 1U : 0U;
    }
    ...
    scaleAnalogDutyToPulse(std::uint32_t period_ns, std::uint32_t value,
                           std::uint8_t resolution_bits) noexcept
    {
        ...
        const std::uint64_t scaled = static_cast<std::uint64_t>(period_ns) * value + maximum / 2U;
	 ...
	 */
    [[nodiscard]] constexpr bool frequencyToPeriodNanoseconds(std::uint32_t frequency_hz,
                                                              std::uint32_t &period_ns) noexcept
    {
        ...
        const std::uint64_t rounded = (nanoseconds_per_second + frequency_hz / 2U) / frequency_hz;
    ...
    [[nodiscard]] constexpr std::uint32_t
    rescalePulseForPeriod(std::uint32_t old_period_ns, std::uint32_t old_pulse_ns,
                          std::uint32_t new_period_ns) noexcept
    {
        ...
        const std::uint64_t scaled =
            static_cast<std::uint64_t>(new_period_ns) * old_pulse_ns + old_period_ns / 2U;
