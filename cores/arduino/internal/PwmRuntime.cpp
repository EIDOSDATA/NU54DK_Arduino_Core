...
namespace nucode::arduino::internal
{
	namespace
	{
		...
		[[nodiscard]] constexpr std::size_t clientCapacity(
			PwmRuntimeClient client) noexcept
		{
			return client == PwmRuntimeClient::tone ? 1U
													: pwm_runtime_channel_capacity;
		}

		/** @brief block에서 지정 핀을 소유한 slot을 찾습니다. */
		[[nodiscard]] PwmSlot *findSlot(PwmBlockState &block,
		...
		[[nodiscard]] PwmRuntimeRouteSet routeSet(
			const PwmBlockState &block, const PwmSlot *additional = nullptr,
		...
		{
			return pwm_set(runtime_block.device, slot.channel, slot.period_ns,
						   slot.pulse_ns, runtime_block.flags);
		}
									  ...
									  const PwmBlockState &block) noexcept
		{
			...
			for (const auto &slot : block.slots)
			{
				...
				const int result = pwm_set(runtime_block.device, slot.channel,
										   slot.period_ns, 0U, runtime_block.flags);
				if (first_error == 0 && result < 0)
				{
					first_error = result;
				}
		...
		[[nodiscard]] PwmRuntimeResult addSlot(PwmBlockState &state,
											   PwmSlot &slot,
											   const PwmSlot &desired) noexcept
		{
			...
			if (stop_result < 0)
			{
				...
				atomic_set(&last_driver_error,
						   static_cast<atomic_val_t>(stop_result));
			...
			if (driver_result < 0)
			{
				const int desired_stop_result = pwm_set(
					runtime_block.device, desired.channel, desired.period_ns, 0U,
					runtime_block.flags);
				const int old_stop_result = stopOutputs(runtime_block, state);
				const PwmRuntimeResult rollback_result = old_routes.count == 0U
															 ? route_backend.clear(state.instance)
															 : route_backend.apply(state.instance, old_routes);
				const int restore_result = restoreOutputs(runtime_block, state);
				atomic_set(&last_driver_error,
						   static_cast<atomic_val_t>(driver_result));
				if (desired_stop_result < 0 || old_stop_result < 0 ||
					rollback_result != PwmRuntimeResult::success || restore_result < 0)
				{
					const int fault_error = restore_result < 0 ? restore_result
															   : (old_stop_result < 0 ? old_stop_result
																					  : (desired_stop_result < 0 ? desired_stop_result : driver_result));
					latchBlockFault(state, fault_error);
					return PwmRuntimeResult::route_error;
				}
				return rollback_result == PwmRuntimeResult::success
						   ? PwmRuntimeResult::driver_error
		...
		[[nodiscard]] PwmRuntimeResult removeSlot(PwmBlockState &state,
												  PwmSlot &removed) noexcept
		{
			PwmRuntimeBlock runtime_block{};
			PwmRuntimeResult result = resolveBlock(state, runtime_block);
			if (result != PwmRuntimeResult::success)
			...
			if (restore_result < 0)
			{
				...
				atomic_set(&last_driver_error,
						   static_cast<atomic_val_t>(restore_result));
				if (stop_rollback_result < 0 ||
					rollback_result != PwmRuntimeResult::success ||
					output_rollback_result < 0)
				{
					const int fault_error = output_rollback_result < 0
	...
	bool installPwmRuntimeRouteBackend(
									 ...
									 std::uint32_t pulse_ns) noexcept
	{
		...
		if (slot != nullptr)
		{
			...
			const int result = pwm_set(runtime_block.device, slot->channel, period_ns,
									   pulse_ns, runtime_block.flags);
			if (result < 0)
			{
		...
		PwmSlot desired{pin, period_ns, pulse_ns,
						static_cast<std::uint8_t>(slot_index), true};
									  ...
									  std::uint32_t period_ns) noexcept
	{
		...
		const std::uint32_t pulse_ns = rescalePulseForPeriod(
			slot->period_ns, slot->pulse_ns, period_ns);
		const int driver_result = pwm_set(runtime_block.device, slot->channel,
										  period_ns, pulse_ns, runtime_block.flags);
		if (driver_result < 0)
		{
	...
	PwmRuntimeResult pwmRuntimeSuspend(
		...
		PwmRuntimeSuspendedOutput &snapshot) noexcept
	{
		...
		const PwmRuntimeSuspendedOutput saved{
			client, slot->pin, slot->period_ns, slot->pulse_ns, true};
		const PwmRuntimeResult result = removeSlot(*state, *slot);
		if (result == PwmRuntimeResult::success)
		{
			snapshot = saved;
	...
	PwmRuntimeResult pwmRuntimeResume(PwmRuntimeSuspendedOutput &snapshot) noexcept
	{
		...
		const PwmRuntimeResult result = pwmRuntimeWrite(
			snapshot.client, snapshot.pin, snapshot.period_ns, snapshot.pulse_ns);
		if (result == PwmRuntimeResult::success)
		{
