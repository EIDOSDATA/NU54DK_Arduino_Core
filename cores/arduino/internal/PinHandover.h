...
namespace nucode::arduino::internal
{
	 ...
	 */
    [[nodiscard]] PinHandoverResult beginGpioPinHandover(std::size_t logical_pin,
                                                         IoResourceOwner target_owner,
                                                         GpioPinHandover &handover) noexcept;

    /** @brief 성공한 주변장치 구성을 ownership manager에 확정합니다. */
    [[nodiscard]] PinHandoverResult commitGpioPinHandover(GpioPinHandover &handover) noexcept;

    /** @brief 실패한 주변장치 구성을 이전 GPIO·interrupt 상태로 되돌립니다. */
    [[nodiscard]] PinHandoverResult rollbackGpioPinHandover(GpioPinHandover &handover) noexcept;
	 ...
	 */
    [[nodiscard]] PinHandoverResult
    abandonGpioPinHandoverFailClosed(GpioPinHandover &handover) noexcept;
	 ...
	 */
    [[nodiscard]] PinHandoverResult restoreGpioAfterPeripheral(GpioPinHandover &handover) noexcept;
	 ...
	 */
    [[nodiscard]] PinHandoverResult
    releasePeripheralPinHandover(GpioPinHandover &handover) noexcept;

#if defined(CONFIG_NUCODE_ARDUINO_INTERRUPTS)
    /** @brief 특정 canonical 핀의 interrupt를 callback 보존 상태로 정지합니다. */
    [[nodiscard]] int suspendInterruptForPinHandover(std::size_t logical_pin,
                                                     PinInterruptHandoverState &state) noexcept;

    /** @brief 핀 전환 성공 뒤 보존했던 interrupt callback을 제거합니다. */
    [[nodiscard]] int commitInterruptForPinHandover(PinInterruptHandoverState &state) noexcept;

    /** @brief 핀 전환 실패 뒤 보존했던 interrupt callback과 trigger를 복원합니다. */
    [[nodiscard]] int rollbackInterruptForPinHandover(PinInterruptHandoverState &state) noexcept;
#endif
