...
namespace nucode::arduino::internal
{
    ...
    /** @brief compact token이 release에 필요한 자원과 세대입니다. */
    struct IoResourceTokenEntry
    {
        IoResourceId resource{};
        std::uint64_t generation{0U};
        bool changed{false};
    ...
    /**
     * @brief channel 단위 자원처럼 최대 두 항목을 즉시 획득·반환하는
     * token입니다.
     *
     * reserve와 driver 변경 사이 rollback이 필요한
     * 경로는 기존 IoResourceLease를
     * 사용합니다. 실패할 수 없는 register
     * 설정 앞에서 즉시 commit하는 event channel
     * 경로는 이 token으로 정적 RAM
     * 사용량을 줄입니다.
     */
    struct IoResourceToken
    {
        IoResourceOwner owner{};
        std::uint64_t manager_epoch{0U};
        std::size_t count{0U};
        IoResourceTokenEntry entries[io_resource_token_capacity]{};
        bool active{false};
    };
     ...
     *
     * manager는 겹치는 두 range를 같은 exclusive 자원으로 취급합니다. 같은
     * owner라도 경계가 다른 겹친 range를 중복 예약할 수 없으므로 부분 buffer
     ...
     *
     * @details driver와 pinctrl을 변경하기 전 호출하고 성공하면 기존 owner 상태를
     * lease에 보존합니다. commit하면 새 owner가 active가 되며 rollback하면 기존
     * generation까지 복구되어 이전 owner의 committed lease가 다시 유효해집니다.
     */
    [[nodiscard]] IoResourceResult
