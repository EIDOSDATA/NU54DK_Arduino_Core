...
namespace
{
    ...
    constexpr unsigned long console_baud = DT_PROP(NUCODE_ARDUINO_SERIAL_NODE, current_speed);
    ...
    SerialPortState console_state{console_device,
                                  &console_rx_queue,
                                  &console_lifecycle_mutex,
                                  &serial_tx_mutex,
                                  &console_diagnostics,
                                  console_baud,
                                  static_cast<std::uint16_t>(SERIAL_8N1)};

#if defined(CONFIG_NUCODE_ARDUINO_SERIAL1)
    const nucode::arduino::internal::PeripheralRouteBinding serial1_binding =
        nucode::arduino::internal::serial1RouteBinding();
    RuntimePeripheralRoute serial1_route(serial1_binding.device, serial1_binding.pinctrl_config,
                                         serial1_binding.owner, serial1_binding.block_kind,
                                         serial1_binding.block_index);
    SerialPortState serial1_state{serial1_binding.device, &serial1_rx_queue,
                                  &serial1_lifecycle_mutex, &serial1_tx_mutex,
                                  &serial1_diagnostics};
    ...
    {
        atomic_set(&state.diagnostics->driver_error, static_cast<atomic_val_t>(driver_error));
    ...
    [[nodiscard]] bool startRx(SerialPortState &state) noexcept
    {
        ...
        const int result = uart_irq_callback_user_data_set(state.device, serialIrqHandler, &state);
        if (result < 0)
        {
    ...
    class ConsoleSerial final : public arduino::HardwareSerial
    {
      public:
        ...
        void begin(unsigned long baud, std::uint16_t config) override
        {
            ...
            if ((get_result < 0) || !sameConfig(actual, requested))
            {
                record(console_state,
                       (get_result < 0) ? SerialError::driver_error
                                        : SerialError::unsupported_config,
                       get_result);
                return;
            }
        ...
        {
            return ::available(console_state);
        }
        int peek() override
        {
            return readByte(console_state, false);
        }
        int read() override
        {
            return readByte(console_state, true);
        }
        void flush() override
        {
            if (k_is_in_isr())
        ...
        {
            return atomic_get(&console_state.diagnostics->started) &&
                   device_is_ready(console_state.device);
        }
    };
    ...
    class Uart30Serial final : public nucode::arduino::Nu54HardwareSerial
    {
      public:
        bool setPins(pin_size_t rx_pin, pin_size_t tx_pin) noexcept override
        {
            ...
            const PeripheralSignal signals[]{PeripheralSignal::uart_rx, PeripheralSignal::uart_tx};
            ...
            const PeripheralRouteBuildError result =
                nucode::arduino::internal::buildPeripheralRoute(
                    nucode::arduino::internal::PinRoute::uart30, pins, signals, 2U, configuration);
            const bool staged =
                (result == PeripheralRouteBuildError::none) && serial1_route.stage(configuration);
            if (staged)
            {
        ...
        void begin(unsigned long baud, std::uint16_t config) override
        {
            ...
            if (atomic_get(&serial1_state.diagnostics->started) != 0)
            {
                ...
                else
                {
                    ...
                    const int configure_result =
                        stopped ? uart_configure(serial1_state.device, &requested) : -EIO;
            ...
            if (!serial1_route_staged)
            {
                ...
                serial1_route_staged = nucode::arduino::internal::defaultSerial1Route(route) ==
                                           PeripheralRouteBuildError::none &&
                                       serial1_route.stage(route);
            }
            if (!serial1_route_staged || !serial1_route.activate())
            {
                record(serial1_state, SerialError::route_error, serial1_route.lastDriverError());
                static_cast<void>(k_mutex_unlock(serial1_state.lifecycle_mutex));
                return;
            }
        ...
        void end() override
        {
            ...
            static_cast<void>(k_mutex_lock(serial1_state.lifecycle_mutex, K_FOREVER));
            const bool was_started = atomic_get(&serial1_state.diagnostics->started) != 0;
            const bool route_present = serial1_route.active() || serial1_route.faulted();
            if (was_started || route_present)
            {
                const bool rx_ok = !was_started || stopRx(serial1_state);
                const bool route_ok = rx_ok && (!route_present || serial1_route.deactivate());
                if (rx_ok && route_ok)
                {
                    record(serial1_state, SerialError::none);
                }
        ...
        int available() override
        {
            return ::available(serial1_state);
        }
        int peek() override
        {
            return readByte(serial1_state, false);
        }
        int read() override
        {
            return readByte(serial1_state, true);
        }
        void flush() override
        {
            ::flush(serial1_state);
        ...
        operator bool() override
        {
            return atomic_get(&serial1_state.diagnostics->started) &&
                   serial1_state.device != nullptr && device_is_ready(serial1_state.device);
        }
    };
#endif
