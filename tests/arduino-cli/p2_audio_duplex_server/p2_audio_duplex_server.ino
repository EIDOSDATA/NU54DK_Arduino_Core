/**
 * @file p2_audio_duplex_server.ino
 * @brief 공개 양방향 LE Audio server 예제를 실행하며 메모리 수명을 계측합니다.
 */
...
{
    publicDuplexSetup();
    Serial.println("P2_READY role=audio-duplex-server");
...
void loop()
{
    ...
    {
        p2Stopped = true;
        (void)audioServer.end();
        codec.end();
        BLEDevice.end();
        nucode::test::reportMemory("stopped");
        Serial.print("P2_STOP role=audio-duplex-server decoded=");
