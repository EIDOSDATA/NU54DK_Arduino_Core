...
ZTEST(ac01_contract, test_open_drain_is_limited_to_connector_gpio)
{
    ...
    zassert_equal(lastGpioError(), GpioError::none, "connector open-drain 구성이 실패했습니다.");
...
ZTEST(ac01_contract, test_pulse_and_shift_negative_contract)
{
    ...
    zassert_equal(pulseIn(PIN_GPIO0, HIGH, 100U), 0UL, "output pin pulse 측정을 허용했습니다.");
    zassert_equal(lastGpioError(), GpioError::wrong_mode,
                  "output pin pulse 오류가 wrong_mode가 아닙니다.");

    pinMode(PIN_GPIO1, INPUT_PULLUP);
    zassert_equal(pulseIn(PIN_GPIO1, 2U, 100U), 0UL, "잘못된 pulse state를 허용했습니다.");
    zassert_equal(lastGpioError(), GpioError::invalid_value, "잘못된 pulse state 진단이 다릅니다.");
