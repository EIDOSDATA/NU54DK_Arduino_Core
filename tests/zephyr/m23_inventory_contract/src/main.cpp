...
namespace
{
                                         ...
                                         IoResourceLease &lease) noexcept
    {
        ...
        return reserved == IoResourceResult::success ? commitIoResources(lease) : reserved;
    }
} // namespace
...
ZTEST(m23_inventory_contract, test_same_serial_block_is_exclusive)
{
    ...
    IoResourceLease spi{};
    zassert_equal(claim({IoOwnerKind::serial, 21U}, block21, uart), IoResourceResult::success,
                  "UARTE21 block 획득이 실패했습니다.");
...
ZTEST(m23_inventory_contract, test_overlapping_dma_ranges_fail_closed)
{
    ...
    zassert_equal(claim({IoOwnerKind::spi, 0U}, whole, owner), IoResourceResult::success,
                  "DMA range 준비가 실패했습니다.");
...
ZTEST(m23_inventory_contract, test_invalid_dma_ranges_are_rejected)
{
    ...
    for (const auto &resource : invalid)
    {
        ...
        zassert_equal(reserveIoResources({IoOwnerKind::application, 0U}, &resource, 1U,
                      ...
                      IoResourceResult::invalid_argument, "잘못된 DMA range가 reserve됐습니다.");
    }
}
