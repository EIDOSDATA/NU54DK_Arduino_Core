...
ZTEST(m25_event_fabric_contract, test_endpoint_domains_are_explicit)
{
    ...
    zassert_equal(timer20->task(TimerTask::start).role, EventEndpointRole::subscriber,
                  "TIMER task role 불일치");
    zassert_equal(egu20->event(0U).role, EventEndpointRole::publisher, "EGU event role 불일치");
    zassert_equal(ppib21->sendTask(0U).domain, 20U, "PPIB21 bridge domain 불일치");
    zassert_not_equal(ppib21->receiveEvent(0U).address, 0U, "PPIB receive endpoint 누락");
}
...
ZTEST(m25_event_fabric_contract, test_cross_domain_connect_fails_closed)
{
    ...
    zassert_equal(dppi20->connect(timer10->compareEvent(0U), timer20->task(TimerTask::start), 0U),
...
ZTEST_SUITE(m25_event_fabric_contract, nullptr, nullptr, nullptr, nullptr, nullptr);
