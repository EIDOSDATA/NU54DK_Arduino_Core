...
namespace
{
    ...
    bool configureInterruptFixture(bool released_high, const char *stage)
    {
        ...
        const int result =
            gpio_pin_configure(description->gpio.port, description->gpio.pin,
                               GPIO_INPUT | GPIO_OUTPUT | GPIO_OPEN_DRAIN | GPIO_PULL_UP | initial);
        if (result < 0)
        {
      ...
      private:
        ...
        bool cleanup()
        {
            ...
            if (description != nullptr)
            {
                ...
                release_result = gpio_pin_set_raw(description->gpio.port, description->gpio.pin,
                                                  deasserted_value);
            }
            ...
            if (release_result < 0)
            {
                nucode::arduino::internal::setGpioBackendError(GpioError::driver_error,
                                                               release_result);
                return false;
            }
            if (detach_error != GpioError::none)
            {
                nucode::arduino::internal::setGpioBackendError(detach_error, detach_driver_error);
                return false;
            }
            return true;
        }
    ...
    bool testOpenDrain(void)
    {
        ...
        Serial.println(
            "NUCODE_AC01_OPEN_DRAIN:PASS:low=LOW:released=HIGH:pullup=PIN_GPIO1_INTERNAL");
        return true;
    }
    ...
    unsigned long measureGeneratedPulse(std::uint32_t width_us, bool use_long_api)
    {
        ...
        static_cast<void>(k_thread_create(&pulse_thread_data, pulse_thread_stack,
                                          K_THREAD_STACK_SIZEOF(pulse_thread_stack), pulseThread,
                                          nullptr, nullptr, nullptr, K_PRIO_COOP(0), 0, K_NO_WAIT));
        const unsigned long measured = use_long_api ? pulseInLong(PIN_GPIO1, HIGH, 100000UL)
    ...
    bool testShift(void)
    {
        ...
        if (!requireSuccess("SHIFT_IN_API") || (low_value != 0x00U) || (high_value != 0xFFU))
        {
    ...
    bool testInterruptMask(void)
    {
        ...
        static_cast<void>(k_thread_create(&heartbeat_thread_data, heartbeat_thread_stack,
                                          K_THREAD_STACK_SIZEOF(heartbeat_thread_stack),
                                          heartbeatThread, nullptr, nullptr, nullptr,
        ...
        delay(5UL);
        if (!requireSuccess("MASK_FIRST_RESTORE") || (atomic_get(&level_callback_count) != 0))
        {
        ...
        Serial.print(
            "NUCODE_AC01_INTERRUPT_MASK:PASS:masked=0:nested=0:restored=1:heartbeat_delta=");
...
void setup(void)
{
    ...
    Serial.println("NUCODE_AC01_GPIO_HIL_READY:schema=1:gpio0=P2.5:gpio1=P2.6:wiring=P2.5_TO_P2.6:"
                   "irq=SW0_P1.13_SELF_OPEN_DRAIN");

    const bool passed = testOpenDrain() && testLowLevel() && testHighLevel() && testPulse() &&
                        testShift() && testInterruptMask();
    if (passed && !failed)
    {
