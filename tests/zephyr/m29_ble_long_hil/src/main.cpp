/**
 * @file main.cpp
 * @brief 두 NU54DK로 M29-W02의 512-byte long read를 반복 검증합니다.
 *
...
#ifndef M29_LONG_CORE_REVISION
#error "M29_LONG_CORE_REVISION is required"
#endif
...
namespace
{
    ...
    constexpr char protocol[] = "M29W02|1";
    constexpr char start_prefix[] = "M29W02|1|START|nonce=";
    constexpr char core_field[] = "|core=";
    constexpr char peer_name[] = "NU54-M29-LONG";
    constexpr char service_text[] = "8e7e2901-7d8c-4c1a-9d2d-8b6519f77410";
    constexpr char characteristic_text[] = "8e7e2902-7d8c-4c1a-9d2d-8b6519f77410";
    ...
    constexpr std::uint32_t required_reads = 100U;
    ...
    nucode::ble::BLECharacteristic test_characteristic(
        characteristic_uuid, nucode::ble::BLEProperty::read,
        nucode::ble::BLEPermission::read, payload_length);

    char nonce[nonce_text_length + 1U] = {};
    std::uint8_t nonce_binary[nonce_binary_length] = {};
    std::uint8_t expected_payload[payload_length] = {};
    char command[128] = {};
    ...
    [[maybe_unused]] std::uint32_t corrupt_reads = 0U;

    /** @brief 현재 image의 고정 role 이름을 반환합니다. */
    const char *roleName()
    {
#if defined(NUCODE_M29_LONG_CENTRAL)
        return "central";
#else
        return "peripheral";
#endif
    ...
    /** @brief 모든 결과 record에 exact nonce와 Core revision을 붙입니다. */
    void printSuffix()
    {
        Serial.print("|nonce=");
        Serial.print(nonce);
        Serial.print("|core=");
        Serial.print(M29_LONG_CORE_REVISION);
    }
    ...
    /** @brief nonce로 512-byte deterministic payload를 구성합니다. */
    void buildPayload()
    {
        ...
        {
            expected_payload[index] = index < nonce_binary_length
                                          ? nonce_binary[index]
                                          : static_cast<std::uint8_t>(
                                                nonce_binary[index % nonce_binary_length] ^
    ...
    bool acceptStartCommand()
    {
        ...
        if (::strncmp(revision_field, core_field, ::strlen(core_field)) != 0 ||
            ::strcmp(revision_field + ::strlen(core_field), M29_LONG_CORE_REVISION) != 0)
        {
            return false;
        }
    ...
    /** @brief 512-byte payload를 byte 단위로 검증합니다. */
    bool validPayload(const std::uint8_t *data, std::size_t length)
    {
        return data != nullptr && length == payload_length &&
               ::memcmp(data, expected_payload, payload_length) == 0;
    }

    /** @brief discovery 뒤 100회의 long read를 같은 exact handle에서 직렬 진행합니다. */
    void onClientEvent(const nucode::ble::BLEGattClientEventInfo &information, void *context)
    {
        ...
        if (!validPayload(information.data, information.length))
        {
            ++corrupt_reads;
            fail("read_payload");
            return;
        ...
        ++completed_reads;
        if (completed_reads < required_reads)
        {
            if (!BLEClient.read(connection_handle))
            {
                fail("read_repeat");
...
#else
        if (!test_characteristic.setValue(expected_payload, sizeof(expected_payload)))
        {
            fail("payload_seed");
            return;
        }
...
void setup()
{
    ...
    Serial.print(roleName());
    Serial.print("|core=");
    Serial.println(M29_LONG_CORE_REVISION);
}
