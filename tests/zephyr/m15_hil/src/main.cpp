...
namespace
{
    ...
    {
        std::uint32_t guard =
            state.magic ^ state.schema ^ static_cast<std::uint32_t>(state.stage) ^ 0xA55A15A5UL;
    ...
    bool validNonce(const char *nonce)
    {
        ...
        for (std::size_t index = 0U; index < nonce_length; ++index)
        {
            ...
            if ((::isdigit(character) == 0) && ((character < 'a') || (character > 'f')))
            {
                return false;
            }
        }
    ...
    void testGrtcAlarm(void)
    {
        ...
        requireSuccess(NU54DK.alarmAfterMicroseconds(alarm_delay_us, alarmCallback, nullptr),
                       "GRTC_ARM");
    ...
    {
        requireSuccess(NU54DK.storagePut(payload_key, expected_payload, sizeof(expected_payload)),
                       "SETTINGS_SAVE");
    ...
    void continueAfterSoftwareReset(ScenarioState &state)
    {
        ...
        requireSuccess(NU54DK.storageGet(payload_key, payload, sizeof(payload), actual_length),
                       "SETTINGS_LOAD");
...
void setup(void)
{
    ...
    const Error state_result = NU54DK.storageGet(state_key, &state, sizeof(state), state_length);
    ...
    reportState(state_is_valid ? &state : nullptr,
                state_is_valid ? stageName(state.stage) : (state_exists ? "corrupt" : "idle"));
