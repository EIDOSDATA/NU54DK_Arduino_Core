...
namespace
{
    ...
    {
        return instance >= 20U && instance <= 22U ? static_cast<int>(instance - 20U) : -1;
    }

    /** @brief allocator 시험에 허용할 고정 논리 핀 집합입니다. */
    [[nodiscard]] bool routeSupports(pin_size_t pin, std::uint8_t instance) noexcept
    {
        ...
        const pin_size_t supported[] = {
            PIN_PWM0, PIN_LED3, PIN_P1_02, PIN_P1_03, PIN_P2_00,
        };
    ...
    [[nodiscard]] PwmRuntimeResult routeBlock(std::uint8_t instance,
    ...
    [[nodiscard]] PwmRuntimeResult routeApply(std::uint8_t instance,
    ...
    void beforeEach(void *)
    {
        ...
        const PwmRuntimeRouteBackend backend = {
            routeSupports, routeBlock, routeApply, routeClear, routeDriverError,
        };
        zassert_true(nucode::arduino::internal::installPwmRuntimeRouteBackend(backend),
...
{
    zassert_true(IS_ENABLED(CONFIG_PINCTRL_DYNAMIC), "동적 pinctrl이 활성화되어야 합니다.");
    zassert_true(IS_ENABLED(CONFIG_PINCTRL_KEEP_SLEEP_STATE),
                 "sleep pinctrl state가 보존되어야 합니다.");
    zassert_true(IS_ENABLED(CONFIG_PM_DEVICE), "device PM이 활성화되어야 합니다.");
    zassert_true(IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME), "runtime device PM이 활성화되어야 합니다.");
...
ZTEST(ac02b_analog, test_adc_alias_resolution_and_ownership_policy)
{
    ...
    zassert_equal(nucode::arduino::internal::lastAnalogError(), AnalogError::invalid_value,
                  "9-bit ADC 요청이 거부되지 않았습니다.");
    zassert_equal(analogRead(A1), -1, "UART20이 소유한 AIN0을 ADC가 강제 탈취했습니다.");
    zassert_equal(analogRead(A5), -1, "PMIC INT가 소유한 AIN4를 ADC가 강제 탈취했습니다.");

    pinMode(A7, OUTPUT);
    zassert_equal(analogRead(A7), -1, "출력으로 구성한 GPIO가 암묵적으로 ADC로 전환되었습니다.");
...
ZTEST(ac02b_analog, test_analog_write_resolution_period_and_capacity)
{
    ...
    zassert_equal(route_observations[0].active_routes.count, 4U, "PWM20 route 수가 4가 아닙니다.");
    ...
    {
        zassert_equal(
            nucode::arduino::internal::pwmRuntimeStop(PwmRuntimeClient::analog_write, pins[index]),
            PwmRuntimeResult::success, "PWM20 channel을 정리하지 못했습니다.");
    }
}

ZTEST(ac02b_analog, test_allocator_route_failure_is_transactional)
{
    zassert_equal(nucode::arduino::internal::pwmRuntimeWrite(PwmRuntimeClient::analog_write,
                                                             PIN_PWM0, 1000000U, 500000U),
    ...
    zassert_equal(nucode::arduino::internal::pwmRuntimeWrite(PwmRuntimeClient::analog_write,
                                                             PIN_LED3, 1000000U, 250000U),
                  PwmRuntimeResult::route_error, "route 오류 주입 결과가 보존되지 않았습니다.");
    zassert_true(
        nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::analog_write, PIN_PWM0),
        "route 실패가 기존 PWM channel을 잃었습니다.");
    zassert_false(
        nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::analog_write, PIN_LED3),
...
ZTEST(ac02b_analog, test_output_restore_failure_latches_pwm_block)
{
    ...
    zassert_equal(nucode::arduino::internal::pwmRuntimeWrite(PwmRuntimeClient::analog_write,
                                                             PIN_LED3, 1000000U, 250000U),
                  PwmRuntimeResult::route_error, "복구 실패한 PWM block이 다시 사용되었습니다.");
    zassert_true(
        nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::analog_write, PIN_PWM0),
...
ZTEST(ac02b_analog, test_pin_mode_reclaims_analog_write_route)
{
    ...
    zassert_equal(digitalRead(PIN_PWM0), HIGH, "pinMode 후 GPIO output을 사용할 수 없습니다.");
}
...
ZTEST(ac02b_analog, test_pin_mode_preflight_and_pwm_resume_are_transactional)
{
    ...
    zassert_equal(nucode::arduino::internal::pwmRuntimeSuspend(PwmRuntimeClient::analog_write,
                                                               PIN_PWM0, snapshot),
    ...
    zassert_equal(nucode::arduino::internal::pwmRuntimeResume(snapshot), PwmRuntimeResult::success,
                  "보존한 snapshot 재시도에 실패했습니다.");
...
ZTEST(ac02b_analog, test_tone_duration_generation_and_single_channel)
{
    ...
    zassert_true(nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::tone, PIN_PWM0),
                 "유한 tone이 시작되지 않았습니다.");
    k_sleep(K_MSEC(5));
    tone(PIN_LED3, 2000U, 0UL);
    zassert_false(nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::tone, PIN_PWM0),
                  "재시작 전 tone이 남았습니다.");
    zassert_true(nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::tone, PIN_LED3),
                 "새 무기한 tone이 시작되지 않았습니다.");
    k_sleep(K_MSEC(30));
    zassert_true(nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::tone, PIN_LED3),
                 "이전 duration work가 새 generation tone을 중지했습니다.");
    noTone(PIN_LED3);
    zassert_false(nucode::arduino::internal::pwmRuntimeActive(PwmRuntimeClient::tone, PIN_LED3),
...
ZTEST(ac02b_analog, test_servo_four_channel_fixed_capacity)
{
    ...
    for (std::size_t index = 0U; index < 4U; ++index)
    {
        ...
        zassert_equal(servos[index].read(), 90, "Servo 각도와 pulse 변환이 다릅니다.");
    }
    zassert_equal(servos[4].attach(pins[4]), INVALID_SERVO,
                  "PWM22의 다섯 번째 Servo channel이 허용되었습니다.");
    zassert_equal(route_observations[2].active_routes.count, 4U, "PWM22 route 수가 4가 아닙니다.");
    for (std::size_t index = 0U; index < 4U; ++index)
    {
        servos[index].detach();
