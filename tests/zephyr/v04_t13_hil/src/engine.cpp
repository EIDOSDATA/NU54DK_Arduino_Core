...
namespace
{
    ...
    t13::Case selected_variant{};
    bool reverse_serial = false;
                           ...
                           std::uint32_t *out, std::uint32_t &count)
{
    ...
    if (opcode == 185U && nargs == 1U && args[0] <= 2U && !gate.claimed() && !wiringClaimed())
    {
    ...
    if (opcode == 113U && nargs == 1U && selected != nullptr && args[0] < selected->serial_count)
    {
        serialBusPins(args[0], out, count);
        return count == 8U ? 0U : 400U;
    }
