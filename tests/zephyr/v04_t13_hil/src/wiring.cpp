...
namespace
{
    ...
    IoResourceId resource(std::uint32_t net)
    {
        ...
        const auto pin = t13::physicalPin(role, net);
    ...
    bool prepare()
    {
        ...
        for (unsigned net = 0U; net < v04::wiring_count; ++net)
        {
            nrf_gpio_cfg_input(t13::physicalPin(role, net), NRF_GPIO_PIN_PULLUP);
        }
        return true;
    }
...
void t13::initializeWiring()
{
    for (unsigned net = 0U; net < v04::wiring_count; ++net)
    {
        nrf_gpio_cfg_input(t13::physicalPin(role, net), NRF_GPIO_PIN_NOPULL);
    }
}

bool t13::wiringClaimed()
{
    return gate.claimed();
}

void t13::wiringService()
{
    const auto now = static_cast<std::uint64_t>(k_uptime_get());
    if (pulse.expired(now))
    {
        nrf_gpio_cfg_input(t13::physicalPin(role, pulse.net()), NRF_GPIO_PIN_PULLUP);
        pulse.clear();
        ++pulse_expirations;
    }
...
std::uint32_t t13::wiringCommand(std::uint32_t opcode, const std::uint32_t *args,
                                 std::uint32_t nargs, std::uint32_t *out, std::uint32_t &count)
{
    wiringService();
    if (opcode == 48U && nargs == 4U)
    {
        if (t13::claimed() || args[0] != 501U ||
    ...
    if (opcode == 50U && nargs == 1U && gate.controller() == role)
    {
        ...
        const auto pin = t13::physicalPin(role, args[0]);
...
/** @brief 컴파일된 S/U 결선만 허용하며 C의 직결 표를 재사용하지 않습니다. */
std::uint32_t t13::physicalPin(unsigned selected_role, unsigned net)
{
    if ((selected_role != 1U && selected_role != 2U) || net >= 17U)
    {
        return UINT32_MAX;
    }
    return net_pins[selected_role == 1U ? 0U : CONFIG_NUCODE_T13_HARNESS - 1U][net];
}

/** @brief 물리 GPIO를 variant의 Arduino 핀 별칭으로 변환합니다. */
std::uint32_t t13::pinId(std::uint32_t physical)
{
    constexpr std::uint32_t pins[]{PIN_P1_14, PIN_P1_10, PIN_P1_04, PIN_P1_05, PIN_P1_06, PIN_P1_07,
                                   PIN_P0_00, PIN_P0_01, PIN_P0_02, PIN_P0_03, PIN_P2_00, PIN_P2_01,
                                   PIN_P2_02, PIN_P2_03, PIN_P2_04, PIN_P2_05, PIN_P2_06};
    for (unsigned net = 0U; net < 17U; ++net)
    {
        if (physicalPin(1U, net) == physical)
        {
            return pins[net];
        }
    }
