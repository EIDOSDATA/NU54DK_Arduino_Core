...
static_assert(EEPROMClass::maximum_size == 1024U, "EEPROM mirror 크기가 AC-03 계약과 다릅니다.");
static_assert(__is_base_of(Stream, File), "File은 Arduino Stream이어야 합니다.");
...
static_assert(DT_SAME_NODE(DT_CHOSEN(zephyr_code_partition), DT_NODELABEL(slot0_partition)),
...
ZTEST(ac03_storage_contract, test_public_bounds_are_fail_closed)
{
    ...
    zassert_equal(EEPROM.lastError(), EEPROMError::out_of_bounds, "끝 주소가 차단되지 않았습니다.");
}
