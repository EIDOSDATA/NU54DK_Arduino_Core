...
ZTEST(m24_uarte_driver_contract, test_all_five_adapters_stage_exact_routes)
{
    ...
    const Case cases[] = {
        {0U, SerialRouteClass::p2_dedicated20,
         SerialElectricalProfile::connector_fixture, pins00, 2U},
        {20U, SerialRouteClass::p2_dedicated20,
         SerialElectricalProfile::connector_fixture, pins20, 2U},
        {21U, SerialRouteClass::p1_flexible,
         SerialElectricalProfile::connector_fixture, pins21, 2U},
        {22U, SerialRouteClass::p1_flexible,
         SerialElectricalProfile::connector_fixture, pins22, 2U},
        {30U, SerialRouteClass::p0_flexible,
         SerialElectricalProfile::dap_uart_bridge, pins30, 2U},
    };
    for (std::size_t index = 0U; index < 5U; ++index)
    {
        ...
        zassert_equal(handle->stage(configuration(
                          cases[index].route, cases[index].electrical,
                          cases[index].pins, cases[index].pin_count, index)),
                      SerialFabricResult::success, "UARTE stage 실패");
    }
}

ZTEST(m24_uarte_driver_contract,
      test_inactive_transfer_and_invalid_baud_fail_closed)
{
    ...
    zassert_equal(handle->configure({12345U, UarteParity::none, false}),
                  ...
                  "지원하지 않는 baud를 허용했습니다.");
    zassert_equal(handle->transmitAsync(workspaces[2], 8U),
                  SerialFabricResult::wrong_state,
                  "inactive UARTE TX를 허용했습니다.");
    zassert_equal(handle->receiveAsync(workspaces[2], 8U),
                  SerialFabricResult::wrong_state,
