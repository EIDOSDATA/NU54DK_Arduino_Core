...
namespace
{
    ...
    [[nodiscard]] bool readLine(StreamType &stream, char *output, std::size_t capacity,
	 ...
	 */
    [[nodiscard]] bool requestHostRelay(const char *command, const char *response)
    {
        ...
        const int expected_count =
            snprintf(expected, sizeof(expected), "NUCODE_AC02B_RELAY:RESPONSE:%s:nonce=%s",
    ...
    [[nodiscard]] bool testSerial1(void)
    {
        ...
        for (unsigned int cycle = 0U; cycle < 2U; ++cycle)
        {
            ...
            if (!Serial1 || (nucode::arduino::internal::lastSerial1Error() != SerialError::none))
            {
            ...
            const int count = snprintf(frame, sizeof(frame), "S1:%s:%u", active_nonce, cycle);
            ...
            char expected[line_capacity]{};
            const int expected_count =
                snprintf(expected, sizeof(expected), "E1:%s:%u", active_nonce, cycle);
            if ((expected_count <= 0) ||
                (static_cast<std::size_t>(expected_count) >= sizeof(expected)) ||
    ...
    [[nodiscard]] bool wireRound(std::uint32_t clock_hz)
    {
        ...
        if (Wire.requestFrom(pmic_address, 1U, true) != 1U)
        {
            ...
            wire_failure_driver_error = nucode::arduino::internal::lastWireDriverError();
            return false;
        }
        if (Wire.available() != 1)
        {
    ...
    [[nodiscard]] bool testWire(void)
    {
        ...
        Serial.print("NUCODE_AC02B_DUT:WIRE:PASS:address=0x6A:register=0x0C:value=0x41:clocks="
                     "100000,400000:reads=2:restart=2:read-only=1");
        finishToken();
        return true;
    }
    ...
    [[nodiscard]] bool testSpi(void)
    {
        ...
        SPI.notUsingInterrupt(static_cast<int>(digitalPinToInterrupt(PIN_BUTTON0)));
        const bool spi_ok = nucode::arduino::internal::lastSpiError() == SpiError::none;
        SPI.end();
        detachInterrupt(digitalPinToInterrupt(PIN_BUTTON0));
        if (!spi_ok || (nucode::arduino::internal::lastSpiError() != SpiError::none))
        {
            return false;
        }
    ...
    [[nodiscard]] bool testAdc(int &low, int &high)
    {
        ...
        return (low >= 0) && (low <= 384) && (high >= 2500) && ((high - low) >= 2200);
    }
