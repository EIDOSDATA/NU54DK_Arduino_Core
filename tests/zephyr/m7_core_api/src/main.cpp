...
namespace
{
	 ...
	 */
    int observeI2cTransfer(const struct emul *target, struct i2c_msg *messages, int message_count,
                           int address)
    {
        ARG_UNUSED(target);
        ...
        for (int index = 0; index < recorded_count; ++index)
        {
            ...
            {
                for (std::size_t byte_index = 0U; byte_index < messages[index].len; ++byte_index)
                {
                           ...
                           const struct spi_buf_set *receive_buffers)
    {
        ...
        const auto *transmit = static_cast<const std::uint8_t *>(transmit_buffers->buffers[0].buf);
...
ZTEST(m7_wire, test_public_header_and_repeated_start_contract)
{
    ...
    zassert_equal(i2c_observation.call_count, 0, "no-STOP write가 read 전에 물리 전송되었습니다.");
...
ZTEST(m7_wire, test_zero_byte_address_probe_contract)
{
    Wire.begin();
    Wire.beginTransmission(test_i2c_address);
    zassert_equal(Wire.endTransmission(), 0U, "0-byte Wire address probe가 거부되었습니다.");
    ...
    zassert_equal(i2c_observation.lengths[0], 0U, "address probe가 0-byte write가 아닙니다.");
...
ZTEST(m7_wire, test_buffer_address_and_no_stop_read_validation)
{
    ...
    zassert_equal(lastWireError(), WireError::tx_buffer_overflow, "TX overflow 진단이 다릅니다.");
    ...
    zassert_equal(lastWireError(), WireError::invalid_address, "범위 밖 주소 진단이 다릅니다.");
...
ZTEST(m7_wire, test_clock_driver_error_and_restart_conflicts)
{
    ...
    i2c_observation.forced_result = -EIO;
    Wire.beginTransmission(test_i2c_address);
    zassert_equal(Wire.write(static_cast<std::uint8_t>(0x55U)), 1U,
                  "오류 주입 TX 구성에 실패했습니다.");
    zassert_equal(Wire.endTransmission(), 4U, "-EIO의 generic Arduino 상태 변환이 다릅니다.");
    zassert_equal(lastWireError(), WireError::driver_error,
                  "I2C driver 오류가 기록되지 않았습니다.");
...
ZTEST(m7_spi, test_all_modes_and_bit_orders)
{
         ...
         mode <= static_cast<int>(arduino::SPI_MODE3); ++mode)
    {
        for (const BitOrder order : orders)
        {
            ...
            SPI.beginTransaction(
                arduino::SPISettings(4000000U, order, static_cast<arduino::SPIMode>(mode)));
            ...
            zassert_equal((spi_observation.operation & SPI_MODE_CPOL) != 0U, expected_cpol,
                          "SPI CPOL 변환이 다릅니다.");
            zassert_equal((spi_observation.operation & SPI_MODE_CPHA) != 0U, expected_cpha,
                          "SPI CPHA 변환이 다릅니다.");
            zassert_equal((spi_observation.operation & SPI_TRANSFER_LSB) != 0U, order == LSBFIRST,
                          "SPI bit order 변환이 다릅니다.");
            SPI.endTransaction();
        }
    }
    ...
    {
        SPI.beginTransaction(arduino::SPISettings(frequency, MSBFIRST, arduino::SPI_MODE0));
...
ZTEST(m7_spi, test_transfer16_buffer_chunking_and_error_paths)
{
    ...
    zassert_equal(lastSpiError(), SpiError::invalid_buffer,
                  "null SPI buffer가 거부되지 않았습니다.");
}
...
ZTEST(m7_adc, test_adc_driver_error_is_preserved)
{
    ...
    zassert_equal(lastAnalogError(), AnalogError::driver_error,
                  "ADC driver 오류가 기록되지 않았습니다.");
...
ZTEST(m7_pwm, test_pwm_zero_middle_full_duty)
{
    ...
    for (const int value : values)
    {
        ...
        const std::uint32_t expected_pulse =
            (value == 255)
                ? 20000U
...
ZTEST(m7_pwm, test_pwm_validation_and_driver_error)
{
    ...
    zassert_equal(lastAnalogError(), AnalogError::invalid_pin,
                  "잘못된 PWM pin이 거부되지 않았습니다.");
    zassert_equal(fake_pwm_set_cycles_fake.call_count, 0U,
                  "잘못된 PWM pin이 driver에 전달되었습니다.");
    analogWrite(PIN_PWM0, -1);
    zassert_equal(lastAnalogError(), AnalogError::invalid_value,
                  "음수 PWM 값이 거부되지 않았습니다.");
    analogWrite(PIN_PWM0, 256);
    zassert_equal(lastAnalogError(), AnalogError::invalid_value,
                  "8-bit 초과 PWM 값이 거부되지 않았습니다.");

    fake_pwm_set_cycles_fake.return_val = -EIO;
    analogWrite(PIN_PWM0, 64);
    zassert_equal(lastAnalogError(), AnalogError::driver_error,
                  "PWM driver 오류가 기록되지 않았습니다.");
