...
namespace
{
	...
	[[nodiscard]] constexpr PinCapability expectedCapabilities(
		std::uint32_t raw, PinPolicy policy) noexcept
	{
...
#undef NUCODE_NU54DK_PHYSICAL_PIN
	...
	static_assert(LED_BUILTIN == 0U && PIN_BUTTON0 == 1U && PIN_A0 == 2U &&
					  PIN_PWM0 == 3U && PIN_LED1 == 4U && PIN_BUTTON3 == 9U,
	...
	void verifyPin(const ExpectedPin &expected)
	{
		...
		zassert_equal(actual->policy, expected.policy,
					  "descriptor policy metadata가 다릅니다.");
		zassert_equal(actual->routes, expected.routes,
					  "descriptor route metadata가 다릅니다.");
...
ZTEST(m14_variant_contract, test_all_31_physical_pads_are_generated_from_core_dts)
{
	...
	zassert_is_null(pinDescription(NUM_PIN_ROLES),
					"범위 밖 descriptor가 nullptr가 아닙니다.");
}
...
ZTEST(m14_variant_contract, test_legacy_id4_resolves_to_canonical_id3)
{
	...
	zassert_equal(pinDescription(PIN_LED1)->gpio.port,
				  DEVICE_DT_GET(DT_NODELABEL(gpio1)), "legacy LED1 controller가 P1이 아닙니다.");
...
ZTEST(m14_variant_contract, test_policy_and_port2_interrupts_fail_closed)
{
	for (const auto &expected : expected_pins)
	{
		...
		{
			zassert_false(hasPinCapability(description->capabilities,
