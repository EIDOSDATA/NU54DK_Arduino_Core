...
static_assert(ARDUINO_API_VERSION == 10502, "고정한 ArduinoCore-API 버전이 변경되었습니다.");
static_assert(same_type<pin_size_t, uint8_t>, "M4의 기본 Arduino 논리 핀 ABI는 8-bit여야 합니다.");
...
using AttachInterruptSignature = void (*)(pin_size_t, voidFuncPtr, PinStatus);

static_assert(same_type<decltype(static_cast<AttachInterruptSignature>(&attachInterrupt)),
