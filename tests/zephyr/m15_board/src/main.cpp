...
namespace
{
    ...
    [[maybe_unused]] void compileFullSurface(BoardSystem &board)
    {
        ...
        static_cast<void>(
            board.storageGet("contract", storage_value, sizeof(storage_value), actual_length));
...
ZTEST(m15_board, test_identity_and_read_only_state_use_production_backend)
{
    ...
    zassert_equal(::strcmp(NU54DK.boardTarget(), "nrf54l15dk/nrf54l15/cpuapp/nu54dk"), 0,
    ...
    ResetReport report = {};
    zassert_equal(NU54DK.resetReport(report), Error::none, "reset cause를 읽지 못했습니다.");
    zassert_equal(report.cause & ~report.supported, 0U, "지원 mask 밖 reset cause가 보고됐습니다.");

    const std::uint64_t uptime_before = NU54DK.uptimeMilliseconds();
    const std::uint64_t ticks_before = NU54DK.hardwareCounterTicks();
    k_sleep(K_MSEC(2));
    zassert_true(NU54DK.uptimeMilliseconds() >= uptime_before, "64-bit uptime이 역행했습니다.");
    zassert_true(NU54DK.hardwareCounterTicks() > ticks_before,
                 "GRTC hardware counter가 진행하지 않았습니다.");
    zassert_true(NU54DK.hardwareCounterFrequency() > 0U, "GRTC 주파수가 0입니다.");
    zassert_false(NU54DK.watchdogRunning(),
                  "시험이 시작하지 않은 watchdog을 실행 중으로 보고했습니다.");
    zassert_false(NU54DK.alarmPending(), "시험이 예약하지 않은 alarm을 대기 중으로 보고했습니다.");
    zassert_false(NU54DK.pmicWritesAuthorized(), "reset 뒤 PMIC write 승인이 자동 유지됐습니다.");
