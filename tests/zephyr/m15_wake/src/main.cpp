...
namespace
{
    ...
    void reportResetFailure(const char *stage, std::uint32_t expected, const ResetReport &report)
    {
        Serial.print("NUCODE_M15_SYSTEM_OFF_FAIL:stage=");
        Serial.print(stage);
    ...
    {
        const bool valid_phase = (state.phase == Phase::timed_armed) ||
                                 (state.phase == Phase::timed_passed) ||
                                 (state.phase == Phase::button_armed);
        return (state.magic == state_magic) && (state.schema == state_schema) && valid_phase &&
               validNonce(state.nonce);
    }
    ...
    Error loadState(ScenarioState &state, bool &exists)
    {
        ...
        const Error error = NU54DK.storageGet(state_key, &state, sizeof(state), actual_length);
    ...
    void abortPersistedResetState(const char *stage, std::uint32_t expected,
    ...
    const char *commandNonce(const char *command, const char *prefix)
    {
        ...
        if ((command == nullptr) || (::strncmp(command, prefix, prefix_length) != 0))
        {
            return nullptr;
        }
    ...
    void armTimedWake()
    {
        ...
        Serial.print("NUCODE_M15_SYSTEM_OFF_REQUEST:schema=2:phase=TIMED:nonce=");
        Serial.print(state.nonce);
        Serial.println(":duration_us=2000000");
        Serial.print("NUCODE_M15_SYSTEM_OFF_ENTERING:schema=2:phase=TIMED:nonce=");
        Serial.print(state.nonce);
    ...
    {
        Serial.print("NUCODE_M15_SYSTEM_OFF_READY:schema=2:phase=BUTTON:command=ARM_BUTTON:nonce=");
        Serial.print(state.nonce);
        ...
        Serial.print("NUCODE_M15_SYSTEM_OFF_REQUEST:schema=2:phase=BUTTON:nonce=");
        Serial.print(state.nonce);
        Serial.println(":wake=SW0:gpio=P1.13:active=LOW");
        Serial.print("NUCODE_M15_SYSTEM_OFF_ACTION:schema=2:phase=BUTTON:nonce=");
        Serial.print(state.nonce);
        Serial.println(":expected=PRESS_LOW:host_wait_ms=2000");
        Serial.print("NUCODE_M15_SYSTEM_OFF_ENTERING:schema=2:phase=BUTTON:nonce=");
        Serial.print(state.nonce);
    ...
    void continueAfterTimedWake(ScenarioState &state, const ResetReport &report)
    {
        ...
        if ((report.cause != expected) || ((report.supported & expected) != expected))
        {
        ...
        Serial.print("NUCODE_M15_SYSTEM_OFF_BOOT:schema=2:phase=TIMED_WAKE:nonce=");
        Serial.print(state.nonce);
        Serial.print(":cause=");
        Serial.print(static_cast<unsigned long>(report.cause));
        Serial.print(":supported=");
        Serial.println(static_cast<unsigned long>(report.supported));
        Serial.print("NUCODE_M15_SYSTEM_OFF_WAKE:PASS:phase=TIMED:nonce=");
        Serial.print(state.nonce);
    ...
    {
        const std::uint32_t expected = static_cast<std::uint32_t>(ResetCause::low_power_wake);
        if ((report.cause != expected) || ((report.supported & expected) != expected))
        {
        ...
        Serial.print("NUCODE_M15_SYSTEM_OFF_BOOT:schema=2:phase=BUTTON_WAKE:nonce=");
        Serial.print(state.nonce);
        Serial.print(":cause=");
        Serial.print(static_cast<unsigned long>(report.cause));
        Serial.print(":supported=");
        Serial.println(static_cast<unsigned long>(report.supported));
        Serial.print("NUCODE_M15_SYSTEM_OFF_WAKE:PASS:phase=BUTTON:nonce=");
        Serial.print(state.nonce);
...
void setup(void)
{
    ...
    if (!state_exists)
    {
        ...
        Serial.println("NUCODE_M15_SYSTEM_OFF_READY:schema=2:phase=TIMED:command=ARM_TIMED:"
                       "duration_us=2000000");
        armTimedWake();
        return;
    }
