#!/usr/bin/env python3
"""! @brief 세 NU54DK의 M30 동시 2-link 보안 연산을 자동 검증합니다. """
...
MILESTONE = "M30W07"
PROTOCOL = "M30W07|1"
TEST_ID = "M30-MULTI-01"
APPLICATION_SOURCE_ROOT = REPOSITORY / "tests" / "zephyr" / "m30_ble_multi_hil"
ROLES = ("peripheral", "mixed", "central")
DEFAULT_RESULT_TIMEOUT_SECONDS = 1200.0


class MultiSecurityExecutionFailure(BlePairHilFailure):
...
def parse_arguments(arguments: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="세 NU54DK로 M30-MULTI-01 동시 보안 연산을 검증합니다."
    )
    for role in ROLES:
...
def _write_line(serial_port: Any, line: str) -> None:
    ...
    serial_port.flush()
    if written != len(data):
        raise BlePairHilFailure("M30-W07 UART command가 일부만 기록됐습니다.")
...
def _wait_for(
    ...
    """! @brief 고정 stage까지 앞선 record도 identity 검사하며 수집합니다. """
...
def _save_failure_transcripts(paths: dict[str, Path], error: Exception) -> None:
    ...
    if isinstance(error, MultiSecurityExecutionFailure):
        for role in ROLES:
    ...
    return {
        ...
        "images": {
            role: image_record(
                ...
                executions[role], build_records[role]
            )
            for role in ROLES
        },
        "transcripts": {
        ...
        "coverage": {
            ...
            "security_errors": 0,
            "key_size_errors": 0,
            "m30_multi_01_status": "passed",
        },
        "binding": {
            "rf": "same-run-128-bit-nonce-and-role-marker",
            "target": "exact-core-revision-and-three-distinct-role-images",
            "links": "generation-handle-specific-security-operation",
        },
        "safety": {
...
def main(arguments: Sequence[str] | None = None) -> int:
    """! @brief 장치 탐색 또는 전체 M30-MULTI-01 gate를 실행합니다. """
    ...
    validate_source_clean(
        ...
        additional_paths=(
            ...
            HIL_DIRECTORY / "m30_ble_multi_protocol.py",
        ),
    )
    build_records = {
    ...
    try:
        ...
        raise
    print(f"NU54DK M30-MULTI-01 HIL PASS: evidence={evidence_path}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"NU54DK M30-W07 HIL FAIL: {error}", file=sys.stderr)
