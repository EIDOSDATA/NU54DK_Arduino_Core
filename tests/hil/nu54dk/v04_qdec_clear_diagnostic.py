...
def vectors():
    ...
    for repetition in range(1, 31):
        for offset in range(3):
            yield (offset + repetition - 1) % 3, repetition


def run(devices, current, append):
    """! @brief 누산 포화 아래400전이에서 clear 동작의 영향을 분리하며 실패는 보존합니다. """
    mismatches = []
    for strategy, repetition in vectors():
        label = f'V04-QDEC-CLEAR-DIAGNOSTIC/strategy{strategy}/repeat{repetition}'
        with qdec.armed(devices, current, append, label, 20, 0, observation_mode=3, read_strategy=strategy):
            ...
            expect(config, [strategy, int(strategy != 1), int(strategy != 2), bound, 128], 'clear strategy')
    ...
    append('V04-QDEC-CLEAR-DIAGNOSTIC/summary', {'status': 'observation', 'comparisons': 90,
