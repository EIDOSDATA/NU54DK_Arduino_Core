...
def main():
    ...
    if args.require_lc3:
        for image in (args.client_image, args.server_image):
    ...
    record = {
        "status": "FAIL",
        "test": "upstream_bap_unicast_bidirectional_sdu",
        ...
        "client_config_sha256": hashlib.sha256(
            (args.client_image.parent / ".config").read_bytes()
        ).hexdigest(),
        "server_config_sha256": hashlib.sha256(
            (args.server_image.parent / ".config").read_bytes()
        ).hexdigest(),
        "client_lines": [],
        "server_lines": [],
        "client_tx_streams": {},
    ...
    server_rx_pattern = re.compile(r"Incoming audio on stream .* len 40 \((\d+)\)")
    try:
        with ProbeLocks([client_uid, server_uid]):
            ...
            with serial.Serial(client_port, 115200, timeout=0.05) as client, \
                ...
                if len([count for count in record["client_tx_streams"].values()
                        if count >= 1000]) < 2:
                    raise RuntimeError("two client TX streams did not reach 1000 SDUs")
                ...
                if args.require_lc3:
                    ...
                    for key in ("client_lines", "server_lines"):
