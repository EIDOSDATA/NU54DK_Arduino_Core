#!/usr/bin/env python3
"""! @brief 두 NU54DK의 M29-W03 512-byte reliable write를 자동 검증합니다. """
...
PROTOCOL = "M29W03|1"
APPLICATION_SOURCE_ROOT = REPOSITORY / "tests/zephyr/m29_ble_long_write_hil"
DEFAULT_RESULT_TIMEOUT_SECONDS = 360.0


@dataclass(frozen=True)
class LongWriteResult:
    """! @brief 한 role의 고정 W03 protocol 결과입니다. """
...
def parse_role_transcript(
    ...
    if role not in ("peripheral", "central"):
        raise BlePairHilFailure(f"알 수 없는 W03 role입니다: {role}")
    ...
    try:
        ...
        raise BlePairHilFailure("W03 transcript에 비 ASCII noise가 있습니다.") from error
    ...
    if role == "peripheral":
        expected = (
            ...
            + suffix,
            link,
            f"{PROTOCOL}|RESULT|role=peripheral|writes=100|bytes=512|corrupt=0"
            f"|partial_commit=0|callback_context=pass".encode("ascii")
            + suffix,
            end,
        )
        result = LongWriteResult(role, 247, 0, 100, 512, 0, 0, 0, "pass")
    else:
        expected = (
            ready,
            begin,
            ...
            link,
            f"{PROTOCOL}|RESULT|role=central|reads=100|writes=100|bytes=512"
            f"|corrupt=0|stale=0|callback_context=pass".encode("ascii")
            + suffix,
            end,
        )
        result = LongWriteResult(role, 247, 100, 100, 512, 0, 0, 0, "pass")
    if tuple(lines) != expected:
        raise BlePairHilFailure(
            f"{role} W03 protocol 순서/값이 다릅니다: 기대={expected!r}, 실제={tuple(lines)!r}"
        )
    return result
...
def parse_arguments(arguments: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="두 NU54DK로 M29-W03 MTU 247·512-byte reliable write 100회를 검증합니다."
    )
...
def main(arguments: Sequence[str] | None = None) -> int:
    """! @brief 장치 탐색 또는 전체 W03 pair gate를 실행합니다. """
    ...
    print(
        "NU54DK M29-W03 pair discovery SUCCESS: "
    ...
    if peripheral_sha256 == central_sha256:
        raise BlePairHilFailure("W03 role HEX가 동일하여 오배치를 거부했습니다.")
    nonce = build_nonce()
    try:
        ...
        evidence = {
            "schema_version": 1,
            "gate": "m29-w03-long-reliable-write-pair-hil",
            "status": "passed",
            ...
            "coverage": {
                ...
                "long_write_bytes": 512,
                "long_write_iterations": 100,
                "payload_corruption": 0,
                "partial_commit": 0,
                "m29_long_01_status": "passed",
                "cancel_offset_overflow_negative": "host-tested",
            },
            "safety": {
        ...
        raise
    print(f"NU54DK M29-W03 long reliable write pair HIL PASS: evidence={evidence_path}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"NU54DK M29-W03 long reliable write pair HIL FAIL: {error}", file=sys.stderr)
