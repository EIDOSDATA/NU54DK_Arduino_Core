...
2026-09-07 확정한 **계획**이다. 실제 실행은 0회다. 이번 사용자 요청은 T12 현재 묶음과
이 계획의 확정까지이며 T13 soak를 시작하는 허가가 아니다. 현재 17신호 결선 C를 유지한다.
실행 정의는 [JSON](v04_t13_topologies.json), 생성·검사는 [계획 도구](v04_t13_plan.py)다.

2026-09-08 보완: **QDEC20/21 단독과 C07은 선행 기능 결함으로 HOLD**다.
[101번 진단](../../../00_Docs/04_검증%20기록/101_T12_QDEC_누산_누락_원인_분리.md)에서
동작 중 manual read/clear 누락을 재현했고 읽기 구간의 IRQ 보호만으로도 해결되지 않았다.
아래 5ms read 구성은 실행 허가가 아닌 보류한 원래 시험 목표다. 해당 경로를 해결하고
T12 기능240을 통과해야 이 구성의 안정성 시험을 시작한다. REPORT-only IRQ는 짧은20회가
일치한 대안 후보이며 기능·포화·큐 오류·연속 조건의 별도 검증 없이 자동 대체하지 않는다.
JSON의 `prerequisite_gates`와 해당 세 실행의 `required_gates`에 같은 제한을 고정했다.
...
System OFF 격리·재연결은 이 두 단계와 별도다. 예약 P2.07/08이나 PMIC 선은 추가하지 않는다.
...
순차 연속 측정만 계산하면 **32×180 + 7×900 + 3600 = 15,660초, 4시간 21분**이다.
이는 QDEC 보류 항목까지 포함한 전체 목표 시간이며 현재 곧바로 전부 실행할 수 있다는 뜻이 아니다.
...
T12 TIMER 반복 요구 대응·QDEC HOLD, 후속 runtime 변경의 영향 T11 회귀, T14 결함 조치, T15 지원 범위,
