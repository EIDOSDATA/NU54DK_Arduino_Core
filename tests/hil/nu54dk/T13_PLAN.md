...
증명 뒤 timer/GPIO wake를 실행하는 순서다. 초기 구현은 107번, 최신 준비·중계 실패·실기 잔여는
[110번](<../../../00_Docs/04_검증 기록/110_문서_정리와_T13_S_잔여_재개.md>)을 따른다.
...
전체 GPIO 연결성을 먼저 확인한다. 구형 runner의 시한 검사는 실기 재개 전에 현행 지시와 정합화하고
firmware lease·watchdog는 유지한다. C 세션 확인을 S/U 허가로 재사용하지 않는다. 분기 배선이나 외부 pull-up은 추가하지 않는다.
...
U 직전 자동 실행·peer 제어 System OFF의 현재 범위는 TODO와
[110번 기록](<../../../00_Docs/04_검증 기록/110_문서_정리와_T13_S_잔여_재개.md>)을 따른다.
