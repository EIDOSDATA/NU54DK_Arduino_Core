...
namespace nucode::arduino::internal
{
    namespace
    {
        ...
        [[nodiscard]] IoResourceResult registerFixedResources(IoResourceOwner owner,
                                                              ...
                                                              IoResourceLease &lease) noexcept
        {
            ...
            const IoResourceResult reserve_result =
                reserveIoResources(owner, resources, count, IoAcquirePolicy::exclusive, lease);
        ...
        void unwindFixedLease(IoResourceLease &lease, IoResourceResult &first_error) noexcept
        {
    ...
    IoResourceResult initializeNu54dkIoResources() noexcept
    {
        ...
        /**
         * @brief 비활성 콘솔은 핀과 serial20을 소유하지 않습니다.
         *
         * 직접 Fabric profile은 해당 block을 임대하기 전에 uart20을 의도적으로 비활성화합니다.
         */
        if (!DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(uart20)) || !IS_ENABLED(CONFIG_SERIAL))
        {
        ...
        const IoResourceId uart20_resources[] = {
            ...
            pselResource(DT_PROP_BY_IDX(DT_CHILD(DT_NODELABEL(uart20_default), group1), psels, 1)),
            pselResource(DT_PROP_BY_IDX(DT_CHILD(DT_NODELABEL(uart20_default), group2), psels, 0)),
