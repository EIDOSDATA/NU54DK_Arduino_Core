...
#define NUCODE_NU54DK_PHYSICAL_PIN(logical_pin, node_label)                                        \
    ...
    static_assert(                                                                                 \
        !((DT_PROP(DT_NODELABEL(node_label), nucode_route_mask) & NUCODE_PIN_ROUTE_PORT2) != 0 &&  \
...
namespace nucode::arduino::internal
{
    namespace
    {
        ...
        [[nodiscard]] constexpr PinCapability enabledCapabilities(std::uint32_t raw_capabilities,
        ...
        static_assert(static_cast<std::uint8_t>(PinPolicy::normal) == NUCODE_PIN_POLICY_NORMAL);
        ...
        static_assert(static_cast<std::uint32_t>(PinRoute::port2) == NUCODE_PIN_ROUTE_PORT2);

        const LogicalPinDescription pin_descriptions[] = {
#define NUCODE_NU54DK_PHYSICAL_PIN(logical_pin, node_label)                                        \
    {static_cast<std::size_t>(logical_pin),                                                        \
     {static_cast<std::size_t>(logical_pin), GPIO_DT_SPEC_GET(DT_NODELABEL(node_label), gpios),    \
      enabledCapabilities(                                                                         \
          DT_PROP(DT_NODELABEL(node_label), nucode_capability_mask),                               \
