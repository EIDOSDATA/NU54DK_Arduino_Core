...
namespace nucode::arduino::internal
{
	namespace
	{
		...
		constexpr pin_size_t invalid_logical_pin =
			static_cast<pin_size_t>(-1);
		...
		[[nodiscard]] bool routeMatrixAllows(PinRoute route, PeripheralSignal signal,
											 int port, std::uint32_t pin) noexcept
		{
		...
		[[nodiscard]] pinctrl_soc_pin_t defaultPsel(PeripheralSignal signal,
													int port, std::uint32_t pin) noexcept
		{
			switch (signal)
			{
		...
		[[nodiscard]] pin_size_t logicalPinForPhysical(std::uint32_t port,
													   std::uint32_t pin,
													   PinRoute required_route) noexcept
		{
				 ...
				 ++logical)
			{
				...
				if ((description == nullptr) ||
					!hasPinRoute(description->routes, required_route))
				{
					continue;
				}
				const int description_port = gpioPortNumber(description->gpio.port);
				if ((description_port == static_cast<int>(port)) &&
					(description->gpio.pin == pin))
				{
	...
	PeripheralRouteBuildError buildPeripheralRoute(
		PinRoute required_route, const pin_size_t *logical_pins,
		...
		PeripheralRouteConfiguration &configuration) noexcept
	{
		...
		for (std::size_t index = 0U; index < pin_count; ++index)
		{
			...
			{
				if (configuration.logical_pins[previous] ==
			...
			if (!routeMatrixAllows(required_route, signals[index], port,
			...
			{
				const PinDescription *const first =
					pinDescription(configuration.logical_pins[0]);
				if ((first == nullptr) || (gpioPortNumber(first->gpio.port) != port))
				{
					return PeripheralRouteBuildError::unsupported_route;
				}
			}
			const pinctrl_soc_pin_t psel =
				defaultPsel(signals[index], port, description->gpio.pin);
			...
			configuration.sleep_pins[index] =
				psel | (NRF_LP_ENABLE << NRF_LP_POS);
		}
		configuration.pin_count = pin_count;
		return PeripheralRouteBuildError::none;
	}

	PeripheralRouteBuildError defaultSerial1Route(
		PeripheralRouteConfiguration &configuration) noexcept
	{
	...
	PeripheralRouteBuildError defaultWireRoute(
		PeripheralRouteConfiguration &configuration) noexcept
	{
	...
	PeripheralRouteBuildError defaultSpiRoute(
		PeripheralRouteConfiguration &configuration) noexcept
	{
