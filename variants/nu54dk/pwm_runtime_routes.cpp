...
#include <cstdint>
    ...
    DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(pwm21)) && DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(pwm22))
...
namespace nucode::arduino::internal
{
    ...
    namespace
    {
        ...
        RuntimePeripheralRoute pwm20_runtime_route(DEVICE_DT_GET(DT_NODELABEL(pwm20)),
                                                   PINCTRL_DT_DEV_CONFIG_GET(DT_NODELABEL(pwm20)),
                                                   {IoOwnerKind::pwm, 20U},
                                                   IoResourceKind::pwm_block, 20U);
        RuntimePeripheralRoute pwm21_runtime_route(DEVICE_DT_GET(DT_NODELABEL(pwm21)),
                                                   PINCTRL_DT_DEV_CONFIG_GET(DT_NODELABEL(pwm21)),
                                                   {IoOwnerKind::pwm, 21U},
                                                   IoResourceKind::pwm_block, 21U);
        RuntimePeripheralRoute pwm22_runtime_route(DEVICE_DT_GET(DT_NODELABEL(pwm22)),
                                                   PINCTRL_DT_DEV_CONFIG_GET(DT_NODELABEL(pwm22)),
                                                   {IoOwnerKind::pwm, 22U},
                                                   IoResourceKind::pwm_block, 22U);
        ...
        [[nodiscard]] PwmRuntimeResult mapBuildError(PeripheralRouteBuildError error) noexcept
        {
            switch (error)
            {
        ...
        [[nodiscard]] PwmRuntimeResult mapRuntimeError(RuntimePeripheralRouteError error) noexcept
        {
            switch (error)
            {
        ...
        [[nodiscard]] PwmRuntimeResult
        buildConfiguration(PwmRouteState &state, const PwmRuntimeRouteSet &routes,
                           PeripheralRouteConfiguration &configuration) noexcept
        {
            if ((routes.count == 0U) || (routes.count > pwm_runtime_channel_capacity))
            {
        ...
        [[nodiscard]] bool
        restorePreviousRoute(PwmRouteState &state,
        ...
        [[nodiscard]] bool supportsRoute(pin_size_t pin, std::uint8_t block_instance) noexcept
        {
            ...
            return buildConfiguration(*state, route, configuration) == PwmRuntimeResult::success;
        }

        /** @brief allocator에 PWM block 장치와 polarity를 제공합니다. */
        [[nodiscard]] PwmRuntimeResult resolveBlock(std::uint8_t block_instance,
        ...
        [[nodiscard]] PwmRuntimeResult applyRoutes(std::uint8_t block_instance,
                                                   const PwmRuntimeRouteSet &routes) noexcept
        {
            ...
            const PeripheralRouteConfiguration previous_configuration = state->active_configuration;
            ...
            if (!state->runtime_route->stage(next_configuration))
            {
                ...
                const int original_driver_error = routeDriverError(*state->runtime_route);
                if (had_previous &&
                    !restorePreviousRoute(*state, previous_configuration, previous_routes))
                {
                    return PwmRuntimeResult::route_error;
                }
        ...
        const PwmRuntimeRouteBackend production_backend{
            supportsRoute, resolveBlock, applyRoutes, clearRoutes, lastDriverError,
        };
    } // namespace
#endif
...
namespace
{
    ...
    {
        static_cast<void>(nucode::arduino::internal::installNu54dkPwmRuntimeRouteBackend());
        return 0;
    }
} // namespace

SYS_INIT(initializeNu54dkPwmRuntimeRoutes, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
#endif
