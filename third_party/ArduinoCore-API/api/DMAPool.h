...
#define __CACHE_LINE_SIZE__ alignof(int)
    ...
    public:
        SPSCQueue(size_t size = 0) : capacity(0), tail(0), head(0), buff(nullptr)
        {
            if (size)
            {
                T *mem = new T[size + 1];
    ...
    public:
        DMABuffer(DMAPool<T, A> *pool = nullptr, size_t samples = 0, size_t channels = 0, T *mem = nullptr) : pool(pool), n_samples(samples), n_channels(channels), ptr(mem), ts(0), flags(0)
        {
        }
    ...
    public:
        DMAPool(size_t n_samples, size_t n_channels, size_t n_buffers, void *mem_in = nullptr) : mem((uint8_t *)mem_in), managed(mem_in == nullptr), wqueue(n_buffers), rqueue(n_buffers)
        {
