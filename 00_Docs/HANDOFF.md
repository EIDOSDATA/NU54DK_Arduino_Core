# 개발 인계 — M30-W08 사용자 중단·재개 준비 복구 필요
...
두 보드 preflight까지 당시 통과했습니다. 현재 구현 지점은 **M30-W08 `M30-POWER-01` 사용자 중단**입니다.
2026-09-15 문서 검토에서 필수 image 3개 부재를 확인했으므로 즉시 전원 시험을 시작할 수 없습니다.
병행한 **HOST-W01~W03 inventory·Host resolver·launcher**도 완료했습니다.
문서 개정은 개발·HIL 재개 허가가 아닙니다. 다음 개발 요청 시 아래 재개 순서와 사람 준비 확인을 먼저 적용합니다.
...
| M30 | W01~W07 완료, 작업 묶음 7/8·test ID 9/10 PASS. W08 중단·재개 준비 복구 필요 |
...
| M30 현재 중단점 | 사용자 중단. 재개 도구·산출물·preflight 복구 → 사람 준비 확인 → 실제 차단 |
...
7. 완료된 W07 exact `d94f5ec3…`의 세 role build와 `M30-MULTI-01` 증거를 보존합니다.
   당시 COM mapping을 현재 장비에 자동 적용하지 않습니다. W07 L2·16-byte key와 DFU L4는 별도 조건입니다.
8. [159번 기록](<04_검증 기록/159_M30_W08_전원_HIL_주입_직전_준비.md>)의 exact `05b639b4…`
   준비/preflight PASS는 역사적 근거로 보존합니다. 현재 산출물 부재·재개 조건은
   [160번 기록](<04_검증 기록/160_전체_문서_검토와_마일스톤_개정.md>)을 따릅니다.
9. [개정 실행 순서](<01_아두이노 코어 설계/18_문서_전면검토와_개선_마일스톤.md>)의
   W08-A source/manifest 검증·실행 환경 안정화 → B 확정 source의 image/manifest 재생성·preflight를 먼저 완료합니다.
10. B 결과를 보고하고 사람의 준비 확인 후에만 C의 네 지점 × 3회 주입을 arm합니다.
    배터리·별도 전원·역급전을 확인해 실제 DUT 전원 상실이 성립해야 합니다.
    이어 D의 영향 회귀·문서/기계 원장·CI가 끝나야 W08과 M30을 완료합니다.
