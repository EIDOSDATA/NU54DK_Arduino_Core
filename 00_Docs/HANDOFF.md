...
right-size, CoC-only 역할의 범용 GATT 분리, GATT schema·client context·event/TX payload의 실제 용량
...
1. **메모리 최적화 P1부터** 진행합니다. P0에서 full profile의 API·capacity를 보존하고 lean role의
   ...
   schema·client context·event/TX payload capacity의 실제 연결은 완료했으며, server/client 방향과
