...
M29 ATT/GATT·L2CAP는 W02 완료, 2/8입니다. 이 문서는 완료한 시험을 재개하라는
...
| 다음 행동 | M29-W03 long/reliable write와 server prepare/execute 구현·시험 |

## M29-W01~W02 완료 상태

M29는 [착수 계약](<01_아두이노 코어 설계/16_M29_ATT_GATT_L2CAP_착수_계약.md>)에서 W01~W08,
10개 test ID와 고정 자원 상한을 정의했다. 현재 진행률은 **2/8**이며 정적 SDK candidate를 실제
...
[142번 기록](<04_검증 기록/142_M29_W02_link별_GATT_long_read.md>)에 있다. 이 결과는 W03의
long/reliable write를 포함하지 않으므로 `M29-LONG-01`은 아직 `NOT RUN`이다.
