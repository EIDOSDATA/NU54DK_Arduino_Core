...
총 44개 preset을 연결했고, 실제 공개 `.ino` clean smoke에서 Public Broadcast 조합 backend와 필요한
CAP/Broadcast backend만 남는 것을 확인했습니다. M31 인접 Host 147건과 P0/builder Host 33건, 총
180건이 PASS했습니다. 나머지 Audio profile 역할 확장과 ELF/map·RAM gate는 후속 작업입니다.
전체 Host suite는 1,465건 PASS(2 skip)이며 장치에는 접근하지
않았습니다.
...
| 메모리 최적화 | P0-1 resolver·P0-2 probe/cache·P0-3a Core adaptive 생성/clean-build 완료. P0-3b capacity·strict role gate·총 44개 preset source filter/clean-build 완료(ISO 11/11, Audio BAP 9/9, HAP 2/2, Audio Control 2/2, Media Control 2/2, Call Control 2/2, CAP 5/5, CSIP 2/2, PBP 2/2), 나머지 Audio profile 역할·ELF/map/RAM gate 진행 중 | [219번 계약](<04_검증 기록/219_M31_W06_메모리_점유_감사와_최적화_계약.md>) · [통합 설계 §5.0](<01_아두이노 코어 설계/21_M31_메모리_최적화_통합_설계.md#50-구현-진행-기록>) |
