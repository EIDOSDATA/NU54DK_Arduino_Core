...
right-size, CoC-only 역할의 범용 GATT 분리, GATT schema·client context·event payload의 실제 용량
연결까지 완료했습니다. 다음은 GATT server/client 방향과 shared TX payload 수명 경계입니다. 동적 high-water는
P2 경계로 남깁니다.
장치에는 접근하지 않았습니다.
...
1. **메모리 최적화 P1부터** 진행합니다. P0에서 full profile의 API·capacity를 보존하고 lean role의
   미사용 feature/source를 제거했습니다. Core pin/route, CoC-only의 범용 GATT 분리와 GATT schema
   schema·client context·event payload capacity의 실제 연결은 완료했으며, server/client 방향과 shared TX pool 수명을 이어서 다룹니다. final
