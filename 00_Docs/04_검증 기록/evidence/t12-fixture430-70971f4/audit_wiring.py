"""! @brief 현재 430과 다음 440의 GPIO·커넥터 대응을 원본 계약과 대조합니다. """
...
selected = [next(row for row in catalog['fixtures'] if row['id'] == value) for value in (430, 440)]
for fixture in selected:
    for link in fixture['links']:
        for role in ('dut', 'peer'):
            ...
            assert expected == net
assert selected[0]['family'] == 'i2s' and selected[1]['family'] == 'pdm'
payload = {'status': 'passed', 'current_fixture': 430, 'next_fixture': 440,
...
print('WIRING_CONTRACT_PASS=430,440;NEXT_FIXTURE_NOT_EXECUTED')
