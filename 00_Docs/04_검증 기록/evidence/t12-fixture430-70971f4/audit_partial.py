...
assert result['status'] == 'failed' and result['external_wiring_executed'] is True
...
assert len(plan) == 192
assert len(functional) == len({row['id'] for row in functional}) == 72
assert len(payloads) == 144
counts = []
for index, (controller, vector) in enumerate(plan[:72]):
...
assert entries[-1]['results'] == [{'role': 1, 'result': [0]}, {'role': 2, 'result': [0]}]
assert entries[-1]['cleanup_only'] is True
assert result['error'] == 'ProtocolError: signal firmware error role=1: [1, 1, 1, 0, 128, 32, 32, 1]'
assert plan[72] == (1, (48000, 24, 0, 32, 1, 0x13579BDF))
...
audit = {'audit_status': 'passed', 'campaign_status': 'failed', 'fixture_id': 430, 'source': SOURCE, 'attempt': 1,
    'functional_records': 72, 'cleanup_records': 73, 'campaign_records': 0, 'total_journal_records': len(entries),
    ...
    'counts': counts, 'continuous_elapsed_seconds': result['campaign'].get('continuous_elapsed_seconds'),
...
assert audit['received_words_both_roles'] == 31104
audit.update(failed_vector=list(plan[72][1]), failed_controller_role=1, unexecuted_vectors=119, whole_fixture_pass=False)
(WORK / 'partial-results-audit.json').write_text(json.dumps(audit, ensure_ascii=False, indent=2) + '\n', encoding='utf-8', newline='\n')
