...
for old, new in ((WORK.parent / "r13/u3r-artifact-index.json", WORK / "target-artifact-index.json"),
                 (Path(r"C:\u3r\m12-build-evidence.json"), WORK / "target-build-evidence.json")):
    assert not new.exists()
    shutil.copyfile(old, new)
index = json.loads((WORK / "target-artifact-index.json").read_text(encoding="utf-8"))
baseline = json.loads((WORK.parent / "t12-fixture408/target-artifact-index.json").read_text(encoding="utf-8"))
...
for role, target in enumerate(index["targets"], 1):
    ...
    rows.append({"scenario": target["scenario"], "source": SOURCE,
                 ...
                 "prior_source": "87b987d9ed50855e0134f2c637c00706572719a5", "artifacts": target["artifacts"]})
