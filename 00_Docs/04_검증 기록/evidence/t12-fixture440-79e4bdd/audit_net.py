...
path = work / 'net-isolation-probe.json'
...
rows = result['results']
assert len(rows) == 9 and result['functional_pass_claimed'] is False
mismatches = []
for row in rows[:8]:
...
audit = {'source': result['source'], 'observations': 32, 'mismatches': mismatches,
...
with (work / 'net-audit.json').open('x', encoding='utf-8', newline='\n') as output:
    output.write(json.dumps(audit, ensure_ascii=False, indent=2) + '\n')
assert not mismatches, mismatches
print('FIXTURE440_INDEPENDENT_CLOCK_GATE_NETS_PASS=32;PINS_RESTORED=4')
