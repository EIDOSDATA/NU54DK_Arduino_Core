...
with pair.evidence_session(WORK / 'net-isolation-flushed-retry.json', evidence) as journal:
    with ProbeLocks(UIDS), ExitStack() as stack:
        ...
        write_new(WORK / 'net-flushed-startup-pins.json', {'source': SOURCE, 'devices': [{'role': device.image['role'], 'pin_cnf': {str(pin): device.target.read32(0x500D8280 + pin * 4) for pin in (4, 5, 6, 7)}} for device in devices], 'capture': 'passive read after settled diagnostic canonical boot'})
