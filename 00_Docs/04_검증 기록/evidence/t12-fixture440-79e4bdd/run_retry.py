...
trace = install(pair, WORK / "setup-trace2.jsonl")
...
capture = (WORK / 'pdm-samples2.jsonl').open('x', encoding='utf-8', newline='\n')
...
try:
    raise SystemExit(runner.main(['--dut', UIDS[0], '--peer', UIDS[1], '--build-root', str(BUILD),
        ...
        '--evidence', str(WORK / 'fixture440-attempt2.json'), '--repetitions', '1', '--execute-fixture']))
