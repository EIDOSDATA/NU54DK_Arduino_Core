...
for fixture, source, relative, count in (
    ...
    records.append({'fixture_id': fixture, 'source': source, 'audit_sha256': hashlib.sha256(path.read_bytes()).hexdigest(), 'functional_pass': count, 'normal_vectors': audit['normal_vectors'], 'recovery_data_pass': audit['recovery_data_pass'], 'expected_error_pass': audit['expected_error_pass'], 'concurrent_pass': audit['spim00_twim22_concurrent_pass'], 'data_pass': audit['data_pass']})
