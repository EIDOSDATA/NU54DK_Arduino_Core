"""! @brief 실제 현재 source의 pair 산출물과 검증된 컴파일 입력을 대조합니다. """
...
for old, new in ((WORK.parent / "r13/u3p-artifact-index.json", WORK / "target-artifact-index.json"),
                 (Path(r"C:\u3p\m12-build-evidence.json"), WORK / "target-build-evidence.json")):
    assert not new.exists()
    shutil.copyfile(old, new)
index = json.loads((WORK / "target-artifact-index.json").read_text(encoding="utf-8"))
baseline = json.loads((WORK.parent / "t12-fixture407-resume/target-artifact-index.json").read_text(encoding="utf-8"))
...
for role, target in enumerate(index["targets"], 1):
    ...
    rows.append({"scenario": target["scenario"], "source": SOURCE,
                 ...
                 "compiled_sources_config_and_membership_unchanged": True,
                 "prior_source": "393e419f4c855037d4e6221c315f9be808a7d274", "artifacts": target["artifacts"]})
(WORK / "build-input-comparison.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2) + "\n", encoding="utf-8", newline="\n")
print("EXACT_PAIR_BUILD_INPUT_AUDIT_PASS=2;SOURCE=" + SOURCE)
