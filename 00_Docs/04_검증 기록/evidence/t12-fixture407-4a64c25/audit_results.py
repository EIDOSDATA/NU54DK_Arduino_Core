"""! @brief 독립 입력 pull-down/up/down 계획과 실제 raw sample·GPIO·DMA·cleanup을 대조합니다. """
...
path = work / 'fixture407-attempt1.json'
...
assert result['fixture_id'] == 407 and result['core_revision'] == source
assert result['fixture_revision'] == 5 and result['swd_frequency_hz'] == 10000000
...
for vector in plan:
    ...
    assert row['id'] == f'V04-ANALOG-SIGNAL/407/2/{vector}/repeat-1'
    assert row['status'] == 'passed' and row['scope'] == 'input-bias-shared-ain6-manual-saadc'
...
audit = {'status': 'passed', 'fixture_id': 407, 'source': source, 'attempt': 1,
         ...
         'functional_scope': 'AIN6 button-unpressed shared input INPUT pulldown/pullup/pulldown, single/double SAADC DMA and cleanup',
         'not_measured': ['button actuation, debounce or wake behavior', 'calibrated voltage', 'calibrated ADC precision', 'PWM period/duty', 'Fixture 408 and later T12', 'T13 soak/concurrency'],
