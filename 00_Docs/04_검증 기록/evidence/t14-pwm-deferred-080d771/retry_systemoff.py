...
command=[str(cli),'--config-file',str(root/'arduino-cli.yaml'),'compile','--verbose','--fqbn','nucode:zephyr:nu54dk','--board-options','feature_set=standard','--build-path',str(root/'retry-systemoffwake'),str(platform/'libraries/NUCODE_NU54DK/examples/SystemOffWake')]
result=subprocess.run(command,cwd=root)
print('RETRY_SYSTEMOFF_EXIT='+str(result.returncode),flush=True)
