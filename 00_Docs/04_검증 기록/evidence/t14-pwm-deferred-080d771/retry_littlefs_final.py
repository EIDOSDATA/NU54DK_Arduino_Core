...
command=[str(cli),'--config-file',str(root/'arduino-cli.yaml'),'compile','--verbose','--fqbn','nucode:zephyr:nu54dk','--board-options','feature_set=standard','--build-path',str(root/'retry-littlefs-final'),str(platform/'libraries/LittleFS/examples/LittleFSPersistence')]
result=subprocess.run(command,cwd=root)
print('RETRY_LITTLEFS_EXIT='+str(result.returncode),flush=True)
