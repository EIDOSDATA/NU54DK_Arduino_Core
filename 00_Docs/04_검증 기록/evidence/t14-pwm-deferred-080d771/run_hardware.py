...
evidence={'source':SOURCE,'at':datetime.now(timezone.utc).isoformat(),'swd_frequency_hz':10000000,
          'wiring':'user-confirmed no inter-board jumpers','vectors_per_board':len(vectors),
        ...
        with session:
            ...
            try:
                for vector in vectors:
                    ...
                    row['passed']+=1
                    if row['passed']%100==0:
                        print('PWM_HIL_PROGRESS',digest[:12],row['passed'],flush=True)
                row['postflight']=hil.read_postflight(device)
                assert row['postflight']['pwm_enable']==[0,0,0]
                assert row['postflight']['dppi20_channel0']==0
                ...
                raise
print('TWO_BOARD_PWM_NOJUMPER_PASS',sum(row['passed'] for row in evidence['devices']))
