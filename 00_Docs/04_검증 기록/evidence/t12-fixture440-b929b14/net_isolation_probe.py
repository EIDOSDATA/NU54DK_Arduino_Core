...
evidence = {'status': 'preflight', 'source': SOURCE, 'fixture_id': 440,
    ...
    'swd_frequency_hz': 10000000, 'devices': [], 'results': [], 'functional_pass_claimed': False}
with pair.evidence_session(WORK / 'net-isolation-probe.json', evidence) as journal:
                ...
                for pin in (4, 5):
                    ...
                    for level in (0, 1):
                        ...
                        append({'id': f'NET/{source.image["role"]}/{pin}/{level}', 'driver_role': source.image['role'],
                            'driver_pin': pin, 'level': level, 'observed': observed})
