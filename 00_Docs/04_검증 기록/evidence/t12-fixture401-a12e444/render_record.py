"""! @brief 실제 Fixture 401 결과에서 새 검증 기록을 생성합니다. """
import json
...
def make_record(work):
    ...
    base = 'evidence/t12-fixture401-a12e444'
    return f'''# T12 Fixture 401 current-source PWM→AIN0 실기 검증
...
| 결과 | 첫 실행 **48개 기능 PASS**, 실기 연속 {audit['continuous_elapsed_seconds']}초 |
...
근거와 11:37:59 UTC의 기록 시각을 [체크포인트]({base}/checkpoint.json)에 보존했다.
[확인서]({base}/confirmation.json)는 catalog revision 2, 두 UID SHA·role·exact source·HEX와
...
[USB 재식별]({base}/usb-inventory.json)은 A D/COM5·COM6, B E/COM7·COM8과 정확한 두 UID를 확인했다.
Controller는 B/role 2만 사용했고 역할을 반전하지 않았다.

[build evidence]({base}/target-build-evidence.json), [산출물 색인]({base}/target-artifact-index.json),
[exact image]({base}/exact-images.json)에 clean source의 두 역할·HEX/ELF·설정·빌드 기록 hash를 보존했다.
NCS v3.4.0, bundle dcbdc366a1, GNU Arm 14.3, bundled Python과 pyOCD 0.42.0을 사용했다.
[직전 T11 입력 대조]({base}/build-input-comparison.json)에서 두 역할의 저장소 컴파일 소스,
...
전체 **{audit['samples_read']:,} samples**를 읽어 vector별 SHA-256와 최솟값·최댓값을 기록했다.
48개 모두 LOW 영역(raw <256)과 HIGH 영역(raw >256)을 관측했으며 전체 raw 범위는
{audit['raw_minimum']}~{audit['raw_maximum']}였다. 이 값은 교정 전압이나 ADC 정확도 보증이 아니다.
...
순서로 대응한다. [독립 감사]({base}/results-audit.json)는 별도 작성한 48개 조합과 고유 기능 ID,
전체 순서·길이·상태·해제 결과, final JSON과 journal 일치, exact image/UID·10 MHz를 확인했다.

| 원본 | SHA-256 |
| --- | --- |
| [결과 JSON]({base}/fixture401-attempt1.json) | `{audit['hashes']['fixture401-attempt1.json']}` |
| [journal]({base}/fixture401-attempt1.json.jsonl) | `{audit['hashes']['fixture401-attempt1.json.jsonl']}` |

두 보드 flash는 첫 시도에 통과했다. Sector erase·`auto_unlock=false`이며 mass erase/recover,
SWD 속도 하향과 재시도는 없었다. [종료 read-only 확인]({base}/postflight.json)은 reset/flash 없이
CPUID `0x411fd210`, 전체 40-byte commit과 role을 2/2 검증했다. CPU snapshot은 {states}이다.

사후 감사 초안은 flash metadata의 필드명을 잘못 읽어 `KeyError`가 났다. 실제 schema의
`frequency_hz`로 바로잡아 감사만 다시 수행했다. [초안]({base}/audit_results_initial.py)과
[작성 교정 기록]({base}/authoring-correction.json)을 보존했으며 firmware·HIL 결과 수정이나
실기 재실행은 없었다.
...
[다음 핀맵 대조]({base}/next-wiring-audit.json)에 catalog와 커넥터 원본의 대응을 보존한다.
'''
