...
path = work / 'fixture401-attempt1.json'
...
assert result['fixture_id'] == 401 and result['core_revision'] == source
...
for vector in plan:
    ...
    assert row['id'] == f'V04-ANALOG-SIGNAL/401/2/{vector}/repeat-1'
...
for role, device, image in zip((1, 2), result['devices'], images):
    ...
    assert device['flash']['swd_frequency_hz'] == 10000000
audit = {
    'status': 'passed', 'fixture_id': 401, 'source': source, 'attempt': 1,
