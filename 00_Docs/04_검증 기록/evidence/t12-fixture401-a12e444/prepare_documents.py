"""! @brief 직전 빌드 입력과 Fixture 401/402의 커넥터 대응을 독립 대조합니다. """
...
source = 'a12e444cfb5ef47471c0e0d436f082acfd200c19'
previous = '9a63251ed6f8b9916d8e49d8210414b21c5c7267'
old = json.loads((repo / '00_Docs/04_검증 기록/evidence/t11-fixture301-9a63251/target-artifact-index.json').read_text(encoding='utf-8'))
...
current, following = [next(row for row in catalog['fixtures'] if row['id'] == number) for number in (401, 402)]
def normalize(net):
...
assert [(link['dut'][:2], link['peer'][:2]) for link in following['links']] == [(['P2', 11], ['P4', 12]), (['P2', 30], ['P2', 30])]
record = {'status': 'passed', 'current_fixture': 401, 'next_fixture': 402, 'current': current, 'next': following, 'catalog_revision': catalog['revision'], 'catalog_sha256': hashlib.sha256(catalog_path.read_bytes()).hexdigest(), 'connector_pinmap_matches': True, 'no_next_fixture_executed': True}
(work / 'next-wiring-audit.json').write_text(json.dumps(record, ensure_ascii=False, indent=2) + '\n', encoding='utf-8', newline='\n')
print('T11_INPUT_CONTINUITY_PASS=2; FIXTURE_401_402_PINMAP_PASS')
