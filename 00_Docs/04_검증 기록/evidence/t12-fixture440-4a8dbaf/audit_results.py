...
if passed:
    ...
    assert len(functional) == len(captures) == len(cleanups) == 192
    assert len(density_rows) == 16 and len(journal) == 400
    assert 'PDM density ordering mismatch' in result['error']
    assert mono[(2, 20, 256, 0, 1)][25] == mono[(2, 20, 256, 0, 1)][50] == mono[(2, 20, 256, 0, 1)][75]
...
audit = {'audit_status': 'passed', 'campaign_status': result['status'], 'source': SOURCE,
    ...
    'unexecuted_vectors': 192 - len(functional),
