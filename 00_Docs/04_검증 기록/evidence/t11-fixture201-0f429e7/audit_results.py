...
evidence = json.loads((work / 'fixture201-attempt1.json').read_text(encoding='utf-8-sig'))
journal = [json.loads(line) for line in (work / 'fixture201-attempt1.json.jsonl').read_text(encoding='utf-8-sig').splitlines()]
assert evidence['status'] == 'passed' and evidence['results'] == journal
assert evidence['core_revision'] == source and evidence['fixture_id'] == 201
...
assert len(data) == 18157 and len(errors) == 12 and len(recovery) == 12 and len(concurrent) == 1
assert concurrent[0]['concurrent_pmic'][:2] == [0, 0x41]
assert len(cleanup) == 2 and len(journal) == 18173
...
expected = set()
for controller, instance_a, instance_b in itertools.product((1, 2), (0, 20), (20, 21, 22)):
    prefix = f'201/{controller}/{(instance_a, instance_b)}/'
...
expected.add('V04-SPI-DATA/201/1/(0, 20)/(2000000, 0, 0, 1024, 3, 4)/spim00-twim22-concurrent')
actual = {row['id'] for row in data + errors}
assert len(expected) == len(actual) == len(data + errors) == 18169
...
assert len(coverage) == 12 and coverage['1/(0, 20)'] == 1515
assert collections.Counter(coverage.values()) == {1514: 11, 1515: 1}
result = {'source': source, 'fixture_id': 201, 'status': 'passed', 'swd_frequency_hz': 10000000, 'data_pass': len(data), 'normal_vectors': 18144, 'expected_error_pass': len(errors), 'recovery_data_pass': len(recovery), 'spim00_twim22_concurrent_pass': 1, 'functional_pass': 18169, 'cleanup_records': 2, 'campaign_records': 2, 'journal_records': len(journal), 'coverage_per_role_instance_pair': dict(sorted(coverage.items())), 'continuous_elapsed_seconds': evidence['campaign']['continuous_elapsed_seconds'], 'journal_matches_final_json': True, 'functional_ids_unique': True, 'independent_plan_ids_match': True, 'sha256': {name: hashlib.sha256((work / name).read_bytes()).hexdigest() for name in ('fixture201-attempt1.json', 'fixture201-attempt1.json.jsonl')}}
