...
for old, new in ((WORK.parent / 'r13/u3x-artifact-index.json', WORK / 'target-artifact-index.json'),
                 (Path(r'C:\u3x\m12-build-evidence.json'), WORK / 'target-build-evidence.json')):
    assert not new.exists()
    shutil.copyfile(old, new)
index = json.loads((WORK / 'target-artifact-index.json').read_text(encoding='utf-8'))
baseline = json.loads((WORK.parent / 't12-fixture430/target-artifact-index.json').read_text(encoding='utf-8'))
...
for role, target in enumerate(index['targets'], 1):
    ...
    rows.append({'scenario': target['scenario'], 'source': SOURCE,
                 ...
                 'prior_source': '56a88a5c9b9c68055fccfa9b185a7f1cb6aa4a73', 'artifacts': target['artifacts']})
