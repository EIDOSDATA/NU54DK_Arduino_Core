...
for old, new in ((WORK.parent / "r13/u3q-artifact-index.json", WORK / "target-artifact-index.json"),
                 (Path(r"C:\u3q\m12-build-evidence.json"), WORK / "target-build-evidence.json")):
    assert not new.exists()
    shutil.copyfile(old, new)
index = json.loads((WORK / "target-artifact-index.json").read_text(encoding="utf-8"))
baseline = json.loads((WORK.parent / "t12-fixture407-run/target-artifact-index.json").read_text(encoding="utf-8"))
...
for role, target in enumerate(index["targets"], 1):
    ...
    rows.append({"scenario": target["scenario"], "source": SOURCE,
                 ...
                 "prior_source": "4a64c2562fdd5e9169faecf56b43043a0afec67c", "artifacts": target["artifacts"]})
