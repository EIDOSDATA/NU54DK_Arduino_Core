"""! @brief 현재 408과 다음 420의 GPIO·커넥터 대응을 원본 계약과 대조합니다. """
...
selected = [next(row for row in catalog['fixtures'] if row['id'] == value) for value in (408, 420)]
for fixture in selected:
    for link in fixture['links']:
        for role in ('dut', 'peer'):
            ...
            assert expected == net
assert selected[0]['family'] == 'analog' and selected[1]['family'] == 'qdec'
payload = {'status': 'passed', 'current_fixture': 408, 'next_fixture': 420,
...
print('WIRING_CONTRACT_PASS=408,420;NEXT_FIXTURE_NOT_EXECUTED')
