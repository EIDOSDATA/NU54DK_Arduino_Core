"""! @brief 실제 403 결과와 세 fixture의 coverage에서 검증 기록을 생성합니다. """
import json
import re
...
def make_record(work):
    ...
    base = 'evidence/t12-fixture403-c95b904'
    return f'''# T12 Fixture 403 current-source PWM→AIN2 실기 검증
...
| Build | `C:/u3j` DUT/peer 2/2 build-only PASS, failed/error/warning 0, {seconds}초 |
...
[확인서]({base}/confirmation.json)에 12:04:31 UTC의 원래 기록 시각, catalog revision 2,
...
[402 대비 입력 대조]({base}/build-input-comparison.json)는 두 역할의 컴파일 소스·설정·소속·메모리가
...
{audit['vectors_observing_high_above_256']}개 vector에서 관측했다. 전체 raw 범위는
{audit['raw_minimum']}~{audit['raw_maximum']}이며 교정 전압·ADC 정확도나 PWM 주기·듀티 측정값이 아니다.
...
| 원본 | SHA-256 |
| --- | --- |
| [결과 JSON]({base}/fixture403-attempt1.json) | `{audit['hashes']['fixture403-attempt1.json']}` |
| [journal]({base}/fixture403-attempt1.json.jsonl) | `{audit['hashes']['fixture403-attempt1.json.jsonl']}` |
...
[부분 coverage 감사]({base}/analog-coverage-audit.json)는 이전 401·402의 원본 gzip을 복원하고
