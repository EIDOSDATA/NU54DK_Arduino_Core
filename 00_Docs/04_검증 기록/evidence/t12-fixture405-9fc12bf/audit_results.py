...
path = work / 'fixture405-attempt1.json'
...
assert result['fixture_id'] == 405 and result['core_revision'] == source
assert result['fixture_revision'] == 3 and result['swd_frequency_hz'] == 10000000
...
for vector in plan:
    ...
    assert row['id'] == f'V04-ANALOG-SIGNAL/405/2/{vector}/repeat-1'
    assert row['status'] == 'passed' and row['scope'] == 'open-drain-shared-ain4-manual-saadc'
    ...
    assert row['high_samples'] == sum(value > 256 for value in samples)
    if phase == 1:
        assert row['high_samples'] * 100 >= len(samples) * 95 and row['median'] > 256
    else:
        assert row['high_samples'] == 0 and max(samples) <= 256
    assert row['phase'] == ['low-before', 'released', 'low-after'][phase]
    for key in ('source_readback', 'source_readback_after'):
        assert len(row[key]) == 9
        assert row[key][:7] == [1, phase, 46, 1, 1, 1, int(phase == 1)]
        assert row[key][8] & 0xF0F == 0x80D
...
for offset in range(0, 12, 3):
    ...
    assert group[1]['median'] > max(group[0]['median'], group[2]['median']) + 256
...
audit = {'status': 'passed', 'fixture_id': 405, 'source': source, 'attempt': 1,
         ...
         'source_open_drain_readbacks': 24, 'source_input_release_readbacks': 12,
         ...
         'functional_scope': 'AIN4 PMIC_INT shared input LOW/release/LOW, static open-drain, single/double SAADC DMA and cleanup',
         'not_measured': ['PMIC interrupt generation or register behavior', 'SB1 continuity', 'calibrated ADC precision', 'PWM period/duty', 'Fixture 406/407/408 and later T12', 'T13 soak/concurrency'],
