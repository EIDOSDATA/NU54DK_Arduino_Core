...
shutil.copyfile(work.parent / 'r13/u4j-artifact-index.json', work / 'target-artifact-index.json')
shutil.copyfile(Path('C:/u4j/m12-build-evidence.json'), work / 'target-build-evidence.json')
...
for role, target in enumerate(index['targets'], 1):
    ...
    membership = json.loads((work / 'membership-differences.json').read_text())[role - 1]
    assert membership['added'] == {'C:/ncs/v3.4.0/zephyr/soc/nordic/common/nrf_sys_event.c': 1} and not membership['removed']
    ...
    rows.append({'role': role, 'changed_translation_units': changed, 'source_membership_identical': False, 'source_membership_difference': membership,
