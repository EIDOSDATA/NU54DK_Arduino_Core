"""! @brief 실제 404 결과와 네 fixture의 누적 감사에서 검증 기록을 생성합니다. """
import json
import re
...
def make_record(work):
    ...
    coverage = read('analog-coverage-audit.json')
    post = read('postflight.json')
    checkpoint = read('checkpoint.json')
    assert audit['status'] == coverage['status'] == 'passed'
    assert audit['fixture_id'] == 404 and audit['source'] == checkpoint['source']
    seconds = re.search(r'with no warnings in ([0-9.]+) seconds', (work / 'build.log').read_text(encoding='utf-8-sig')).group(1)
    states = '·'.join(f"{'A' if row['role'] == 1 else 'B'} {row['state']}" for row in post['devices'])
    base = 'evidence/t12-fixture404-e080bbc'
    return f'''# T12 Fixture 404 current-source PWM→AIN3 실기 검증
...
| Build | `C:/u3k` DUT/peer 2/2 build-only PASS, failed/error/warning 0, {seconds}초 |
...
[76번 Fixture 403](76_T12_Fixture_403_current_source_PWM_ADC_검증.md) 완료 뒤 A 쪽 신호를
P1.06에서 P1.07/AIN3로 옮기는 404 안내를 제공했다. 사용자가 “404 진행해”라고 지시하여
직전 결선 안내 상태의 실행 지시로 기록했다. [체크포인트]({base}/checkpoint.json)와
[확인서]({base}/confirmation.json)에 {checkpoint['user_confirmation_recorded_at_utc']}의 원래 기록 시각,
...
| --- | --- | --- |
| B PWM → A AIN3 | **P1.07**, P2-9 | **P1.14**, P4-12 |
| 공통 GND | GND, P2-30 | GND, P2-30 |

양쪽 USB 전원 OFF 상태에서 A 쪽만 이동한 조건이다. B·GND는 유지하며 DAP UART 양쪽 분리·SWD 연결,
동일 I/O 전압·각자 USB 전원·외부 저항/보드 간 전원 rail/다른 출력 없음 조건이다.
Controller는 B/role 2만 사용했다. [USB 재식별]({base}/usb-inventory.json)은
...
[exact image]({base}/exact-images.json)에 두 역할의 새 HEX/ELF·설정·빌드 기록·hash를 보존했다.
NCS v3.4.0·bundle dcbdc366a1·GNU Arm 14.3·bundled Python·pyOCD 0.42.0을 사용했다.
[403 대비 입력 대조]({base}/build-input-comparison.json)는 두 역할의 컴파일 소스·설정·소속·메모리가
직전 c95b904와 같음을 확인했다. Embedded commit identity는 별도 유지한다.
...
전체 **{audit['samples_read']:,} samples**를 읽고 vector별 sample hash·min/max를 기록했다.
LOW(raw <256)는 {audit['vectors_observing_low_below_256']}개, HIGH(raw >256)는 {audit['vectors_observing_high_above_256']}개 vector에서 관측했다.
전체 raw 범위는 {audit['raw_minimum']}~{audit['raw_maximum']}이며 교정 전압·ADC 정확도나 PWM 주기·듀티 측정값이 아니다.
...
[독립 감사]({base}/results-audit.json)는 별도 48개 계획·고유 ID·전체 순서·상태·길이·해제,
JSON/journal 일치·image/UID·10 MHz를 대조했다.

| 원본 | SHA-256 |
| --- | --- |
| [결과 JSON]({base}/fixture404-attempt1.json) | `{audit['hashes']['fixture404-attempt1.json']}` |
| [journal]({base}/fixture404-attempt1.json.jsonl) | `{audit['hashes']['fixture404-attempt1.json.jsonl']}` |

양쪽 flash와 전체 cycle은 첫 실행에서 통과했다. Sector erase·`auto_unlock=false`를 사용했고
mass erase/recover·SWD 하향·재시도는 없었다. [종료 read-only 확인]({base}/postflight.json)은
reset/flash 없이 CPUID `0x411fd210`, full 40-byte commit·role을 2/2 검증했다.
CPU snapshot은 {states}이다.

[부분 coverage 감사]({base}/analog-coverage-audit.json)는 이전 401~403의 원본 gzip을 복원하고
각 SHA를 확인한 뒤 현재 404까지 별도 48개 고유 계획을 대조했다. 네 fixture 합계는 기능
**{coverage['functional_total']}개**, cleanup **{coverage['cleanup_total']}개**, samples **{coverage['samples_total']:,}개**다.
...
## 다음: Fixture 408 PWM→AIN7

양쪽 USB를 분리하고 **A 쪽만 P1.07 → P1.14/AIN7**로 이동한다(P2-9 → P4-12).
**B P1.14와 공통 GND는 그대로** 유지한다. DAP UART 양쪽 분리·SWD 연결·동일 I/O 전압·
각자 USB 전원·다른 출력/전원선 없음 조건을 유지하고 USB 재연결 완료를 확인한다.
AIN7은 LED4 buffer 입력과 공유한다. B만 PWM 출력, A는 SAADC 입력으로 두며 controller role 2만 허용한다.
[다음 핀맵 감사]({base}/next-wiring-audit.json)에 GPIO/connector 대응을 보존했다.
이번에는 408을 실행하지 않았으며 새 확인과 exact HEAD 이미지가 필요하다.
'''
