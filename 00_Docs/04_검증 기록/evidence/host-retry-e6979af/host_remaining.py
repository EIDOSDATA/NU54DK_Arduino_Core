...
(WORK / 'host-remaining.json').write_text(json.dumps({'source': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
    'scope': 'Unexecuted groups after this canonical retry stopped at CMake Ninja execution failure; canonical gate remains failed',
    'prior_attempts': 'host-all.log', 'groups': rows}, indent=2) + '\n', encoding='utf-8')
