...
environment["PATH"] = ';'.join([str(HOST.parent), str(LLVM), "C:/ncs/toolchains/dcbdc366a1/opt/bin", str(MINGW / "bin"),
...
record = {"started_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
          ...
          "build_tools": {name: {"path": str(Path("C:/ncs/toolchains/dcbdc366a1/opt/bin") / name), "sha256": hashlib.sha256((Path("C:/ncs/toolchains/dcbdc366a1/opt/bin") / name).read_bytes()).hexdigest()} for name in ("cmake.exe", "ninja.exe")},
