...
trace = install(pair, WORK / "continuous-setup-trace.jsonl")
statistics_capture = (WORK / 'continuous-statistics.jsonl').open('x', encoding='utf-8', newline='\n')
original_command = pair.Device.command
def capture_statistics(device, opcode, values=(), timeout=10):
    result = original_command(device, opcode, values, timeout)
    if opcode == 39:
        statistics_capture.write(json.dumps({'role': device.image['role'], 'index': values[0], 'words': result}) + '\n')
        statistics_capture.flush()
    return result
pair.Device.command = capture_statistics
...
def record_boot(*args, **kwargs):
    ...
    if len(startup_pins) == 2:
        write_new(WORK / 'continuous-startup-pins.json', {'source': SOURCE, 'devices': startup_pins,
...
capture = (WORK / 'continuous-samples-unused.jsonl').open('x', encoding='utf-8', newline='\n')
...
try:
    raise SystemExit(runner.main(['--dut', UIDS[0], '--peer', UIDS[1], '--build-root', str(BUILD),
        ...
        '--evidence', str(WORK / 'fixture440-continuous.json'), '--repetitions', '1', '--execute-fixture', '--pdm-continuous']))
