...
shutil.copyfile(work.parent / 'r13/u4l-artifact-index.json', work / 'target-artifact-index.json')
shutil.copyfile(Path('C:/u4l/m12-build-evidence.json'), work / 'target-build-evidence.json')
index = json.loads((work / 'target-artifact-index.json').read_text())
prior = json.loads((work.parent / 't12-fixture440-resume/target-artifact-index.json').read_text())
...
for role, target in enumerate(index['targets'], 1):
    ...
    assert sorted(name for name, sha in target['repository_compiled_sources_sha256'].items() if old['repository_compiled_sources_sha256'].get(name) != sha) == ['tests/zephyr/v04_pair_hil/src/main.cpp', 'tests/zephyr/v04_pair_hil/src/signal_hil.cpp']
    ...
    rows.append({'role': role, 'repository_translation_units': len(target['repository_compiled_sources_sha256']),
        'repository_inputs_identical': False, 'changed_TUs': ['main.cpp', 'signal_hil.cpp'], 'added_header': 'pdm_continuous.h', 'membership_identical': True, 'config_identical': True, 'dts_identical': True})
(work / 'build-input-comparison.json').write_text(json.dumps({'source': source,
    'prior_source': '4a8dbafe74e251bbc2897775dae15688ab950823', 'comparisons': rows}, indent=2) + '\n', encoding='utf-8')
