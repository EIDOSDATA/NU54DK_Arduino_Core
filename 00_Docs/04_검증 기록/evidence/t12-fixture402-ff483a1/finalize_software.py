...
destination = repo / '00_Docs/04_검증 기록/evidence/t12-fixture402-ff483a1'
checks = []
...
verification = {'scope': 'Documentation and evidence changes after Fixture 402, no production code changes', 'checks': checks, 'markdown_files': markdown_count, 'contract_tests': 45, 'readiness_blockers': 8}
...
record_path = repo / '00_Docs/04_검증 기록/75_T12_Fixture_402_current_source_PWM_ADC_검증.md'
...
'''
assert record.count('## 다음: Fixture 403 PWM→AIN2') == 1
record_path.write_text(record.replace('## 다음: Fixture 403 PWM→AIN2', section + '## 다음: Fixture 403 PWM→AIN2'), encoding='utf-8', newline='\n')
todo_path = repo / '00_Docs/TODO_v0.4.0.md'
todo = todo_path.read_text(encoding='utf-8')
before = '이번 pair target 2/2 PASS. 문서·contract·inventory 결과는 75번 후속 검증에 기록.'
assert before in todo
todo_path.write_text(todo.replace(before, '이번 pair target 2/2, Markdown 184, contract 45, inventory 75·Serial 23·System 16 PASS; readiness blocker 8개 유지.'), encoding='utf-8', newline='\n')
