...
for function_name in ('choose_exact_port', 'choose_unique_response', 'choose_exact_frame'):
...
evidence = WORK / (case + '-' + SOURCE[:7] + '.json')
...
try:
    ...
    write_new(WORK / (case + '-' + SOURCE[:7] + '-failure.json'), failure)
