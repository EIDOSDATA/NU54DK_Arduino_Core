...
for index, (controller, vector) in enumerate(plan):
    ...
    for role in (1, 2):
        ...
        assert capture['requested_words'] == len(capture['raw_words']) == words
        seed = vector[5] ^ (0x5A5A5A5A if role == 1 else 0)
        mask = (1 << vector[1]) - 1
        if vector[1] <= 16 and vector[2] == 0:
            mask |= mask << 16
        expected = [((seed + 0x9E3779B9 * (offset + 1)) ^ ((offset << 16) | (offset >> 16))) & mask for offset in range(words)]
        assert [value & mask for value in capture['raw_words']] == expected
        payload_hashes.append(hashlib.sha256(struct.pack(f'<{words}I', *capture['raw_words'])).hexdigest())
