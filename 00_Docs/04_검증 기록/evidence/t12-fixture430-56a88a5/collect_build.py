...
for old, new in ((WORK.parent / 'r13/u3v-artifact-index.json', WORK / 'target-artifact-index.json'),
                 (Path(r'C:\u3v\m12-build-evidence.json'), WORK / 'target-build-evidence.json')):
    assert not new.exists()
    shutil.copyfile(old, new)
index = json.loads((WORK / 'target-artifact-index.json').read_text(encoding='utf-8'))
baseline = json.loads((WORK.parent / 't12-fixture420-final/target-artifact-index.json').read_text(encoding='utf-8'))
...
for role, target in enumerate(index['targets'], 1):
    ...
    rows.append({'scenario': target['scenario'], 'source': SOURCE,
                 ...
                 'prior_source': 'a3d0ab59bcee7a7940177fd768ad0f4a7c40c65c', 'artifacts': target['artifacts']})
