...
407 준비 exact 076685a는 INPUT 바이어스·12 vector 구현 및 pair target 2/2·계약·Inventory·정렬을 통과했다. 사용자 407 결선·USB 재연결도 확인했다. Windows Smart App Control이 고정 g++.exe를 차단해 필수 Host 검증이 미완료이므로 **407 flash/HIL NOT RUN**이다. [80번 준비·차단 기록](<80_T12_Fixture_407_준비와_Host_실행_차단.md>)에 원본 진단·다음 행동을 보존했다. 승인된 Host 환경과 전체 회귀를 확보한 뒤 407을 실행하며 **408도 필수**다. 이번 checkpoint는 로컬 보존이며 최종 회귀 전 push하지 않는다.
