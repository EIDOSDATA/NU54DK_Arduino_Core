...
SPIS double buffer는 첫 pair가 armed된 시점에 두 번째 pair의 CPU semaphore acquire를 미리
요청한다. transaction 완료 뒤 `buffer_needed`를 받은 application은 반환된 pair를 채워
`provideNextBuffers()`로 재공급해야 한다. 이 호출은 현재 pair가 DMA-owned인 동안 다음 pair를
선행 예약하며, 다음 CS 이후에 늦게 호출해 생기는 무버퍼 구간을 지원 계약으로 두지 않는다.
초기 두 pair는 `queueBuffers()` 한 번으로 제공하고 이후에는 완료 pair만 순환한다.
