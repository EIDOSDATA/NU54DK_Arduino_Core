...
| M31-W02 raw ISO 기반 | **진행 중** | 고정 시험 SDU의 CIS·BIS·combined·time sync 무선 HIL은 유효. 공개 `RawCis` central/peripheral과 `RawBis` source/receiver의 사용자 payload 두 보드 각 20회 실기 PASS. 나머지 7개 ISO 예제는 `begin()`/`poll()` 중심으로 공개 payload API 재작업 필요 | [공개 CIS 실기](<04_검증 기록/192_M31_W02_공개_CIS_사용자_SDU_실기.md>)·[공개 BIS 실기](<04_검증 기록/193_M31_W02_공개_BIS_사용자_SDU_실기.md>)·[이전 W02 고정 시험 audit](<04_검증 기록/evidence/m31-w02-arduino-e6ae812e/closure-audit.json>). 암호화·time sync·combined 공개 송수신 API와 각 역할 새 build/HIL 후 완료 판정 |
