...
| M31-W02 raw ISO 기반 | **진행 중** | CIG/CIS central·peripheral, BIG/BIS source·receiver, combined CIS/BIS, ISO time sync의 고정 시험 SDU 무선 경로는 HIL PASS. 설치 예제 11개는 `begin()`/`poll()`만 호출해 사용자 payload 송수신 API가 없어 재작업 필요 | 이전 설치 package 역할 예제 11/11 build와 CIS·BIS·암호화·wrong code·sync loss·time sync·세 보드 combined exact HIL 증거는 보존. 공개 ISO 송수신 API·예제·새 build/HIL을 완료 기준으로 추가; [이전 W02 audit](<04_검증 기록/evidence/m31-w02-arduino-e6ae812e/closure-audit.json>) |
