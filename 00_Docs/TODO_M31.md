...
3. 기본 SoftDevice Controller의 AoA CTE 송신과 AoD 미지원 경계를 지킨다. RX/IQ·antenna switching은
   별도 controller·board 적용성 후보다. 대체 Zephyr LL source의 존재로 수신 가능 판정을 만들지 않는다.
...
6. 보드 3개로 가능한 소스 조사 → 계약·parser → 구현·예제 → build → 기능 HIL → 회귀·인계를
   자동 실행 대상으로 삼는다. mapping 불명확, probe lock 충돌, 접근 권한 부족, 필수 외부 장비·peer
   요구가 확인된 해당 행은 `NOT RUN` 또는 근거 있는 `UNSUPPORTED`로 남기고 독립 작업은 계속한다.
...
8. HOST-W04~HOST-W06의 구현·CI를 병행한다. 실제 세 Host 완료는 HOST 원장과 M33 release gate에서
   판정한다. 미확보 Ubuntu/macOS 장비로 인한 `NOT RUN`은 M31 firmware 개발의 선행 차단이 아니다.

## 2. 작업 묶음 — 8개 유지

| 작업 | 상태 | 구현·검증 범위 | 완료 산출물 |
| --- | --- | --- | --- |
| M31-W01 capability·착수 계약 | **미착수** | ISO·전체 Audio profile·DF·CS 적용성, 전체 NCS Bluetooth sample inventory, 역할·자원·시험 기준 고정; 1보드 capability 실행 | M31 착수 계약, readiness와 sample parity JSON, schema/parser·Host negative, capability target·build matrix·exact evidence |
...
| M31-W04 Direction Finding | **미착수** | connectionless/connected AoA CTE TX, AoD 미지원, RX/IQ와 antenna switching source candidate의 controller·board 적용성 조사 | TX 제공 경로·예제·target/HCI evidence, 관찰 가능한 TX 경로 HIL, RX/IQ·AoD의 정확한 판정과 장비 요구 |
...
- [ ] 외부 mic/codec/speaker, 상용 phone/headset 등 필요한 원래 sample의 확장 경로를 별도 표기

## 4. W01에서 먼저 구현할 산출물과 실행 순서

1. 저장소·branch·HEAD·미커밋 변경·board submodule·SDK/toolchain lock, 변경 전 전체 Host 기준선과
   시작 시 exact source의 CI 상태를 기록한다. 다른 작업의 변경 소유권과 실행 중 검사를 보존한다.
...
8. W04 DF와 W05 CS는 controller 적용성에 맞춰 병행할 수 있다. W06 전까지 실패 원인과 profile별
   자원 사용량을 분리한다. HOST-W04 Ubuntu prerequisite·resolver·launcher와 CI도 독립 진행한다.
...
- [ ] 실험적·미지원·외부 장비 요구를 example README와 parity 원장에 일치시킴
...
HOST-W04~HOST-W08을 뜻한다. 실제 Ubuntu/macOS PC가 없으면 해당 장비 행을 `NOT RUN`으로
남기며 M31의 필수 firmware gate에 합산하지 않는다. HOST-W07의 USB/serial/debug/lifecycle와
HOST-W08의 전체 Host 마감은 M33 공개 조건으로 이어진다.

## 7. 보드 3개 자동화 범위와 다음 개입 지점

| 단계 | 자동으로 진행할 범위·최소 보드 | 추가 장비·사람 개입이 필요한 경우 |
| --- | --- | --- |
...
| W03 | 2~3개 합성 PCM/encoded payload와 Audio profile 제어·상태·실제 RF data path | 외부 mic/codec/speaker·상용 phone/headset의 물리 I/O/상호운용을 새로 수행할 때 |
| W04 | 1개 CTE TX 설정·시작/중지·controller event, 2개 적용 가능한 광고/연결 peer 관찰 | 실제 CTE/IQ 수신을 입증할 수 없는 경우 해당 수신/전파 관찰은 NOT RUN; AoA RX는 지원 controller·antenna/receiver 필요 |
| W05 | 2개 CS initiator/reflector·RAS·raw 결과·거리 추정 출력·보안/복구, 3개 peer 분리 | cross-vendor CS peer 또는 별도로 요청한 정밀 거리/각도 시험 |
| W06~W08 | 3개 역할을 순차 재배치한 기능·통합·예제·회귀·원장·문서 인계 | 미확보 외부 peer의 상호운용, 해결되지 않은 필수 기능 결함·mapping/접근 문제 |
| HOST | 장비 독립 manifest·resolver·launcher·negative와 사용 가능한 native CI | 실제 Ubuntu/macOS 설치·USB upload·serial·debug 장비/권한 |

CTE TX 명령 수용·연결 peer 동작만 관찰했다면 그 범위만 기록한다. 수신 IQ 증거가 없는데 CTE
수신이나 각도 측정 PASS로 기록하지 않는다. AoD는 고정 SDC `UNSUPPORTED`, 대체 controller의
AoA RX/IQ는 적용성 판정 전 `source_candidate`다. nRF54L15 대상이 아닌 `nrf_dm`을 CS 대체 예제로
등록하지 않는다.
...
| M31-DF-01 | W04/W07, CTE 역할별 판정 | 적용 TX mode별 start/stop 20회·전체 180초 timeout, 잘못된 CTE type/length 수용 0; RX/IQ·AoD는 별도 상태와 근거 |
...
- [ ] 외부 audio I/O·타사 peer·AoA RX/IQ 및 미지원 역할의 별도 상태 유지; 정밀 RF/음질/각도/거리
  정확도는 이번 기능 gate 밖임을 문서화하고 기능 runtime PASS로 확대하지 않음
...
- [ ] M33 전체 sample catalog·추가 GATT/profile/template·상호운용·세 Host·release gate에 정확한 잔여 인계

근거 있는 `UNSUPPORTED`와 사용자 확정 범위 밖 행은 기능 PASS 수에 넣지 않고 판정 완료 수로
별도 보고한다. 필수 board-only 기능의 `FAIL`/`NOT RUN`이 남으면 해당 W는 미완료다. 추가 장비가
필요한 확장 행의 `NOT RUN`은 그 이유와 후속 소유자를 명시하며 M31 적용 기능 완료와 구별한다.
