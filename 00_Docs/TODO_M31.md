...
| M31-W03 전체 LE Audio profile | **진행 중** | Arduino LC3 내부 loopback과 고정 upstream native BAP unicast의 두 보드 LC3 encode→ISO→server decode 1,000 유효 SDU, 양방향 ISO 각 1,000 SDU를 확인. 서버 무효 packet/PLC 24회는 별도 기록. 공개 Arduino unicast와 §3의 나머지 profile·negative·반복 복구는 잔여 | profile·role별 예제와 build/runtime 상태, 기능 HIL·negative, 자원 예산·외부 I/O 경계; [native BAP LC3](<04_검증 기록/181_M31_W03_native_BAP_LC3_실제_무선_전송.md>) |
...
아래 `W03-01`은 Arduino codec 내부 loopback과 native 보드 간 LC3 한 방향 decode까지,
`W03-02`는 native unicast 양방향 ISO까지 진행했다. 두 행의 공개 Arduino stream 구현과
정량 negative·회복 검증은 미완료이며, 나머지 행은 계획/조사 단계다. 각 행은 source·target·Arduino build·runtime·negative
