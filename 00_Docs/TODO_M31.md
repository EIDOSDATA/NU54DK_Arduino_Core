...
| M31-W02 raw ISO 기반 | **진행 중** | 고정 시험 SDU의 CIS·BIS·combined·time sync 무선 HIL은 유효. 공개 `RawCis` 중앙/주변과 `RawBis` 일반·암호화·시각 동기 source/receiver **8개 역할**의 사용자 payload 두 보드 각 20회 실기 PASS, 잘못된 Broadcast Code의 유효 SDU 수신 0. 나머지 **3개** CIS→BIS 예제는 `begin()`/`poll()` 중심으로 공개 payload API 재작업 필요 | [공개 CIS](<04_검증 기록/192_M31_W02_공개_CIS_사용자_SDU_실기.md>)·[일반 BIS](<04_검증 기록/193_M31_W02_공개_BIS_사용자_SDU_실기.md>)·[암호화 BIS](<04_검증 기록/194_M31_W02_공개_암호화_BIS_사용자_SDU_실기.md>)·[시각 동기 BIS](<04_검증 기록/195_M31_W02_공개_BIS_시각동기_사용자_SDU_실기.md>)·[이전 W02 고정 시험 audit](<04_검증 기록/evidence/m31-w02-arduino-e6ae812e/closure-audit.json>). combined 공개 송수신 API와 각 역할 새 build/HIL 후 완료 판정 |
