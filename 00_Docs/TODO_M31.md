...
2026-09-21 결정으로 **M31 완료 후 v0.5.0 Windows 릴리스**를 준비한다. 현재 요청은 문서 정비를
`main`에 반영하고 **`M31-MEM-OPT`를 생성하는 데서 종료**한다. 다음 구현은 메모리 최적화부터
진행하고 W04·W05 → W06 → W07 → W08로 이어간다. **진행 중**은 잔여가 있다는 원장 상태이며
