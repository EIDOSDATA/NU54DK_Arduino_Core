...
`0.5.0-RC1`에서 W04~W08을 닫았다. M31은 완료했고 공개 승인과 실제 공개는 별도다.
...
| M31-W08 마감·Windows 릴리스 준비 | **완료** | exact private RC 이중 재현, 격리 Windows 설치 예제 113/113 병렬 build, 0.4.1→RC upgrade·uninstall·reinstall/cache 수명주기, 대표 upload/UART/debug와 원장·문서 정합 PASS | [W08 완료 267](<04_검증 기록/267_M31_W08_Windows_RC_준비와_M31_완료.md>) · [exact audit](<04_검증 기록/evidence/m31-w08-close-20260926/closure-audit.json>); 공개는 별도 승인 |
...
2. 별도 공개 승인 전에는 tag/Release/catalog를 만들지 않고 M32/M33·Host 보류 작업도 시작하지 않는다.
...
W01~W08은 완료했다. 1~8번의 원장·기준선은 보존하며 재구현하지 않는다.
후속 작업은 M31과 W08을 다시 열지 않고 별도 공개 승인 지점에서 재개한다.
...
| W08 완료 | 원장·지원표·문서 인계와 Windows RC 준비 PASS | 공개 승인·tag·Release·catalog·공개 download smoke는 별도 gate; 사용자 후속 외부 실물 검증은 비차단 |
...
- [x] M31-W08 소유 v0.5.0 Windows 패키지·설치·RC 준비와 별도 공개 승인 조건을 정리
