...
91,037→70,081 B로 줄였다. GATT schema capacity를 실제 server slot 배열에 연결해 동일 1×1
fixture의 schema·client context·event payload를 실제 선언 용량에 연결해 113,218→60,682 B로
줄였다. server/client 방향·shared TX payload 최적화는 계속 진행한다.
`SPI.begin()` 사용자가
