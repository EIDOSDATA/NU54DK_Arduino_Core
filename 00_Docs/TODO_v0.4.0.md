...
> **지원 종료:** 이 문서는 완료된 v0.4.0 개발·검증의 역사 기록입니다. 현재 설치·지원 버전은
> [v0.4.1](<05_릴리스/v0.4.1/README.md>) 하나이며 v0.4.0은 stable catalog에서 제공하지 않습니다.

v0.4.0은 당시 정식 배포였습니다. 합의한 T13 S/U부터 T22 승인, T23 GitHub Release와 stable
...
후속 M28~M45의 지원 경계·선행 작업은 [127번 계획 정비 기록](<04_검증 기록/127_후속_마일스톤_지원_경계와_착수_계획_정비.md>)과
[v0.5.0 착수 계획](TODO_v0.5.0.md)으로 분리했습니다. 127번 기록 당시에는 후속 마일스톤이
모두 계획·구현 미착수였습니다. 현재 M28·M29·M30 완료와 M31 4/8 상태는 v0.5.0 계획에서 관리하며,
이 역사 기록은 v0.4.0의 완료 상태·지원 범위·공개 패키지를 바꾸지 않습니다.

### 공개 후 설치 도구 유지보수

초기 prerequisite 재사용 검사 오류가 최종 PASS 뒤에 표시되는 로그 문제는 **해결 완료**했습니다.
범위·원인·검사 결과는 [128번 기록](<04_검증 기록/128_Nordic_설치_복구_검증_로그_교정.md>)에 남깁니다.
현재 설치본·SDK·보드와 공개 v0.4.0 자산은 변경하지 않으며 차기 패키지용 저장소 수정입니다.
...
새 PC 준비는 [인계 문서](HANDOFF.md)를 따릅니다.
