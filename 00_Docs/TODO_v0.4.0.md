...
| 상태 | 활성 TODO — R00~R13·current-source T11 완료; T12 401~408·420 개별 시험 PASS, 430 I2S 72개 부분 통과 후 FAIL·교정 대기 |
| 작성·갱신일 | 2026-09-07 |
| 작성 직전 기준 commit | `7c44abffb30402c56cb5e2fd7a1c21829b3b3a16` — 430 부분 72 PASS 뒤 FAIL의 실기 source. C:/u3x pair 2/2 build. 이 문서 commit과 구분 |
| 목표 | 합의한 코어 기능 검증을 마치고 Windows용 `v0.4.0` 정식 공개 및 공개 URL 검증 완료 |
| 다음 착수 항목 | **430 짧은 buffer 오류 위치·시간 진단 → 교정 → 결선 유지 재확인 후 새 exact pair 전체 192개 재검증** |
| 이번 요청의 실행 범위 | 사용자 “결선 완료. 430을 테스트 해.”, 2026-09-06T17:43:49Z. SWD 10 MHz, source 세 단계의 실패·교정·부분 결과를 보존. 최종 72 PASS·1 FAIL·119 미실행과 cleanup 73·postflight 2 확인. 440 미실행. 문서·증거 정리 및 main commit/push |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | [86번](<04_검증 기록/86_T12_Fixture_430_current_source_I2S_검증.md>)에 초기 두 실패·24 startup 진단과 최종 7c44abf 부분 72 PASS·1 FAIL·119 미실행을 분리 보존. 양쪽 cleanup 73회·I2S off·신호 핀 복원·runtime identity 확인. 430 전체는 FAIL |
| 진행 중인 T 항목 | T12/T14 430 짧은 buffer 오류 교정·결선 유지 재확인 대기. 440과 남은 T12 요구 미완료. T14 공용 PWM start_via_task 미시작 STOP timeout 및 T13 이후 유지 |
| 다음 구체적 행동 | 48 kHz·24-bit·stereo·32-word·단일 buffer에서 start→첫 queue 시간, 요청/반환 순서와 오류 위치를 분리 진단한다. 교정 뒤 새 clean HEAD pair와 두 probe/image identity를 확인해 전체 192개 재검증. 서로 다른 source의 부분 PASS를 합산하지 않음 |
| 다음 작업에 필요한 사용자 행동 | 현재 430 결선과 DAP UART 분리·SWD 연결·SB/PMIC 설정이 그대로인지 재확인. 원래 확인은 2026-09-06T18:13:49Z 만료했고 재확인 답변은 아직 없음. 440으로 변경하지 않음 |
| 외부 결선 상태 | 430 A P1.04↔B P1.04(SCK), A P1.05↔B P1.05(LRCK), A P1.06↔B P1.07, A P1.07↔B P1.06, 공통 GND. MCK 없음. 원래 확인 17:43:49Z. 최종 postflight에서 양쪽 I2S off·8개 신호 핀 PIN_CNF=2 |
| 작업 checkout 분리 | 없음. 제품 작업은 `main`에 통합됐고 과거 임시 worktree/branch는 정리했다 |
| 마지막 정식 외부 HIL source | `7c44abffb30402c56cb5e2fd7a1c21829b3b3a16` — 430 부분 72 PASS 뒤 FAIL. 마지막 업로드 양쪽 동일 source. 이후 reset/flash 없이 자원 해제·runtime identity 확인 |
| 작성 당시 readiness | 필수 16개 중 미해결 8개 유지. 420의 정의된 기능·준비 취소 검증 완료; T12 전체·T13 이후·R14·RC·공개는 대기 |
...
| 이 TODO 작성 작업의 실행 중 시험 | C:/u3x build·전체 Host·430 실행·postflight 모두 종료. 양쪽 source 7c44abf, SLEEPING·I2S off·신호 핀 복원. 실행 중 시험 없음 |
...
| 최종 정렬 gate | clang-format 22.1.8, 직접 관리 C/C++/ino 360개 dry-run PASS. 한국어 Doxygen·BSD/Allman·4칸·중괄호 필수. I2S finite helper와 native 검증은 실제 참조되어 유지 |
| CI 확인 | 원격 GitHub Actions 미확인. 이번 exact 7c44abf 전체 Host 659(658 PASS·1 조건부 SKIP), native compiler SKIP 0, pair 2/2·정렬 360 PASS. 430 실기 전체는 FAIL. 전체 target 행렬·package·예제 compile 행렬은 재실행하지 않음 |
| 문서 작업 검증 | 이번 Markdown 195개·raw gzip·Git stage byte 검증은 [86번](<04_검증 기록/86_T12_Fixture_430_current_source_I2S_검증.md>) 참조. 84/85번과 과거 증거는 유지 |
| 최종 HIL 입력 찾기 | 86번 evidence/t12-fixture430-56a88a5·c461182·7c44abf의 source별 폴더. 최신 C:/u3x exact pair. 전체 PASS 감사 script는 준비 파일이며 실제 결과는 partial-results-audit.json. 문서 commit과 업로드 source가 다르므로 다음 flash는 실제 clean HEAD pair 필요 |
...
- [x] **T06 — PDM·I2S·QDEC 합성 신호 프로그램 준비**
  ...
  - 증거: fixture 420/430/440, PDM96·I2S96 vector와 Host 판정 PASS, M25 Stream/pair target build PASS. 준비 시점에는 실기 NOT RUN이었으며 현재 420 기능 48·준비 취소 6 PASS는 [85번](<04_검증 기록/85_T12_Fixture_420_current_source_QDEC_재검증.md>)에 구분한다. 430 실기는 72개 부분 통과 후 FAIL이며 440은 미실행.
...
- [ ] **T12 — M25 입력·출력·스트림 기능 검증**
  - 상태·선행: 부분 완료 — Fixture 401~404·408 각각 PWM 48 PASS·405 AIN4 오픈드레인·406 AIN5/407 AIN6 입력 바이어스 각각 12 PASS, 420 QDEC 기능 48·준비 취소 6 PASS, 430 부분 72 PASS 후 FAIL·440·전체 요구 대기 / T05·T06·T09, R00~R13과 current-source T11 회귀 완료, 해당 T10 확인.
...
- [ ] **T14 — 발견된 결함 수정과 재시험**
  - 상태·선행: 진행 중 / Fixture 101 deferred RX 분기, Fixture 201 RXDELAY, Fixture 301 TWIS 지연 buffer 재개는 각각 exact 수정 뒤 전체 재시험 PASS. R01~R13은 완료했다. 420 HIL의 파형·준비 취소 교정은 exact 6bd8d3f에서 재시험 PASS. 공용 PWM 미시작 start_via_task STOP timeout과 430 48 kHz·24-bit·stereo·32-word 오류는 미해결이다.
...
420 QDEC 최신 결과는 [85번 기록](<04_검증 기록/85_T12_Fixture_420_current_source_QDEC_재검증.md>)의 **exact 6bd8d3f 기능 48·cleanup 48·시작 전 취소 6개 PASS**다. SWD 10 MHz, 22.063초 기능 실행과 두 보드 identity·핀 복원·PWM/QDEC 해제를 확인했다. [84번](<04_검증 기록/84_T12_Fixture_420_current_source_QDEC_검증.md>)의 이전 실패·교정 기록은 유지한다. a3d0ab5와 코드·설정이 같음을 대조했으며 이전 전체 Host 656 PASS·1 조건부 SKIP와 정렬 359 PASS는 해당 source 결과로 구분한다. 430 I2S는 현재 72개 부분 통과 뒤 실패했으며 아래 86번의 교정·재검증이 다음 작업이다. 공용 PWM 지연 시작 취소 이슈는 T14, 440 PDM·남은 T12 요구·T13 이후와 readiness 미해결 8개도 유지한다.

430 I2S 최신 결과는 [86번 기록](<04_검증 기록/86_T12_Fixture_430_current_source_I2S_검증.md>)의 **exact 7c44abf 72 PASS·1 FAIL·119 미실행**이다. 48 kHz·24-bit·stereo·32-word·단일 buffer 오류로 전체 campaign은 실패했다. 양쪽 cleanup 73회와 I2S off·핀 복원·runtime identity를 확인했다. 전체 Host 658 PASS·1 조건부 SKIP, 정렬 360·pair 2/2 PASS이며 초기 두 실패와 별도 startup 진단 24개도 보존했다. 다음은 430 결선 유지 재확인과 짧은 buffer 오류 교정·전체 192개 재검증이다. 440 및 남은 T12·T13 이후·T14 공용 PWM 이슈·readiness 미해결 8개는 유지한다.
