...
| 항목 | 내용 |
| --- | --- |
| 문서 ID / 개정 | TODO-V04-001 / 3.4 |
| 상태 | 활성 TODO — R00~R13·최종 software gate·current-source T11 완료; T12 Fixture 401~403 부분 PASS, 후속 실기·통합·RC·공개 대기 |
| 작성·갱신일 | 2026-09-06 |
| 작성 직전 기준 commit | `c95b9049a62e7c911e4b67104a8f36391ab7e168` — 이번 Fixture 403 exact source |
| 목표 | 합의한 코어 기능 검증을 마치고 Windows용 `v0.4.0` 정식 공개 및 공개 URL 검증 완료 |
| 다음 착수 항목 | **T12 Fixture 404 PWM→AIN3: A P1.06→P1.07 전원 OFF 결선 변경과 사용자 확인** |
| 이번 요청의 실행 범위 | 2026-09-06 사용자 403 시작 지시. Exact pair build, 10 MHz 48-vector 실기, 증거·문서·commit·main push 뒤 404 결선 직전 정지 |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | Fixture 403 exact c95b904 첫 실행 48 PASS·10,368 samples·cleanup 48. [76번 기록](<./04_검증 기록/76_T12_Fixture_403_current_source_PWM_ADC_검증.md>)에 보존. 401~403 합계 기능 144개·samples 31,104개; T12 부분 완료 |
| 진행 중인 T 항목 | 실행 중 시험 없음. T12 Fixture 401~403 완료, 404부터 대기. T11 완료·각 exact 근거 보존; T13~T15·최종 통합·RC/공개 미완료 |
| 다음 구체적 행동 | 두 USB를 분리하고 A 쪽만 P1.06에서 P1.07/AIN3로 이동한다(P2-10→P2-9). B P1.14/P4-12와 공통 GND/P2-30은 유지한다 |
| 다음 작업에 필요한 사용자 행동 | Fixture 404 전원 OFF 결선 변경·DAP UART 양쪽 분리/SWD 연결·동일 I/O 전압·각자 USB 재연결 완료 확인. 외부 저항/전원선/다른 출력 없음 |
| 외부 결선 상태 | 403 GPIO 안내 뒤 사용자 시작 지시 문맥에 따른 확인. B P1.14/P4-12 PWM→A P1.06/P2-10 AIN2, GND/P2-30만 사용. A P1.05 연결 제거 조건. DAP UART 양쪽 분리·SWD 연결·동일 I/O 전압·각자 USB. A D/COM5·6, B E/COM7·8 |
| 작업 checkout 분리 | 없음. 제품 작업은 `main`에 통합됐고 과거 임시 worktree/branch는 정리했다 |
| 마지막 정식 외부 HIL source | `c95b9049a62e7c911e4b67104a8f36391ab7e168` — T12 Fixture 403 첫 실행 48 PASS. 이전 401·402와 T11 exact 근거 보존 |
| 작성 당시 readiness | 필수 16개 중 미해결 8개 유지. T11 단독 완료·T12 401~403 부분 PASS; M24/M25 전체·후속 gate·RC/공개 미완료 |
...
| 이 TODO 작성 작업의 실행 중 시험 | HIL/build 종료. 양쪽 exact c95b904 DUT/peer, 매 vector 양쪽 disarm [0], 종료 read-only CPUID·full commit·role 2/2 PASS. CPU snapshot은 76번 원본 참조. 10 MHz 첫 실행 PASS; 과거 flash/CTS 원인 해결을 주장하지 않음 |
...
| 문서 작업 검증 | 이번 pair target 2/2, Markdown 185, contract 45, inventory 75·Serial 23·System 16 PASS; readiness blocker 8개 유지. 제품·canonical runner 변경 없음; 기존 full software·401·402·T11 exact 근거 보존 |
| 최종 HIL 입력 찾기 | C:/u3j exact c95b904 DUT/peer와 첫 실행 JSON/journal은 76번에 있다. 404는 새 exact HEAD image·확인서가 필요하며 기존 build·원본 증거 보존 |
...
- [ ] **T12 — M25 입력·출력·스트림 기능 검증**
  - 상태·선행: 부분 완료 — Fixture 401 AIN0·402 AIN1·403 AIN2 각각 48 PASS, 후속 fixture·전체 요구 대기 / T05·T06·T09, R00~R13과 current-source T11 회귀 완료, 해당 T10 확인.
  ...
  - 결선·증거: Fixture 401 exact a12e444 48 PASS·10,368 samples·cleanup 48은 [74번 기록](<./04_검증 기록/74_T12_Fixture_401_current_source_PWM_ADC_검증.md>)에 등록. Fixture 402 exact ff483a1 48 PASS는 [75번 기록](<./04_검증 기록/75_T12_Fixture_402_current_source_PWM_ADC_검증.md>)에 보존. Fixture 403 exact c95b904 48 PASS는 [76번 기록](<./04_검증 기록/76_T12_Fixture_403_current_source_PWM_ADC_검증.md>)에 등록. 다음은 404. PWM period/duty capture·ADC calibration/채널 순서 등 전체 T12 요구는 이 HIGH/sample-count 결과로 완료 처리하지 않는다.
