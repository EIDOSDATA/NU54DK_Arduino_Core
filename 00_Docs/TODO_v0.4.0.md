...
정확한 제외 범위·마지막 실패·다음 작업은 [109번 최신 결정](<04_검증 기록/109_T13_S_세_복구_묶음_재검증.md>)을 따른다.

아래 시각별 체크포인트와 역할 전환 준비 설명은 이 결정 이전의 이력이다.

2026-09-08 23:17 KST:1486a13 exact 양쪽 target·Host12·원격 전체 Host828을 확인했다.
역방향 target 선행 START의 SPI20 예행을 통과했고 원래8MHz에서 정식100회 진행 중이다.
I2S B 예행은 underrun 뒤 마지막 DMA 재사용의 첫 word 일부 때문에 중단 꼬리 판정에서
실패했다. 원본 RX·양쪽 TX RAM과 고정 nrfx의 buffers_reused 경로를 대조해 판정을 보완하며,
이전 실제 오류나 이번 실패를 PASS로 바꾸지 않는다. 최신 실행·증거는109번을 따른다.

2026-09-08 23:05 KST: 고정 serial21/21·자원 충돌 사용자 수용 완료는 유지한다.
SPI20 정식은45회와7단계 성공 뒤46회/8단계에서 첫 byte0x60→0x7F로 실패했다.
SPI21도23회/8단계 역방향 첫 byte0xF2→0xFF로 실패했다. SPI22 정식은 미시작이다.
I2S 새 예행은 의도한 B underrun 뒤 마지막 TX word 절단·zero 꼬리까지 거부한 판정 오류를
확인했다. 전체 RX·양쪽 RAM을 보존하고 중단 전 오류와 구분하는 Host9·I2S 원본/시작 순서
Host3을 통과했다. 역방향 SPI/TWI는 target을 먼저 시작하도록 보강한다. 새 exact 빌드·
원격 Host 이후 수정본을 실기로 재검증한다. 이전 I2S 첫 버퍼3word·복구 후1word 오류는
해결한 것으로 처리하지 않는다. 최신109번을 따른다.

2026-09-08 22:43 KST: **고정 serial 취소/NACK21/21 완료**. 새7c0fde3에서 TWIM4조건
각100회·400/400 PASS와 양쪽 STOP을 확인했다. I2S B 예행은 A 첫 버퍼 데이터 오류로
실패했고 정상3초 대조는 PASS다. SPI20·21의 기본10단계 예행은 통과했고22를 실행 중이다.
I2S 공급 중단 전 데이터 오류의 전체 RX 보존과 SPI 진단의 PREPARE ENABLE0 전제를
보강했다. 최신 실기·수정·근거는109번이며 세 묶음의 정식 완료는1/3이다.

2026-09-08 22:13 KST 재개: **자원 충돌은 PASS(사용자 수용)로 완료**다. 실제 예행14/14와
5조건 각100회는 유지하며 나머지9조건 반복은 생략 결정이다. TWIM 취소4조건→I2S B 복구→
SPI20/21/22 역할 전환의 세 묶음을 새 S 유지 확인으로 재검증한다. 실행 소스7c0fde3의
원격15/15·전체 Host819·exact target2/2 준비를 확인했다. 실제 진행과 남은 S 범위는
[109번](<04_검증 기록/109_T13_S_세_복구_묶음_재검증.md>)을 따른다. 아래21:26 종료는 이전 세션 이력이다.
...
미실행 조건·판정 오류 수정·실제 실패·후속 마일스톤은 [108번 최종 기록](<04_검증 기록/108_T13_S_자동_실행_종료와_재개_항목.md>)을 따른다.
...
순서로 가능한 범위를 수행한다. 각 예행 성공 조건만100회를 시작하며 만료 시 미시작·부분 실행을
완료로 세지 않는다. T12 완료·QDEC 제한 보고 유지, T13/U/RC/공개 미완료다.107번 원본을 따른다.
...
[106번 정리 기록](<04_검증 기록/106_Git_이력_정리와_구버전_패키지_공급_종료.md>) 참조.
...
Host 부정 입력/판정 회귀·exact 두 role build·CI 이후 새 source 실기로 진행한다.
현재6782084의 I2S/PDM100회 실행 checkout은 수정하지 않는다.

T13 후속 진단 보완: fd8d4ee의 대체 PWM 경로는 HIL overlay의 PWM capability 누락으로
START 전에 거부됐다. T13 전용 P1.06 capability를 보완하고 실제 PWM PSEL·GPIOTE IRQ enable,
진단 첫 실패의 핀/clock 상태를 보존한다. 제품 정책·정상 통과 기준과 기존 S 물리 결선은 유지한다.
...
T13 후속 준비: f591571 실기와 분리한 C:/tr13dev에서 단독 UART TX/RX 취소·SPIM 취소·TWIM 취소/NACK의
...
T13 PWM 원인 분리 준비: PWM21 LOW535µs 실패 직후 최대8에지 또는10ms만 추가 기록하는
별도 진단 모드를 준비한다. 최초 실패와500±8µs 기준을 그대로 유지하며 정상 soak에는 허용하지 않는다.
양쪽 PWM/DMA·GPIOTE/TIMER 레지스터를 읽기만 해 파형 지연과 capture 지연의 구분 근거를 보강한다.

**현재 작업:** 실패 선 A P2.04↔B P2.02 단독 재검사20/20 PASS 후 사용자 지시로 T13 S 복구·동시·안정성을 재개한다.
[105번](<04_검증 기록/105_T13_S_GPIO_전수_결선_진단.md>)의 최초 전수 진단16정상/1실패와 후속 재검사를 함께 보존한다.
...
| 다음 착수 항목 | **실기 중지 유지. 재개 지시 후 I2S B 복구부터 남은 S 오류 복구·System OFF → U 안내. 연속 전환 재실행 제외** |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | GPIO/GPIOTE 2,502·steady PWM 675·추가 PWM 288·I2S 432 기능 PASS. 각 exact source·새 501·cleanup/postflight 분리 보존. T13 계획 확정 뒤 source별 정상 안정성29/36 근거 확보. 최신104번과 진행 중인 복구 예행 기록을 따른다 |
| 진행 중인 T 항목 | T13 S runner·복구·동시 안정성 준비/실기, T14/T15 알려진 문제 정리와 T17 문서 유지 |
| 다음 구체적 행동 | 연속 전환 제외·마지막 실패와 중지 상태를 보존. 실기 재개 지시 전 보드 접근 없음 |
| 다음 작업에 필요한 사용자 행동 | 현재 실기 중지 지시가 우선. 재개 지시와 당시 결선 확인 조건을 대조하고, U에서는 GPIO 표 안내·재배치 확인 |
| 외부 결선 상태 | S 유지·repair3 보고. 최초 전수16정상/1실패 뒤 실패 선20/20 PASS. 새 전체 사전검사 필수, 원래12:26:14Z 만료 유지 |
| 작업 checkout 분리 | main에477e159 반영. C:/tr13dev는clean477e159 exact 실기 checkout으로 보존하고 문서·증거는main에서 갱신 |
...
| 이 TODO 작성 작업의 실행 중 시험 | f591571 S preflight36/36·PSEL/C01 회귀 PASS, 정식 soak 실행 중. C:/tr13dev의 serial fault5 mode 준비는 Host18·target2/2 PASS이며 아직 실기 아님 |
...
- [ ] **T13 — 복구·동시 실행·장시간 안정성 검증**
  ...
  - 상태·선행: **실기 중지: 정상 안정성36/36·고정 serial21/21·PWM6/6 완료, 자원 충돌 사용자 수용 완료, stream3/4·다른 오류 복구 잔여. 연속 전환은 범위 제외** / T07·해당 T11/T12 단독 PASS·해당 T10 확인. QDEC20/21 단독과 C07은 알려진 문제 보고 후 현재 실행 목록에서 제외하며, T12 완료를 다시 보류하지 않는다.
