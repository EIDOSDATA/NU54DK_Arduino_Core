...
| 다음 착수 항목 | **R12 — BLE·Storage 수명주기 구조 확대** |
...
| 진행 중인 T 항목 | R00~R11 software 완료. [62번 R11](<./04_검증 기록/62_R11_Analog_Stream_peripheral_분리.md>): 124개 본문·3개 header 보존, data 1,000회/재시작 10회, R03 26개 PASS. 전체 Host 621 PASS/2 SKIP, target 6/6 build-only PASS. current-source T11 NOT RUN |
| 다음 구체적 행동 | R12: BLE GAP connection/advertising/scanning, GATT database/server/client, Security pairing/bond/인증과 profile 책임을 private lifecycle 경계로 분리한다. connection reference·generation·stale callback 거부·고정 queue와 재진입을 production Host fault injection으로 검증한다. 이어 EEPROM record/저장·LittleFS slot/partition 수명주기를 분리하고 저장 실패·손상 복구·재부팅 유지 및 R04 reference 동시성을 검증한다. 공개 header·고정 storage 형식·feature 호환성을 보존하며 R13 전체 software gate 뒤 T11 직전에 정지 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R11 전체 Host 및 C:/r11 target 6/6 종료; C:/r11p 기준선 2/2 보존. 새 flash/HIL 없음 |
