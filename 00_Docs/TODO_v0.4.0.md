...
| 항목 | 내용 |
| --- | --- |
| 문서 ID / 개정 | TODO-V04-001 / 2.9 |
| 상태 | 활성 TODO — R00~R13·최종 software gate 완료; current-source T11 UART 101~103·SPI 201·202 PASS, SPI 203/TWI·후속 실기·공개 대기 |
| 작성·갱신일 | 2026-09-06 |
| 작성 직전 기준 commit | `1349e208073d0fd7d3b020a5e9facf771b371237` — 이번 Fixture 202 exact source |
| 목표 | 합의한 코어 기능 검증을 마치고 Windows용 `v0.4.0` 정식 공개 및 공개 URL 검증 완료 |
| 다음 착수 항목 | **Fixture 203 SPI 전원 OFF 결선 변경과 사용자 확인** |
| 이번 요청의 실행 범위 | 2026-09-06 Fixture 202 연결 완료·시작 지시. Exact pair build, 10 MHz 전체 Fixture 202 실기, 실패·최종 증거·문서·commit·main push 뒤 Fixture 203 결선 직전 정지 |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | T11 SPI 202 exact 1349e20 기능 9,084 PASS. 최초 peer flash 실패·읽기 전용 진단 뒤 한 번의 새 실행은 [71번 기록](<./04_검증 기록/71_T11_Fixture_202_current_source_SPI_회귀.md>)에 구분 보존. UART 101~103과 SPI 201의 exact 원본은 67~70번 유지 |
| 진행 중인 T 항목 | 실행 중 시험 없음. Current-source T11 중 SPI 203·TWI 301 대기, T12~T15·RC/공개 미완료 |
| 다음 구체적 행동 | 두 USB 전원을 분리하고 Fixture 203 표대로 A 신호선 네 개를 P2-12/11/10/9로 이동한다. B의 같은 번호 핀과 GND는 유지하며 사용자 결선 완료 확인 뒤 exact HEAD image를 준비한다 |
| 다음 작업에 필요한 사용자 행동 | Fixture 203 SPI 전원 OFF 결선 변경·양쪽 DAP UART 분리/SWD 연결·동일 I/O 전압 확인·USB 재연결 완료 확인 |
| 외부 결선 상태 | 2026-09-06 사용자 Fixture 202 완료 확인. A P2-25/26·P4-4/5 ↔ B P2-12/11/10/9, P2-30 공통 GND. 양쪽 DAP UART 분리·SWD 연결, 각자 USB 전원, 동일 I/O 전압, 전원 rail/외부 pull-up 없음. A D/COM5·6, B E/COM7·8 exact UID SHA 재확인 |
| 작업 checkout 분리 | 없음. 제품 작업은 `main`에 통합됐고 과거 임시 worktree/branch는 정리했다 |
| 마지막 정식 외부 HIL source | `1349e208073d0fd7d3b020a5e9facf771b371237` — Fixture 202 기능 9,084 PASS. 이전 UART·SPI 201과 과거 SPI/TWI exact 원본 보존 |
| 작성 당시 readiness | 필수 16개 중 미해결 8개; current-source UART 101~103·SPI 201·202 PASS. SPI 203/TWI·T12~T15·RC·최종 공개 gate는 미완료 |
...
| 이 TODO 작성 작업의 실행 중 시험 | HIL/build 종료. 양쪽 exact 1349e20 DUT/peer image, controller 역할별 disarm [0], 종료 read-only CPUID·full commit·role 2/2 PASS, CPU snapshot A SLEEPING·B SLEEPING. SWD 10 MHz. 최초 peer flash timeout 원인 미확정; 응답 회복 확인 뒤 한 번 새 실행 PASS, 최초 실패와 과거 103 유사 실패 보존 |
...
| 문서 작업 검증 | 이번 pair target 2/2, Markdown 180, contract 45, inventory 75·Serial 23·System 16 PASS; readiness blocker 8개 유지. Full software gate·T09/BLE·UART·SPI 201은 64~70번 exact 원본 유지. 제품·canonical runner 변경 없음 |
| 최종 HIL 입력 찾기 | C:/u3e의 exact 1349e20 DUT/peer와 최종 attempt2 증거는 71번에 있다. Attempt1 peer flash 실패는 기능 0개·외부 명령 미실행. 이후 문서 HEAD로 재개할 때 새 exact image를 준비한다. 과거 build·실패·raw evidence 보존 |
...
- [ ] **current-source T11 회귀:** R00~R13 최종 exact source로 영향받는 UART·SPI·TWI 단독 기능을
  재검증하고 나서 T12로 전환한다.
  - 진행: exact 154324c Fixture 101 기능 1,644 PASS. [67번 기록](<./04_검증 기록/67_T11_Fixture_101_current_source_UART_회귀.md>) 참조. Fixture 102는 exact a49cc0d 기능 822 PASS로 [68번 기록](<./04_검증 기록/68_T11_Fixture_102_current_source_UART_회귀.md>)에 등록했다. Fixture 103은 exact 7aece93 기능 2,466 PASS로 [69번 기록](<./04_검증 기록/69_T11_Fixture_103_current_source_UART_회귀.md>)에 등록해 승인 UART route 세 묶음을 완료했다. Fixture 201도 exact 0f429e7 기능 18,169 PASS로 [70번 기록](<./04_검증 기록/70_T11_Fixture_201_current_source_SPI_회귀.md>)에 등록했다. Fixture 202도 exact 1349e20 기능 9,084 PASS로 [71번 기록](<./04_검증 기록/71_T11_Fixture_202_current_source_SPI_회귀.md>)에 등록했다. Fixture 203·301은 새 결선 확인 뒤 실행한다.
