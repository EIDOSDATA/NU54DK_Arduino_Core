...
43bc032의 PWM21/22 정상180초와477e159 UART20 TX100회 PASS. 03f5ba4/C:/t4e04의 RX20/21/22/30 예행도 통과해 고정 serial 예행은21/21, 정식100회는1/21이다.
I2S20 양쪽 공급 생략 예행2/2 PASS. PDM20은 실제 overflow 뒤 B 반복 STOP 실패로 중단했고 PDM21은 미실행이다.
리셋 없는 직접 읽기로 양쪽 PDM/SERIAL21 off·GPIO17입력을 확인했다. HIL 해제 성공 뒤 handle을 비우는 수정과 peer 오류 독립 판정을 준비하고 exact 검사를 진행한다.
현재 두 보드는03f5ba4에서 나머지 UART TX100회를 수행한다. 정지 오류 수정 준비를 PDM 복구 PASS로 쓰지 않는다.
...
- **현재 장치:** 03f5ba4/C:/t4e04. 현재 S 확인서·exact UID·SWD10MHz에서 UART TX 정식100회 진행. PDM STOP 수정은 후속 source로 분리한다.
