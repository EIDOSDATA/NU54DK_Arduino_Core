...
| 이번 요청의 실행 범위 | TODO 작성·연결·문서 검사·커밋·푸시만 수행; T01 이후 구현은 별도 착수 요청에 따름 |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | 25단계 TODO 작성과 진입점 연결; 개별 시험 구현에 착수하지 않음 |
| 진행 중인 T 항목 | 없음 |
| 다음 구체적 행동 | T01: 기존 75개 inventory·23개 serial 계약·M25 API를 읽고 인스턴스/모드별 시험표 초안 작성 |
| 다음 작업에 필요한 사용자 행동 | T01~T08의 문서·코드 준비에는 외부 결선 불필요; 실기 flash와 배선은 별도 실행 범위 확인 |
| 외부 결선 상태 | 이 TODO에서 확인하지 않음; 과거 연결 상태를 현재 승인으로 재사용하지 않음 |
...
| 이 TODO 작성 작업의 실행 중 시험 | 없음; 장치 flash·실기 시험은 실행하지 않음 |
| 로컬 임시 build·evidence | 새 경로 생성 안 함; 과거 임시 경로는 존재·revision·hash 확인 전 재사용 금지 |
...
- [ ] **T01 — 최종 시험 목록 확정**
  - 상태·선행: 미착수 / 이 문서와 42번 합의 확인. 결선 불필요.
  ...
  - 증거: 미등록. 수치를 아직 합의하지 않았으므로 임의의 기본값을 확정된 기준으로 기록하지 않는다.

- [ ] **T02 — 현재 코드와 검증 상태 대조**
  - 상태·선행: 미착수 / T01 초안. 결선 불필요.
...
- [ ] **T03 — 두 보드 공통 실행기 준비**
  - 상태·선행: 미착수 / T01·T02. 외부 결선 불필요; 장치 조회 시 현재 연결을 읽기 전용으로 확인.
