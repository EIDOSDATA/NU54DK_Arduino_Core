...
| 다음 착수 항목 | **R11 — Analog/Stream peripheral별 분할** |
...
| 진행 중인 T 항목 | R00~R10 software 완료. [61번 R10](<./04_검증 기록/61_R10_Serial_Fabric_동시_호출.md>): A/B Host 619 PASS/2 SKIP, B target 9/9 build-only PASS; C fixture Host 12/12. current-source T11 NOT RUN |
| 다음 구체적 행동 | R11: Analog SAADC/PWM sequence와 Stream PDM/I2S/QDEC를 각각 private context·driver·IRQ·lifecycle 파일로 분리한다. 기존 family mutex와 Stream DMA metadata spinlock의 단일 소유를 유지하고 factory 및 의미가 같은 내부 queue/변환 helper만 공유한다. R03의 STOP generation·timeout 시 lease 유지·진단을 보존하며 연속/double buffer, overflow·underrun, stop/restart와 복합 호출을 Host 및 target로 검증한다 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R10 모든 software 검사 종료. 다음 R11 기준선 및 production Host 확대. 새 flash/HIL 없음 |
