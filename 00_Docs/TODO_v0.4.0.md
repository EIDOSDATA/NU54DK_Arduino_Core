...
| 진행 중인 T 항목 | R00~R11 및 R12-A/B/C software 완료. [63번 기록](<./04_검증 기록/63_R12_BLE_Storage_수명주기.md>): Security 77개 본문·3개 파일 보존, 실제 Host 12개, 전체 Host 624 PASS/2 SKIP, M21 target 3/3 build-only PASS. R12-D 남음. current-source T11 NOT RUN |
| 다음 구체적 행동 | R12-D: EEPROM mirror/Settings 저장과 순수 record codec, LittleFS path/mode·고정 slot 참조·filesystem mount 수명주기를 private 경계로 분리한다. 기존 12-byte record·CRC·key·1024-byte 한도와 partition/NO_FORMAT 계약은 보존한다. 실제 production Host로 저장·load·mount·sync 실패, record 손상 거부/명시적 복구, 별도 process 재시작 보존, R04 참조 동시성을 검증한다. R13 전체 software gate 뒤 T11 직전에 정지 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R12-C1 전체 Host와 C:/r12c1 M21 target 3/3 종료; C0 기준선 보존. flash/HIL 없음 |
