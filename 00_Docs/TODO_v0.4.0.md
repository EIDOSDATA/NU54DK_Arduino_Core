...
전체 문서의 초기 정리·커밋·푸시는 완료했습니다. 순서1~3의 확정 완료는 **6/58(10.3%)**이며,
...
- **2단계 S:** UART20/30 A와 UART21/22/30 B의 CTS100회,5조건 완료. 다른 CTS 오류와
  ...
  독립 b6ac3bef 대열은 UART21 A break100회를 진행합니다. 수정 parity의 나머지7조건은 아직 미실행입니다.
...
- **진단 보강:** 사용자의 디버거 제안에 따라 exact ELF의 UART 오류 latch에 하드웨어 감시점을
  준비했습니다. 정상 baseline/restart의 첫 오류 기록 직후에 정지해 CPU·UART/DMA·GPIO·클록·
  NVIC·event queue·stack을 읽습니다. 주입 구간에는 설치하지 않으며 진단 결과를 물리 PASS에 합산하지 않습니다.
  현재 시험의 STOP 증거가 확인된 경계에서 한 번 실행하고 독립 S 대열을 재개합니다.
