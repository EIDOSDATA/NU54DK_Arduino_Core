...
| 다음 착수 항목 | **R10 — Serial Fabric 동시 호출과 orchestration 분리** |
...
| 진행 중인 T 항목 | R00~R09 완료. [60번 R09](<./04_검증 기록/60_R09_Arduino_SPI_경계.md>)에 SPI 전후 12개 Host, header 5개/본문 20개 보존, 전체 Host 618 PASS/2 SKIP, target 5개 및 메모리·symbol·소속 비교 보존. R08 M3 source 목록 누락은 b3a6b90에서 보완, 별도 M3 재빌드 PASS. 새 physical NOT RUN |
| 다음 구체적 행동 | R10: 기존 UARTE/SPIM/SPIS/TWIM/TWIS adapter를 유지하며 Serial Fabric registry·IRQ·lifecycle 경계를 분리한다. 같은 handle/block의 stop/cancel/deactivate·submit 교차를 짧은 예약 상태로 보호하고 전역 mutex 밖에서 정지를 기다려 다른 block이 진행하도록 한다. 정지 실패 시 DMA/lease/fault 유지와 Kconfig 상호 배제를 보존하고 실제 Host thread로 먼저 재현·검증한다 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R09 Host/target 종료. C:/r09 SPI 관련 4개 build-only PASS, 최초 M3 링크 실패 보존; C:/r09m 수정 M3 1/1 PASS. 새 flash/HIL 없음 |
