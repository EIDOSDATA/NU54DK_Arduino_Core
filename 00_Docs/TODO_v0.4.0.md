...
| 다음 착수 항목 | **R07 — EventFabric 기계적 분할 파일럿** |
...
| 진행 중인 T 항목 | R00~R06 완료. [57번 R06](<./04_검증 기록/57_R06_builder_모듈과_설치_경로.md>)에 CLI 7개 전후 동일, Host 614 PASS/2 SKIP, 원본·설치 compile 및 한국어/공백 설치 package build 복사본 검증 보존. 새 physical NOT RUN |
| 다음 구체적 행동 | R07: EventFabric 책임·context·전역 상태·ISR/thread·생성물 대응표를 먼저 작성한다. registry 및 TIMER/EGU/GPIOTE/DPPI/PPIB를 기계적으로 추출하고 public header·symbol·mutex·상태/순서를 보존한다. endpoint/domain/channel/자원 회수 Host와 선택/비선택 target, 전후 symbol/메모리 비교를 수행한다 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R06 software/설치 compile 종료. R07 Event target 기준선 1/1 build-only PASS. 장치 시험/flash/HIL 없음 |
