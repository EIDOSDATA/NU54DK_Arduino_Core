...
| 진행 중인 T 항목 | R00~R11 software 완료. R12-A GAP 완료: [63번 기록](<./04_검증 기록/63_R12_BLE_Storage_수명주기.md>), 85개 본문·6개 파일 보존, 실제 Host 10개, 전체 Host 622 PASS/2 SKIP, target 4/4 build-only PASS. R12 B/C/D 진행 예정. current-source T11 NOT RUN |
| 다음 구체적 행동 | R12-B: GATT database/schema·server value/notification·client operation/subscription·공통 lifecycle 경계를 분리한다. 실제 production Host로 disconnect 뒤 늦은 callback, queue overflow, 사용자 callback 재진입, 등록 rollback 및 read/write/subscribe 실패를 검증한다. C Security/profile, D EEPROM/LittleFS를 이어 수행하고 R13 전체 software gate 뒤 T11 직전에 정지 |
...
| 이 TODO 작성 작업의 실행 중 시험 | R12-A 전체 Host 및 C:/r12a3 target 4/4 종료; C:/r12p 기준선 2/2 보존. flash/HIL 없음 |
