...
현재 정식 배포는 **v0.3.0**입니다. v0.4.0은 T13 S 정리와 U 실행 준비를 마치고 U 실기를 진행 중이며
RC·정식 공개 전입니다.
...
| U 준비·실행 | **결선 완료 / 실기 진행 중** | 기존 `dee41fe6` 준비를 바탕으로 UARTE00 4신호+GND 결선 확인. C drive 정리 뒤 exact image 재생성·실기 진행 중 |
...
**현재 물리 단계는 확인된 U 최소 결선 → exact image 재생성 → U 실기입니다.** 완료한 S 시험을 다시 시작하지 않습니다.
그 뒤 T14의 미커버 요구·알려진 제한을 판정하고 T15 지원 확정으로 연결합니다.
현재 요청은 U 실기와 T13 종료까지이며 정식 공개를 포함하지 않습니다.

2026-09-10 후속 실행 지시: 사용자가 UARTE00 최소 결선 4신호+GND 완료와 중단 없는 시험을
지시했습니다. `7f78a36c`에서 U 결선 검사를 net 11·13·15·16으로 한정하고 S의 17신호 검사는
유지했으며 Host 30/30을 통과했습니다. 기존 `C:/tz20` 삭제를 확인해 clean exact source의 두
역할 image를 다시 생성한 뒤 U 실기를 수행합니다. 아직 물리 PASS로 기록하지 않습니다.

T14/T17 문제 문서 **29개 정비 완료**: **해결 완료 / 검증·판정 미완료 / 실기 미실행**을 분리했습니다.
해결한 문제는 아래 상태 칸에 명시하고, 제외된 항목은 현행 문제·잔여 표에서 삭제했습니다.
Markdown 229개 UTF-8·로컬 링크와 diff 검사를 통과했으며 비문서 변경은 없습니다.
원본 이력과 지원 제한은 보존했고 새 실기·커밋·푸시는 수행하지 않았습니다.
...
| T13 U UART00 | **결선 완료 / 실기 진행 중** | 1Mbps 양방향 180초·RTS/CTS·TX/RX DMA 취소/복구의 실제 결과 대기 |
...
- [ ] **T13 — 복구·동시 실행·장시간 안정성 검증**
  ...
  - 결선·증거: [T13 계획](../tests/hil/nu54dk/T13_PLAN.md)과 생성 topology JSON에 원래 계획과 현재 적용 범위를 구분한다. 최신 실행 상태와 제외 결정은 이 문서 상단을 따르며 완료한 S를 다시 실행하지 않는다.
...
| 2 | T14~T15 | 충돌 요구 3종의 검증·지원 판정, 필요한 수정 영향 회귀, instance/mode/route별 최종 지원·readiness 반영 |
