...
| 항목 | 내용 |
| --- | --- |
| 문서 ID / 개정 | TODO-V04-001 / 1.5 |
| 상태 | 활성 TODO — T01~T10 완료, T11 UART 101~103·SPI 201 PASS·나머지 SPI/TWI fixture 대기 |
| 작성·갱신일 | 2026-09-05 |
| 작성 직전 기준 commit | `f21377ee9bbfee35a05748d4e5ba3ab1fd6b79b9` |
| 목표 | 합의한 코어 기능 검증을 마치고 Windows용 `v0.4.0` 정식 공개 및 공개 URL 검증 완료 |
| 다음 착수 항목 | **T11 — Fixture 202 SPI route 결선 확인·실행** |
| 이번 요청의 실행 범위 | 사용자 승인: Fixture 201을 10 MHz SWD로 전체 실행하고 오류 진단·수정·재시험, 문서·commit/push. 다음 fixture는 새 결선 확인 뒤 실행 |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | T01~T10, UART Fixture 101~103, SPI Fixture 201. Exact `f21377e`에서 SPI 계획 ID 18,169개·cleanup 2·campaign 기록 2건, 총 18,173 result를 10 MHz SWD로 PASS |
| 진행 중인 T 항목 | T11 진행 중. Fixture 101~103 UART와 Fixture 201 SPI PASS; Fixture 202~203 SPI와 301 TWI는 `NOT RUN` |
| 다음 구체적 행동 | Fixture 201 점퍼를 제거하고 Fixture 202 결선표를 안내한다. USB 전원을 끈 상태에서 변경한 뒤 새 confirmation으로 실행한다 |
| 다음 작업에 필요한 사용자 행동 | 두 USB를 분리하고 Fixture 201 배선을 모두 제거한 뒤, 안내할 Fixture 202로 재결선하여 두 USB 재연결·완료를 확인한다 |
| 외부 결선 상태 | Fixture 201 결선과 양쪽 `DISABLE_UART` 분리 상태에서 exact `f21377e` 전체 SPI campaign PASS. Fixture 202로 바꾸기 전 반드시 USB 전원을 분리한다 |
| 작업 checkout 분리 | 없음. 제품 작업은 `main`에 통합됐고 과거 임시 worktree/branch는 정리했다 |
| 마지막 정식 외부 HIL source | `f21377ee9bbfee35a05748d4e5ba3ab1fd6b79b9` — Fixture 201 두 보드 exact role image SPI PASS |
...
| 이 TODO 작성 작업의 실행 중 시험 | exact `f21377e` pair 2/2 build, 10 MHz SWD Fixture 201의 계획 SPI ID 18,169개 전부 PASS. 실행 시간 2,180.983초, 실패·누락·범위 이탈 0. 문서 반영 뒤 M12 Host·계약·문서 gate를 재실행한다 |
| 로컬 임시 build·evidence | Fixture 201 image는 `C:/r74`, 진단·성공 원본은 사용자 Documents에 보존. 검증 기록에는 실패 재현 `2a219a9`, 교정 targeted `a254d01`, 전체 PASS `f21377e` evidence와 journal을 등록했다 |
...
| CI 확인 | 이전 `661fad9`의 [Software Gates](https://github.com/EIDOSDATA/NU54DK_Arduino_Core/actions/runs/33913811058)와 [Reproducible Builds](https://github.com/EIDOSDATA/NU54DK_Arduino_Core/actions/runs/33913811030) success 보존. Fixture 201 기록 commit은 push 뒤 새 CI 결과를 확인한다 |
| 문서 작업 검증 | Markdown 148개 UTF-8·내부 링크 PASS, CI contract 45/45·inventory 75·serial 23·system 16·release 16 계약 PASS, package 20/20·M12 Host 전체 PASS. M27 미해결 8개 유지 |
...
- [ ] **T11 — M24 통신 인스턴스 기능 검증**
  - 상태·선행: 진행 중 / Fixture 101~103 UART 정상 4,860·예상 오류 72·cleanup 6건 PASS. Fixture 201 SPI 계획 ID 18,169개·cleanup 2건 PASS. Fixture 202~203 SPI·301 TWI 미실행.
...
- [ ] **T14 — 발견된 결함 수정과 재시험**
  - 상태·선행: 검증 중 반복하는 항목 / Fixture 101 deferred RX의 SPI 분기 진입은 `2542a01`에서 수정·재시험 PASS. Fixture 201의 8 MHz serial SPIM 수신 1-bit 지연은 `a254d01`에서 인스턴스별 RXDELAY로 교정했고 targeted·전체 재시험 PASS. 이후 결함도 계속 이 항목으로 회귀.
  ...
  - 결선·증거: 재시험 대상에 따라 필요. Fixture 201 실패·교정·전체 PASS는 [47번 기록](<./04_검증 기록/47_M24_Fixture_201_SPI_실기_검증.md>)에 등록했다. 새 결함이 나오면 다시 미완료로 전환한다.
