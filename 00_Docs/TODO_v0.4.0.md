...
| 항목 | 내용 |
| --- | --- |
| 문서 ID / 개정 | TODO-V04-001 / 3.16 |
| 상태 | 활성 TODO — R00~R13·기존 source별 T11 완료; T12 401~408·420·430 개별 시험 PASS, 다음 440 PDM 결선 대기 |
| 작성·갱신일 | 2026-09-07 |
| 작성 직전 기준 commit | `36ba819bbe03280fa82c62ef76b00c87a92c2aff` — 430 전체 192개 PASS의 실기 source. C:/u3z pair 2/2와 C:/u4a 영향 target 8/8 build. 이 문서 commit과 구분 |
| 목표 | 합의한 코어 기능 검증을 마치고 Windows용 `v0.4.0` 정식 공개 및 공개 URL 검증 완료 |
| 다음 착수 항목 | **440 PDM 재결선·사용자 확인 → 새 clean HEAD exact pair 검증·실행** |
| 이번 요청의 실행 범위 | 사용자 “뭐가 문제였니? 결선 유지중이야.”, 2026-09-06T18:35:58Z 재확인. T12/T14에서 430 지연 진단·공용 compact token 교정·전체 192개 PASS. SWD 10 MHz, 문서·증거 정리·main commit/push. 440 미실행 |
...
| 필드 | 현재 값 |
| --- | --- |
| 이번에 끝낸 일 | [87번](<04_검증 기록/87_T12_Fixture_430_current_source_I2S_재검증.md>)에 70971f4의 queue 지연·underrun 재현과 36ba819의 전체 192개 PASS·cleanup 192회·postflight 2개를 분리 보존. 수신 384건·82,944 payload word와 시간 trace 384건 독립 감사 PASS |
| 진행 중인 T 항목 | T12는 430 정의된 I2S 기능까지 완료, 440과 나머지 요구 미완료. T14 430 오류는 교정·전체 재검증 PASS. 공용 PWM start_via_task 미시작 STOP timeout 및 T13 이후 유지 |
| 다음 구체적 행동 | 440 catalog·firmware의 고정 role별 3개 신호선 배치를 확인해 안내한다. 사용자 USB 분리·재결선·재연결 확인 뒤 실제 clean HEAD pair·두 probe/image identity를 검사하고 440만 실행한다. 430 PASS를 T12/M25 전체 완료로 확대하지 않음 |
| 다음 작업에 필요한 사용자 행동 | USB 둘 다 분리 후 440 결선: A P1.04↔B P1.05(clock), A P1.05↔B P1.04(gate), A P1.06↔B P1.07(data), 공통 GND. A P1.07·B P1.06은 미연결. DAP UART 분리·SWD 연결·SB/PMIC 유지 후 USB 재연결과 완료 확인 |
| 외부 결선 상태 | 430 유지: A P1.04↔B P1.04, A P1.05↔B P1.05, A P1.06↔B P1.07, A P1.07↔B P1.06, 공통 GND. MCK 없음. 최신 유지 확인 18:35:58Z, 유효기간 19:05:58Z. 마지막 postflight 양쪽 I2S off·8개 신호 핀 PIN_CNF=2. 440 재결선 미확인 |
| 작업 checkout 분리 | 없음. 제품 작업은 `main`에 통합됐고 과거 임시 worktree/branch는 정리했다 |
| 마지막 정식 외부 HIL source | `36ba819bbe03280fa82c62ef76b00c87a92c2aff` — 430 전체 192개 PASS. 양쪽 동일 source. 이후 reset/flash 없이 자원 해제·runtime identity 확인 |
...
| 이 TODO 작성 작업의 실행 중 시험 | C:/u3z·C:/u4a build·전체 Host·계약·Inventory·package·430·postflight 모두 종료. 양쪽 source 36ba819, SLEEPING·I2S off·신호 핀 복원. 실행 중 시험 없음 |
...
| CI 확인 | 원격 GitHub Actions 미확인. Exact 36ba819 전체 Host 660(659 PASS·1 조건부 SKIP), native compiler SKIP 0, 계약 45·Inventory·package 20·정렬 361·pair 2/2·영향 target 8/8 PASS. 430 실기 192 PASS. 전체 target·예제 compile 행렬 및 T11 외부 실기는 이번에 재실행하지 않음 |
| 문서 작업 검증 | Markdown 196개·새 원본 121개 gzip·Git stage byte 검증은 [87번](<04_검증 기록/87_T12_Fixture_430_current_source_I2S_재검증.md>) 참조. 86번과 기존 세 source 증거는 변경 없이 보존 |
| 최종 HIL 입력 찾기 | 87번 evidence/t12-fixture430-70971f4·36ba819의 source별 폴더. 최신 C:/u3z exact pair, results-audit.json·timings-audit.json이 전체 PASS 근거. 문서 commit과 업로드 source가 다르므로 다음 flash는 실제 clean HEAD pair 필요 |
...
- [x] **T06 — PDM·I2S·QDEC 합성 신호 프로그램 준비**
  ...
  - 증거: fixture 420/430/440, PDM96·I2S96 vector와 Host 판정 PASS, M25 Stream/pair target build PASS. 준비 시점에는 실기 NOT RUN이었으며 현재 420 기능 48·준비 취소 6 PASS는 [85번](<04_검증 기록/85_T12_Fixture_420_current_source_QDEC_재검증.md>)에 구분한다. 430 실기는 전체 192개 PASS이며 440은 미실행.
...
- [ ] **T12 — M25 입력·출력·스트림 기능 검증**
  - 상태·선행: 부분 완료 — Fixture 401~404·408 각각 PWM 48 PASS·405 AIN4 오픈드레인·406 AIN5/407 AIN6 입력 바이어스 각각 12 PASS, 420 QDEC 기능 48·준비 취소 6 PASS, 430 I2S 192개 PASS; 440·전체 요구 대기 / T05·T06·T09, R00~R13과 current-source T11 회귀 완료, 해당 T10 확인.
...
- [ ] **T14 — 발견된 결함 수정과 재시험**
  - 상태·선행: 진행 중 / Fixture 101 deferred RX 분기, Fixture 201 RXDELAY, Fixture 301 TWIS 지연 buffer 재개는 각각 exact 수정 뒤 전체 재시험 PASS. R01~R13은 완료했다. 420 HIL의 파형·준비 취소 교정은 exact 6bd8d3f에서 재시험 PASS. 430 짧은 buffer 오류는 exact 36ba819에서 교정·전체 재검증 PASS이며 공용 PWM 미시작 start_via_task STOP timeout은 미해결이다.
