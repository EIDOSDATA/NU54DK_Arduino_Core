...
**2026-09-10 기준: 요청한 S 정리와 U 실행 준비는 완료했습니다.** 이후 전체 문서 정비를 위해
본 저장소 Markdown 228개와 고정 보드 문서 6개를 먼저 검토했습니다. 이번 변경은 문서·생성 안내
정비이며 보드 접근·GPIO 구동·flash·U 실기는 수행하지 않습니다. 검토 범위와 검사 결과는
[114번](<04_검증 기록/114_전체_문서_정비와_남은_마일스톤.md>)에 기록합니다.

**다음 물리 단계는 U 재결선 확인 → U 실기입니다.** 완료한 S 시험을 다시 시작하지 않습니다.
그 뒤 T14의 미커버 요구·알려진 제한을 판정하고 T15 지원 확정으로 연결합니다.
현재 요청은 문서 정비와 남은 작업 보고까지이며 U 실행이나 정식 공개를 포함하지 않습니다.
...
| UART 공유 GPIO 경로 | net5의 LOW 전달 실패·후속 정상화와 net3 구동 중 SWD 단절을 113번에 보존 | 납땜·단선 오류로 확정하지 않음. 완료한 S 범위의 재실행 사유로 삼지 않음 |
...
113번 종료 관측은 두 보드 P1.04 `PIN_CNF=0`과 CPU 실행·sleep 복구입니다. 이번 문서 작업에서
장치에 다시 접속하지 않았으므로 스위치·USB·현재 image 상태를 새로 확인했다고 주장하지 않습니다.
...
- S 정리와 U 준비까지의 요청은 종료했습니다. QDEC 재진단·시리얼 핸드오버·T13 peer 제어
  System OFF 추가 결합 시험은 제외 상태를 유지합니다.
...
- [ ] **T13 — 복구·동시 실행·장시간 안정성 검증**
  ...
  - 남은 실행: UARTE00 1 Mbit/s 8N1 full-duplex 180초, 양 역할 RTS/CTS 정지·재개, TX/RX DMA 취소·fresh restart. 완료한 S 정상 36조건(C05 3600초 포함)과 수용된 충돌·복구를 중복 예약하지 않는다.
...
- [ ] **T14 — 발견된 결함 수정과 재시험**
  - 남은 요구 대조: 110번의 SPI CS 조기 종료, TWIS read-request 공급 지연, 전체 GPIO alias/DMA overlap, 실제 GPIOTE/DPPI active 충돌, PWM과 analogWrite/tone/Servo active 충돌은 기존 S 결과만으로 전 범위 PASS를 주장할 수 없다. 각 요구에 충분한 기존 근거가 있는지 확인하고, 부족하면 필요한 구현·실기 또는 명시적 지원 제한으로 T15에 연결한다. 이번 문서 정리에서 임의로 제외하거나 완료 처리하지 않는다.
...
| 순서 | 단계 | 완료해야 하는 결과 |
| --- | --- | --- |
| 1 | T13 U | 새 U 결선 검사와 UART00 full-duplex·flow control·DMA 취소/복구 실기 |
| 2 | T14~T15 | 미커버 요구 5종과 QDEC 알려진 제한 판정, 수정 영향 회귀, instance/mode/route별 최종 지원·readiness 반영 |
| 3 | T16~T18 | 후보 API·profile·예제를 설치본에 통합, 최종 사용자 문서, stable 생성/검증 절차 준비 |
| 4 | R14·T19~T21 | RC 고정, 전체 회귀·재현 build, 전체 예제 설치/업로드 수명주기, 비공개 stable 패키지 검사 |
| 5 | T22~T24 | exact 결과에 대한 소유자 공개 승인, tag/Release/index 공개, 공개 URL 설치·업로드 검증 |
| 6 | T25 | 최종 기록·커밋·푸시·CI 확인과 정확히 식별한 임시 산출물 정리 |

이는 남은 순서이지 이번 문서 정비에서 실행한 작업 목록이 아닙니다. T14에서 필요한 추가 실기
범위가 아직 확정되지 않았으므로 S의 58조건이나 완료한 T 번호 개수로 v0.4.0 전체 진행률을
계산하지 않습니다. 이번 S 종료·U 준비 목표는 각각 100%이며 정식 릴리스는 미완료입니다.
