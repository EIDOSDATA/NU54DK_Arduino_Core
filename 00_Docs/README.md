...
| v0.5.0 릴리스·다음 구현 | [v0.5.0 TODO](TODO_v0.5.0.md) — M31 완료 후 Windows 릴리스. 다음은 `M31-MEM-OPT`의 메모리 최적화 → W04·W05 → W06~W08 |
| 메모리 최적화 방법·정정 사항 | [통합 설계](<01_아두이노 코어 설계/21_M31_메모리_최적화_통합_설계.md>) — 기능 선택·Kconfig/source/link·정적 pool·계측·회귀. 구현 미착수, 절감량 미확정 |
...
[221번 기록](<04_검증 기록/221_main_마일스톤별_이력_정리.md>)으로 분리합니다. 현재 main에서
후속 `M31-MEM-OPT`를 분기하며 최적화·실기·Host 작업은 아직 실행하지 않았습니다.
