...
100번 현재 실행은 exact 4e48252의 GPIO/GPIOTE campaign이다. 13:55 UTC GPIO 342·task 720·edge 407 PASS,
edge 1440까지 계속 실행 중이며 최종 결과는 TODO/100번을 확인한다. 다음 PWM/QDEC/I2S 준비 pair 2/2와
Host 판정 검사를 수행했다. [T13 조합·GPIO 결선](../tests/hil/nu54dk/T13_PLAN.md)은 32 단독/8 동시,
현재 C→S→U 두 후속 결선 단계로 확정했다. 현재는 결선을 바꾸지 않으며 T13 soak 실행은 하지 않는다.

현재 실행 범위는 [100번](<04_검증 기록/100_T12_공통_기능_묶음과_T13_조합_확정.md>)의 T12 현재 공통 결선 묶음과 T13 조합·추가 결선 확정까지다. 사용자는 현재 결선 유지와 HW 미조작을 확인했다. 고정 세션은 2026-09-08 07:00 KST까지이며 기존 30분 확인서와 구별한다. 아래 이전 실기 결과를 새 기능 코드의 PASS로 재사용하지 않는다.
...
현행 시간 기준: [98번](<04_검증 기록/98_T13_단독_안정성_3분_기준_조정.md>)의 사용자 지시로 T13 단독 안정성은 각 인스턴스 180초다. 후속 99번 사용자 동의로 동시는 각 조합 900초, 전체 대표 고부하 한 조합은 3600초로 대체한다. 아래 이전 인계 당시 600초 계획과 구분하며 최신 재개 상태는 활성 TODO를 따른다.
...
1. T12 PWM peer capture HIL과 독립 판정을 확장하고 Host·target build를 끝낸다.
   PWM20/21/22의 12 slot, period/duty ±5%, 0/25/50/75/100%, TOP 1000/4000과
   load/sequence/loop/DPPI/triggered-step/극성 조건을 시험 목록에 대응한다.
   안전한 GPIO 경로를 회로도·manifest·fixture와 대조한 뒤 사용자에게 결선을 안내한다.
   기존 401~408 ADC 값·sample count PASS는 PWM 파형 측정 PASS가 아니다.
2. GPIO/GPIOTE 승인 pin/channel, I2S 430의 1024 word·100 연속 buffer·TX-only/RX-only,
   QDEC 420의 실행 중 역전·read/clear/restart·invalid transition과 반복 조건을 완료한다.
   TIMER의 이번 compare 10회/조합과 전체 시험표의 반복 조건도 최종 대응한다.
   PDM 440의 기본·연속 기능은 91·92번에서 완료했으므로 이를 빠뜨리거나 불필요하게 처음부터 반복하지 않는다.
3. 해당 T12 단독 PASS 뒤 T13의 오류 복구·handover·86방향 충돌·600초 단독/7200초 동시 soak를 수행한다.
   공유 자원 변경에 영향을 받는 최종-source T11 회귀는 후속 통합에서 계획한다.
