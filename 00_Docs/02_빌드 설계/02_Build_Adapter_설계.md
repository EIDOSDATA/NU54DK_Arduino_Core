...
| --- | --- |
| 문서 상태 | **현재 구현 계약** |
| 현재 정식 버전 | `v0.4.1` (v0.4.0 기능·빌드 계약 유지) |
| 다음 목표 버전 | `v0.5.0` |
...
개발 `M31-MEM-OPT`의 `adaptive` 선택은 source record와 final cache 이관을 유지하면서
다음 단계를 실행한다. P0·P1 구현 범위이며 공개 v0.4.1 builder에는 포함되지 않는다.
