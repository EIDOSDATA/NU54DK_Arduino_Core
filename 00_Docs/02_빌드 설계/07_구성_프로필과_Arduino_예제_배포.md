...
위 수치는 현재 stable `v0.4.1`의 Windows 설치본 계약이다. 공개 `v0.5.0-rc.1`은
16개 library·113개 예제의 discovery·설치본 compile을 완료했다. [RC 설치 안내](../05_릴리스/v0.5.0-rc.1/README.md)와
...
| `secure_ble_dfu` | Secure BLE DFU (MCUboot) | sysbuild·서명·고정 dual-slot layout | v0.5.0 RC의 인증 BLE DFU; 외부 signing key 필수 |
| `ble_audio_io` | BLE Audio external I/O (DAP UART disconnected) | BLE Audio와 외장 I/O 설정 | v0.5.0 RC의 연결 경로; 외장 실물 검증은 사용자 후속 NOT RUN |

`adaptive`·`secure_ble_dfu`·`ble_audio_io`는 v0.5.0 RC에 포함되며 v0.4.1 stable에는 없다. 현재 `boards.txt`의
기본값은 `standard`이고 `adaptive`는 명시적으로 선택하는 실험적 최소 구성 경로다.
`secure_ble_dfu`와 `ble_audio_io`는 각 DFU·Audio 계약을 따르는 별도 구성이다.
...
동일 조건 Nordic native FLASH/RAM 비교도 완료했으며 [M31 TODO](../TODO_M31.md)가 근거를 관리한다.
...
공개 RC 소스는 **library 16개·`.ino` 113개**, W08 설치본 compile은 **113/113**이다.
...
표시하지 않는다. 현재 공개 RC의 최종 상태를 이 과거 snapshot으로 판단하지 않는다.
...
stable 30개와 공개 RC 113개는 서로 다른 패키지 목록이다. 다음 표는 단계별 예제 계약이다.
...
- 해당 릴리스 gate에서 설치 archive의 예제 집합·설정·upstream provenance를 원장과 대조한다.
  M31-W08과 공개 RC는 완료했으며 M32/M33의 추가 예제는 후속 원장과 별도 검증한다.
