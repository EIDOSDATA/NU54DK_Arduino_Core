...
개발 기준은 `main`이다. W03 완료 후 기능 개발을 멈췄고 **HOST-W04 이후는 사용자 지시로 보류**했다.
아래 실행 순서와 병행 계획은 새 기능 작업이 재개될 때 적용한다.
...
재개된 작업은 M31/M32·독립적인 M32-A·M33 예제 준비·Host별로 병행할 수 있다.
추가 외장 장치 확보는 자동 가능한 구현·검사의 선행조건이 아니다.
