...
전체 M22 gate를 새 identity로 다시 실행해 통과했습니다.

- RC1 tag와 7개 공개 자산은 변경하지 않습니다.
- RC1 실패 evidence는 삭제하거나 RC2 PASS로 이름을 바꾸지 않습니다.
- RC2 package, tag와 evidence는 새 identity에 고정했으며 RC1 결과를 상속하지 않았습니다.
...
29/29 compile과 Blink pyOCD Upload는 M22 RC2 gate에서 별도로 통과했습니다.
...
Windows HID의 OS pairing, 실제 입력과 bond 복원은 M21에서 수동 검증했습니다.
...
RC2 package lock과 실제 archive가 일치하며 설치본 29/29 compile gate를 통과했습니다.
...
Maximum Sketch size는 712704 byte입니다. Stable `v0.2.0`에서 RC2 image로 이동하면
application partition 경계와 storage 사용법이 달라질 수 있으므로 중요한 데이터를 먼저
백업하고 [Migration](./MIGRATION.md)을 확인하십시오.

## 배포와 검증 결과

- Public prerelease tag: `v0.3.0-rc.2`
...
Public Release ID는 `380675618`, 공개 시각은 `2026-09-01T16:52:21Z`입니다. 실제 PASS 범위와
evidence hash는 [M22 RC2 검증 기록](<../../04_검증 기록/30_M22_v0.3.0_rc2_통합_릴리스_기준선.md>)을
따릅니다. Stable index는 검증 전후 byte-identical이며 `v0.3.0` 승격은 별도 단계입니다.
