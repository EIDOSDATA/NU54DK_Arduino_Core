...
| 채널 | 별도 RC Boards Manager index를 사용하는 Public GitHub Prerelease 예정 |
| 현재 정식 버전 | `v0.2.0` |
| 공식 사용자 OS | Windows 10/11 x64 |
| 지원 보드 | NU54DK / `nucode:zephyr:nu54dk` |
| 현재 상태 | **공개 전·검증 전 — tag, 자산과 M22 final evidence 미생성** |
| 작성자 | Quantum / NUCODE |

`v0.3.0-rc.2`는 `v0.3.0-rc.1`의 공개 package 기능을 이어받아 M22 전체 공개 설치
수명주기를 다시 검증하기 위한 교정 후보입니다. RC1에서 확인된 문제는 동일 PC 격리
clean-room 도구가 Nordic SDK와 Toolchain 설치 대상 leaf를 먼저 만든 데서 발생했습니다.
일반 사용자 package의 firmware 또는 API 결함으로 판정된 것은 아닙니다.

RC2는 아직 공개되지 않았습니다. 이 디렉터리의 URL, 자산명과 시험 절차는 공개 준비 계약이며,
실제 GitHub Release와 검증 증거가 생기기 전에는 설치 성공이나 M22 PASS를 뜻하지 않습니다.

## 문서

- [Release notes](./RELEASE_NOTES.md): RC2 교정 범위와 이어받는 기능 후보
- [Testing](./TESTING.md): 공개 뒤 GitHub 자산, Arduino IDE 설치와 시험 절차
- [Migration](./MIGRATION.md): Stable 또는 RC1에서 RC2로 이동하고 복귀하는 절차
- [Known issues](./KNOWN_ISSUES.md): 미확정·제외 범위와 공개 전 경계
...
최종 Core commit과 board package gitlink는 RC2 package plan을 생성할 때 고정하고
[M22 RC2 검증 기록](<../../04_검증 기록/30_M22_v0.3.0_rc2_통합_릴리스_기준선.md>)에
기록합니다. RC1의 commit이나 evidence를 RC2 값으로 복사하지 않습니다.
...
위 항목은 RC2 package에 포함하려는 범위입니다. 실제 archive의 파일, manifest, 크기와
SHA-256은 두 번의 독립 package 생성 결과가 byte-for-byte 같을 때만 확정합니다.
...
RC2는 다음 항목을 새 identity로 다시 실행해야 합니다.
...
6. Public clean-room과 final evidence가 완성되기 전에는 RC2 검증 완료를 선언하지 않습니다.
