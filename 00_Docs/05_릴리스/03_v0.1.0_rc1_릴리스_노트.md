...
첫 release candidate다. 정식 `v0.1.0`이 아니며, 모든 기술 gate가 통과해도 라이선스의 최종
법률 검토와 stable 공개 승인은 사람에게 남는다.
...
- Arduino CLI 고정 package의 Blink, library, configuration, negative, parallel, M6~M11 회귀
...
각 RC는 `m11-rc-plan.json`과 gate별 evidence를 결합한
`m11-rc-evidence-manifest.json`을 가진다. 필수 gate가 하나라도 없거나 source/archive가 바뀌면
상태는 `hold`다. 기술 gate가 모두 통과한 상태도 `ready-for-human-approval`일 뿐 stable
공개를 자동 승인하지 않는다.

clean Windows 수명주기는 `0.0.96`→`0.0.97` preview로 검증한다. importer는 두 preview와
...
않는다. RC 전용 index, 고정 RC package의 compile 회귀와 RC HEX 1회 pyOCD+UART HIL은 M11에서
별도로 검증한다. M10 이후 RC commit에는 문서·시험·release automation 경로 변경만 허용하며
