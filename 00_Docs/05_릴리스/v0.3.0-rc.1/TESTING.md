...
이 문서는 GitHub public prerelease를 Arduino IDE에 설치해 확인하는 사용자 절차입니다. Release가
공개되기 전에는 아래 URL이 열리지 않습니다. 당시 stable `v0.2.0`은 그대로 유지됩니다.

## 1. GitHub RC 자산 확인

| 목적 | URL |
| --- | --- |
| Release 페이지 | <https://github.com/EIDOSDATA/NU54DK_Arduino_Core/releases/tag/v0.3.0-rc.1> |
| Boards Manager RC index | <https://github.com/EIDOSDATA/NU54DK_Arduino_Core/releases/download/v0.3.0-rc.1/package_nucode_nu54dk_rc_index.json> |
| Core archive | <https://github.com/EIDOSDATA/NU54DK_Arduino_Core/releases/download/v0.3.0-rc.1/nucode-nu54dk-zephyr-0.3.0-rc.1.zip> |
...
RC ZIP이나 index를 직접 수정하지 마십시오. Checksum을 확인하려면 같은 directory에서 다음처럼
비교할 수 있습니다.

```powershell
Get-FileHash .\nucode-nu54dk-zephyr-0.3.0-rc.1.zip -Algorithm SHA256
Get-Content .\nucode-nu54dk-zephyr-0.3.0-rc.1.CHECKSUMS.sha256
```

## 2. Arduino IDE에 RC index 추가

1. Arduino IDE 2.x를 엽니다. 기준 검증 version은 2.3.10입니다.
2. `File → Preferences`를 엽니다.
3. `Additional Boards Manager URLs`에 아래 URL을 한 줄로 추가합니다.
...
6. Version `0.3.0-rc.1`을 명시적으로 선택하고 설치합니다.
7. Post-install 실행 확인이 나오면 승인하고 Nordic prerequisite 설치가 끝날 때까지 기다립니다.
8. Arduino IDE를 다시 시작합니다.

설치 완료 후 Boards Manager에 `0.3.0-rc.1 installed`가 표시되는지 확인합니다.
...
사용자 예제가 아니므로 메뉴에 나타나지 않습니다.
Standard 예제는 `Feature set = Standard peripherals`, BLE 7개 예제는
`Feature set = BLE NUS`를 선택해 compile·upload하고 시험 뒤 필요한 profile로 되돌립니다.
...
Upload 실패 때 mass erase/recover를 먼저 실행하지 말고 [Troubleshooting](./TROUBLESHOOTING.md)을
따릅니다.
...
- AC-02B 기준 3-wire fixture는 exact `0b7f892`에서 PASS했습니다. 개별 주변장치 예제의 사용자
  시험은 다른 wiring·전원·pin 조합이므로 조건을 함께 별도로 기록하십시오.
- BLE NUS/GAP/GATT 예제는 서로 다른 두 NU54DK 또는 해당 역할을 수행하는 검증 peer가 필요합니다.
...
- Core `0.3.0-rc.1`, Arduino IDE/CLI와 Windows version
...
RC 문제는 [GitHub Issues](https://github.com/EIDOSDATA/NU54DK_Arduino_Core/issues)에 보고할 수 있습니다.
Stable로 복귀하려면 [Migration](./MIGRATION.md)의 downgrade 절차를 따릅니다.
