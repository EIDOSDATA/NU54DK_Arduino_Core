...
> **배포 준비 중:** 이 제약 목록은 rc.2 공개 전 기준이다. rc.2의 실제 Arduino IDE 설치
> 완료 경로와 새 artifact gate가 끝나기 전에는 지원 완료를 선언하지 않는다.
...
- Arduino IDE GUI 자동화는 제한적이다. rc.1에서 index 발견까지만 확인하고 긴
  `post_install` 완료 callback을 놓쳤으므로, rc.2 공개 전에는 실제 IDE 설치 완료 경로를
  별도 검증해야 한다.
