...
> ZIP을 사용한 Arduino IDE 2.3.10 backend 설치 완료 경로를 검증했다.
...
- stable package index와 `v0.1.0` stable Release는 별도 출시 판정 전까지 제공하지 않는다.
- rc.1은 `post_install` gRPC UTF-8 오류로 회수됐다. 설치가 완료됐을 수 있어도 새 설치나
  downgrade 대상으로 사용하지 않는다.
- rc.2 공개 시점에는 probe가 연결되지 않았고 별도 clean Windows PC가 오프라인이었다.
  따라서 새 RC2 byte의 pyOCD HIL 두 범위와 clean Windows 전체 lifecycle은 재실행하지
  않았으며 strict M11 evidence manifest는 HOLD 상태로 공개 자산에서 제외했다.
