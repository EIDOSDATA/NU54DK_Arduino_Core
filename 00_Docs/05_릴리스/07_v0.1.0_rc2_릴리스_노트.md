...
> IDE 2.3.10 backend의 실제 gRPC 설치 완료 응답을 확인했다.
...
새 RC2 byte의 `hil_rc_pyocd`, 기존 M10 범위를 가져오는 `hil_pyocd`, `clean_windows`는 시험
장치 미연결과 원격 PC 오프라인으로 재실행하지 않았다. 이를 PASS로 기록하거나 HOLD evidence
manifest를 공개하지 않았다. RC1과 RC2 archive는 모두 125개 파일이고 변경된 파일은
`platform.txt`, `post_install.bat`, `install-nordic.ps1`, `verify-nordic.ps1`뿐이므로 firmware,
DTS, pin mapping과 Upload 구현은 동일하다. 이 공개는 설치 오류 교정을 검증하기 위한
Prerelease 핫픽스이며 stable 승인으로 해석하지 않는다.
