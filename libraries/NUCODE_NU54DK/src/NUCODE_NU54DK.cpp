...
namespace nucode::nu54dk
{
	namespace
	{
								 ...
								 bool require_watchdog_policy, bool confirms_watchdog_policy) noexcept
		{
			...
			if (require_watchdog_policy &&
				(atomic_get(&pmic_watchdog_policy_confirmed) == 0))
			{
				static_cast<void>(k_mutex_unlock(&pmic_mutex));
	...
	Error BoardSystem::enterSystemOffOnButton(WakeButton button) noexcept
	{
		...
		if (result < 0)
		{
			static_cast<void>(gpio_pin_interrupt_configure_dt(
				specification, GPIO_INT_DISABLE));
			static_cast<void>(k_mutex_unlock(&system_mutex));
			return recordDriverError(result);
		}
		sys_poweroff();
	...
	Error BoardSystem::pmicReadStatus(PmicStatus &status) noexcept
	{
		...
		const std::uint16_t decoded_voltage = static_cast<std::uint16_t>(
			3500U + ((voltage & 0x7FU) * 10U));
	...
	Error BoardSystem::pmicSetChargeVoltage(std::uint16_t millivolts) noexcept
	{
		...
		return mutatePmicRegister(
			PmicRegister::vbat_control, 0x7FU, code, true, false);
	}
	...
	Error BoardSystem::pmicSetChargeCurrent(std::uint16_t milliamps) noexcept
	{
		...
		return mutatePmicRegister(
			PmicRegister::charge_current_control, 0x7FU, code, true, false);
	}
	...
	Error BoardSystem::pmicSetRegisterWatchdog(PmicRegisterWatchdog watchdog) noexcept
	{
		...
		return mutatePmicRegister(
			PmicRegister::ic_control, 0x03U, raw, false, true);
	}

	Error BoardSystem::pmicRequestShutdown() noexcept
	{
		return mutatePmicRegister(
			PmicRegister::ship_reset, 0x60U, 0x20U, true, false);
	}

	Error BoardSystem::pmicRequestShipMode() noexcept
	{
		return mutatePmicRegister(
			PmicRegister::ship_reset, 0x60U, 0x40U, true, false);
	}
}
