...
namespace
{
    ...
    constexpr std::uint32_t servo_period_ns = static_cast<std::uint32_t>(REFRESH_INTERVAL) * 1000U;

    /** @brief microsecond pulse를 nanosecond로 안전하게 변환합니다. */
    [[nodiscard]] constexpr std::uint32_t pulseNanoseconds(std::uint16_t pulse_us) noexcept
    {
        return static_cast<std::uint32_t>(pulse_us) * 1000U;
    }

    /** @brief 객체가 실제로 소유하는 slot을 반환합니다. */
    [[nodiscard]] ServoSlot *ownedSlot(Servo *owner, std::uint8_t index) noexcept
    {
        return index < MAX_SERVOS && servo_slots[index].owner == owner ? &servo_slots[index]
                                                                       : nullptr;
    }

    /** @brief 정수 값을 포함 범위로 제한합니다. */
    [[nodiscard]] constexpr int clampValue(int value, int minimum, int maximum) noexcept
    {
        return value < minimum ? minimum : (value > maximum ? maximum : value);
    }
} // namespace

Servo::Servo() : servo_index_(INVALID_SERVO), minimum_(MIN_PULSE_WIDTH), maximum_(MAX_PULSE_WIDTH)
{
}
...
std::uint8_t Servo::attach(int pin, int minimum, int maximum)
{
    if (k_is_in_isr() || pin < 0 ||
        static_cast<unsigned long>(pin) > static_cast<unsigned long>(static_cast<pin_size_t>(-1)) ||
    ...
    const std::uint16_t initial_pulse =
        static_cast<std::uint16_t>(clampValue(DEFAULT_PULSE_WIDTH, minimum, maximum));
    const PwmRuntimeResult result = nucode::arduino::internal::pwmRuntimeWrite(
        PwmRuntimeClient::servo, logical_pin, servo_period_ns, pulseNanoseconds(initial_pulse));
...
void Servo::detach()
{
    ...
    const PwmRuntimeResult result =
        nucode::arduino::internal::pwmRuntimeStop(PwmRuntimeClient::servo, slot->pin);
...
void Servo::writeMicroseconds(int value)
{
    ...
    const PwmRuntimeResult result = nucode::arduino::internal::pwmRuntimeWrite(
        PwmRuntimeClient::servo, slot->pin, servo_period_ns, pulseNanoseconds(pulse_us));
    if (result == PwmRuntimeResult::success)
    {
...
int Servo::read()
{
    ...
    const std::uint32_t span = static_cast<std::uint32_t>(maximum_ - minimum_);
    const std::uint32_t offset = static_cast<std::uint32_t>(slot->pulse_us - minimum_);
    const int angle = span == 0U ? 0 : static_cast<int>((offset * 180U + span / 2U) / span);
    static_cast<void>(k_mutex_unlock(&servo_mutex));
    return angle;
}
