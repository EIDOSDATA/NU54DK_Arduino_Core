#!/usr/bin/env python3
"""! @brief M31 v0.5.0 Windows RC package를 재현 생성하고 검증합니다. """
...
def validate_contract(repository: Path = REPOSITORY) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        or not isinstance(candidate.get("index_url"), str)
        or not isinstance(candidate.get("evidence"), list)
        or not candidate["evidence"]
    ):
        raise M31ReleaseFailure("공개 RC candidate 계약이 잘못됐습니다")
    for relative in candidate["evidence"]:
        if not isinstance(relative, str):
            raise M31ReleaseFailure("공개 RC candidate 증거 경로가 문자열이 아닙니다")
        path = (root / relative).resolve()
        if not path.is_relative_to(root_resolved) or not path.is_file():
...
def prepare(repository: Path, output: Path, revision: str) -> Path:
    ...
    if "stable_v0.5.0_publication" not in plan["blockers"]:
        raise M31ReleaseFailure("RC 준비 plan에서 stable 별도 공개 승인이 사라졌습니다")
...
def validate_plan(plan_path: Path, *, repository: Path = REPOSITORY) -> dict[str, Any]:
    ...
    if (
        ...
        or not publication_blockers.intersection(plan["blockers"])
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="M31 v0.5.0 Windows RC 준비")
