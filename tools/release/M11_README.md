...
> 릴리스에 그대로 재사용하지 않는다. 현재 공개 상태와 버전별 절차는
> [릴리스 자동화 안내](README.md)를 따른다. M27도 완료한 v0.4.0의 절차이다.
> 당시 버전의 공개 공급은 종료됐으며 아래 명령은 원본 source·자산의 재현 계약을 설명한다.
...
& $Python tools/release/nu54_release.py record-docs `
  ...
  --document "migration=00_Docs/05_릴리스/v0.1.0-rc.2/MIGRATION.md" `
  --document "troubleshooting=00_Docs/05_릴리스/v0.1.0-rc.2/TROUBLESHOOTING.md" `
  --document "release_notes=00_Docs/05_릴리스/v0.1.0-rc.2/RELEASE_NOTES.md" `
  --document "known_issues=00_Docs/05_릴리스/v0.1.0-rc.2/KNOWN_ISSUES.md" `
