# NU54DK Boards Manager 패키징 — v0.1.0 stable / v0.2.0 준비

| 항목 | 내용 |
| --- | --- |
| 현재 정식 버전 | **v0.1.0 공개 완료** |
| 다음 개발 버전 | **v0.2.0** |
...
공개된 `v0.1.0` ZIP, 부속 자산, Release와 stable index 항목은 **덮어쓰거나 다시 만들지
않는다**. 현재 HEAD 이후 변경은 `v0.2.0` 준비선으로 취급하며, 다음 조합은 금지한다.

```text
--version 0.1.0 --commit HEAD
```

stable index에서 기존 `0.1.0` 항목의 URL, size, checksum도 변경하지 않는다. 이후 버전은
새 Release와 새 버전 항목으로 추가한다. 즉 `v0.2.0`을 공개할 때도 기존 `v0.1.0` 항목은
그대로 유지한다.
...
`v0.2.0` 구현과 검증이 완료되면 패키징 도구의 허용 버전, Release tag, index 항목을
`v0.2.0`으로 별도 추가한다. 준비 중인 HEAD 산출물을 `0.1.0`으로 표시하거나 기존
`v0.1.0` Release에 업로드하지 않는다. 정식 산출물은 항상 최종 Release commit/tag를
명시해 만든다.
