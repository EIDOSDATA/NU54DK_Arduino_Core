...
시작합니다. 현재 개발 branch는 `0.5.0-RC1`입니다. stable은 `v0.4.1`, 공개 후보는
`v0.5.0-rc.1`이며 버전별 지원 범위는 [릴리스 문서](<00_Docs/05_릴리스/README.md>)에 있습니다.

## 소스와 환경 준비

```powershell
git clone --branch 0.5.0-RC1 --recurse-submodules https://github.com/EIDOSDATA/NU54DK_Arduino_Core.git
...
2026-09-27에 `0.5.0-RC1`의 main 이후 83개 commit과 문서 정리를 하나로 합치고
`M31-MEM-OPT` branch를 삭제했습니다. 공개 tag `v0.5.0-rc.1`은 원래 source
`7786984a186980f6220271cd506636e4564bc55d`를 계속 가리킵니다. 자세한 대응과 복구 근거는
[269번 기록](<00_Docs/04_검증 기록/269_공개_RC_이력_Squash와_문서_전수_정비.md>)에 있습니다.

기존 checkout은 우선 변경을 commit 또는 별도 복사로 보관한 뒤 `git fetch origin --prune`으로
새 ref를 받습니다. 정리 전 branch와 새 branch를 일반 `pull`로 합치면 원래 이력이 다시 섞일 수
있습니다. 작업을 보존한 채 아래처럼 **사용하지 않는 새 local branch 이름**으로 확인할 수 있습니다.

```powershell
git fetch origin --prune
git switch --create rc1-after-squash --track origin/0.5.0-RC1
git submodule update --init --recursive
```

개인 변경이 있다면 새 branch에서 필요한 commit만 검토해 적용합니다. 공개 RC 재현에는
branch HEAD 대신 `v0.5.0-rc.1` tag를 사용합니다.
