...
- **현재 stable·지원 대상은 v0.4.1이며 공개 후보는 v0.5.0-rc.1입니다.** 이전 stable·RC·preview는 지원·stable catalog
...
- **개발 브랜치의 M28·M29·M30은 완료했습니다.** M30은 W01~W08 8/8, test ID 10/10,
  ...
  3보드 역할 HIL·W08 Windows RC 준비와 공개 RC 다운로드·설치 smoke를 완료했습니다.
  현재 개발 기준은 `main`에서 분기한 **`0.5.0-RC2`**이며, 공개 RC1 사용 중 확인한 예제 설정·
  Upload probe·UTF-8 진단·Verify 진행 표시를 [RC2 TODO](00_Docs/TODO_v0.5.0-RC2.md)에 따라
  교정합니다. RC2 구현·검증과 정식 v0.5.0 stable 승격은 각각 별도 판정입니다.
  2026-09-21 후속 요청으로 main의 미공개 개발 이력을 정리했습니다. 원본 보존·
  새 SHA 대응은 [221번 기록](<00_Docs/04_검증 기록/221_main_마일스톤별_이력_정리.md>)을 따릅니다.
  2026-09-27 사용자 요청으로 `0.5.0-RC1`의 main 이후 이력을 squash하고 `M31-MEM-OPT`를
  삭제했습니다. 공개 RC tag의 원본 source·검증 이력은 그대로 보존합니다.
  대응과 재동기화는 [269번 기록](<00_Docs/04_검증 기록/269_공개_RC_이력_Squash와_문서_전수_정비.md>)과
  [기여 안내](CONTRIBUTING.md)를 따릅니다. 이어진 명시적 요청으로 RC 완료 상태를 main에 반영하고
  v0.4.1 공개 마감 이후 개발 이력을 하나로 통합했습니다. main 정리·보존 범위는
  [270번 기록](<00_Docs/04_검증 기록/270_main_RC_통합_Squash와_문서_동기화.md>)을 따릅니다.
  `0.5.0-RC1` branch와 공개 tag는 그대로 보존하며 main 통합은 stable 공개 승인이 아닙니다.
...
- **2026-09-21 결정: v0.5.0은 완료한 M31을 기준으로 Windows 10/11 x64 범위로 릴리스합니다.**
  ...
  221번·269번·270번의 이력 정리는 해당 사용자 요청에 한정하며 향후 반복 허가가 아닙니다.
