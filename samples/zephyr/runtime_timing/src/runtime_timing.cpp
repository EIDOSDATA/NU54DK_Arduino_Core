...
namespace
{
    ...
    void timingTimerExpiry(k_timer *timer)
    {
        ...
        else
        {
            ...
            if ((elapsed32(now_ms, before_ms) > 1000U) || (elapsed32(now_us, before_us) > 1000000U))
            {
    ...
    void finishPhase(void)
    {
        ...
        nu54_m3_runtime_timing_trace.equal_worker_calls[index] =
            static_cast<std::uint32_t>(atomic_get(&equal_worker_counter)) - baseline_equal_worker;
...
void setup(void)
{
    ...
    const std::uint32_t worker_before = static_cast<std::uint32_t>(atomic_get(&low_worker_counter));
...
void loop(void)
{
    ...
    if (elapsed32(static_cast<std::uint32_t>(millis()), phase_started_ms) >= phase_duration_ms)
    {
        finishPhase();
        ...
        if (current_phase == TimingPhase::Complete)
        {
            ...
            else
            {
                ...
                printk("M3_RUNTIME_TIMING: FAIL %u\n",
                       static_cast<unsigned int>(nu54_m3_runtime_timing_trace.failure));
            }
        }
